## ShazamCore

> `/System/Library/PrivateFrameworks/ShazamCore.framework/Versions/A/ShazamCore`

```diff

 427.0.48.0.0
-  __TEXT.__text: 0x1737c
-  __TEXT.__objc_methlist: 0x1354
+  __TEXT.__text: 0x17384
+  __TEXT.__objc_methlist: 0x136c
   __TEXT.__const: 0x1454
   __TEXT.__gcc_except_tab: 0x178
   __TEXT.__oslogstring: 0x9a3

   __DATA_CONST.__objc_catlist: 0x10
   __DATA_CONST.__objc_protolist: 0x28
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0xcf0
+  __DATA_CONST.__objc_selrefs: 0xcf8
   __DATA_CONST.__objc_superrefs: 0xe8
   __DATA_CONST.__got: 0x2e8
   __AUTH_CONST.__const: 0x1120
   __AUTH_CONST.__cfstring: 0x12a0
-  __AUTH_CONST.__objc_const: 0x29c0
+  __AUTH_CONST.__objc_const: 0x29d8
   __AUTH_CONST.__auth_got: 0x5f0
   __AUTH.__objc_data: 0x48
   __DATA.__objc_ivar: 0x10c

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 767
-  Symbols:   1482
+  Functions: 768
+  Symbols:   1485
   CStrings:  233
 
Symbols:
+ +[SHDeviceFeatures supportsInstantShazam]
+ __OBJC_$_CLASS_METHODS_SHDeviceFeatures
+ __OBJC_$_CLASS_PROP_LIST_SHDeviceFeatures
Functions:
+ +[SHDeviceFeatures supportsInstantShazam]
```
