## GenerativePartnerServiceUI

> `/System/Library/PrivateFrameworks/GenerativePartnerServiceUI.framework/GenerativePartnerServiceUI`

```diff

-291.6.0.5.102
-  __TEXT.__text: 0xc8f64
+297.6.0.5.0
+  __TEXT.__text: 0xcaeec
   __TEXT.__delay_helper: 0xdc
   __TEXT.__objc_methlist: 0x364
-  __TEXT.__const: 0x6b4c
-  __TEXT.__constg_swiftt: 0x20a0
-  __TEXT.__swift5_typeref: 0x10bc1
-  __TEXT.__swift5_reflstr: 0x1984
-  __TEXT.__swift5_fieldmd: 0x1768
+  __TEXT.__const: 0x6b3c
+  __TEXT.__constg_swiftt: 0x20c8
+  __TEXT.__swift5_typeref: 0x10c6b
+  __TEXT.__swift5_reflstr: 0x19b4
+  __TEXT.__swift5_fieldmd: 0x1798
   __TEXT.__swift5_assocty: 0x430
-  __TEXT.__swift5_capture: 0x1530
+  __TEXT.__swift5_capture: 0x1588
   __TEXT.__swift5_builtin: 0x64
-  __TEXT.__swift5_proto: 0x28c
+  __TEXT.__swift5_proto: 0x290
   __TEXT.__swift5_types: 0x1c4
   __TEXT.__swift5_mpenum: 0x20
-  __TEXT.__cstring: 0x38e3
-  __TEXT.__oslogstring: 0x2036
-  __TEXT.__swift_as_entry: 0x158
-  __TEXT.__swift_as_ret: 0x140
-  __TEXT.__swift_as_cont: 0x2ac
+  __TEXT.__cstring: 0x3883
+  __TEXT.__oslogstring: 0x2076
+  __TEXT.__swift_as_entry: 0x160
+  __TEXT.__swift_as_ret: 0x144
+  __TEXT.__swift_as_cont: 0x2bc
   __TEXT.__swift5_protos: 0x20
-  __TEXT.__unwind_info: 0x31b8
-  __TEXT.__eh_frame: 0x4950
+  __TEXT.__unwind_info: 0x3258
+  __TEXT.__eh_frame: 0x4a78
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x1b8
-  __DATA_CONST.__objc_classlist: 0xb0
+  __DATA_CONST.__const: 0x1c8
+  __DATA_CONST.__objc_classlist: 0xa8
   __DATA_CONST.__objc_catlist: 0x8
   __DATA_CONST.__objc_protolist: 0x20
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x5f0
+  __DATA_CONST.__objc_selrefs: 0x608
   __DATA_CONST.__objc_protorefs: 0x10
-  __DATA_CONST.__got: 0xca8
-  __AUTH_CONST.__const: 0x5318
-  __AUTH_CONST.__objc_const: 0x1408
-  __AUTH_CONST.__auth_got: 0x1b90
+  __DATA_CONST.__got: 0xca0
+  __AUTH_CONST.__const: 0x5420
+  __AUTH_CONST.__objc_const: 0x1398
+  __AUTH_CONST.__auth_got: 0x1bc8
   __AUTH.__objc_data: 0x1b8
-  __AUTH.__data: 0xb18
-  __DATA.__data: 0x26f4
-  __DATA.__bss: 0x4598
+  __AUTH.__data: 0xb90
+  __DATA.__data: 0x276c
+  __DATA.__bss: 0x45f8
   __DATA.__common: 0x148
-  __DATA_DIRTY.__objc_data: 0x768
-  __DATA_DIRTY.__data: 0x1de0
+  __DATA_DIRTY.__objc_data: 0x718
+  __DATA_DIRTY.__data: 0x1d30
   __DATA_DIRTY.__bss: 0xc80
   __DATA_DIRTY.__common: 0x50
   - /System/Library/Frameworks/Accounts.framework/Accounts

   - /System/Library/Frameworks/AuthenticationServices.framework/AuthenticationServices
   - /System/Library/Frameworks/Combine.framework/Combine
   - /System/Library/Frameworks/CoreFoundation.framework/CoreFoundation
+  - /System/Library/Frameworks/CoreGraphics.framework/CoreGraphics
   - /System/Library/Frameworks/CoreServices.framework/CoreServices
   - /System/Library/Frameworks/Foundation.framework/Foundation
   - /System/Library/Frameworks/SafariServices.framework/SafariServices

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 5068
-  Symbols:   313
-  CStrings:  415
+  Functions: 5143
+  Symbols:   318
+  CStrings:  413
 
Symbols:
+ _CGColorCreateCopyByMatchingToColorSpace
+ _CGColorSpaceCreateWithName
+ _OBJC_CLASS_$_UIGraphicsImageRendererFormat
+ _UIImagePNGRepresentation
+ _kCGColorSpaceSRGB
CStrings:
+ "View.task @ GenerativePartnerServiceUI/ExternalProviderEnableSheet.swift:"
+ "[ConcreteExternalProviderEnableSheet] opening privacy link"
+ "[ConcreteExternalProviderEnableSheet] presenting enablement sheet for %{public}s"
+ "[ConcreteExternalProviderEnableSheet] user dismissed sheet via Not Now button"
+ "[ConcreteExternalProviderEnableSheet] user opted in to %{public}s"
+ "received selectedLLMDidChange"
- ",” it can work with your device to do more for you. “"
- "When you use the extension from “"
- "[ExternalProviderEnableSheet] opening privacy link"
- "[ExternalProviderEnableSheet] presenting enablement sheet for %{public}s"
- "[ExternalProviderEnableSheet] user dismissed sheet via Not Now button"
- "[ExternalProviderEnableSheet] user opted in to %{public}s"
- "section title"
- "” terms of use and privacy policy apply."
```
