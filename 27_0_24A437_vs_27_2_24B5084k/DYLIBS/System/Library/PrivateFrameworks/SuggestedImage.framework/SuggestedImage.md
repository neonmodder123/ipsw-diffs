## SuggestedImage

> `/System/Library/PrivateFrameworks/SuggestedImage.framework/SuggestedImage`

```diff

-198.1.102.0.0
-  __TEXT.__text: 0xf80dc
-  __TEXT.__objc_methlist: 0x3f4
-  __TEXT.__const: 0x7aad
+198.2.7.0.0
+  __TEXT.__text: 0xfbfbc
+  __TEXT.__objc_methlist: 0x404
+  __TEXT.__const: 0x7b7d
   __TEXT.__gcc_except_tab: 0xa84
-  __TEXT.__cstring: 0x5a26
+  __TEXT.__cstring: 0x5b26
   __TEXT.__dlopen_cstrs: 0x64
-  __TEXT.__constg_swiftt: 0x1c74
-  __TEXT.__swift5_typeref: 0x1678
+  __TEXT.__constg_swiftt: 0x1c90
+  __TEXT.__swift5_typeref: 0x16aa
   __TEXT.__swift5_builtin: 0xdc
-  __TEXT.__swift5_reflstr: 0x17c7
-  __TEXT.__swift5_fieldmd: 0x1bf4
-  __TEXT.__swift5_assocty: 0x288
-  __TEXT.__swift5_proto: 0x444
-  __TEXT.__swift5_types: 0x204
+  __TEXT.__swift5_reflstr: 0x1837
+  __TEXT.__swift5_fieldmd: 0x1c34
+  __TEXT.__swift5_assocty: 0x2a0
+  __TEXT.__swift5_proto: 0x44c
+  __TEXT.__swift5_types: 0x208
   __TEXT.__swift_as_entry: 0x39c
   __TEXT.__swift_as_cont: 0xb34
   __TEXT.__swift5_protos: 0x34
-  __TEXT.__oslogstring: 0x21c8
+  __TEXT.__oslogstring: 0x22f8
   __TEXT.__swift5_mpenum: 0x80
   __TEXT.__swift_as_ret: 0x678
   __TEXT.__swift5_capture: 0x59c
-  __TEXT.__unwind_info: 0x4390
-  __TEXT.__eh_frame: 0xc508
+  __TEXT.__unwind_info: 0x43d0
+  __TEXT.__eh_frame: 0xc520
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_selrefs: 0x4c8
   __DATA_CONST.__objc_superrefs: 0x38
   __DATA_CONST.__objc_arraydata: 0x20
-  __DATA_CONST.__got: 0x648
-  __AUTH_CONST.__const: 0x4b70
+  __DATA_CONST.__got: 0x650
+  __AUTH_CONST.__const: 0x4c80
   __AUTH_CONST.__cfstring: 0x160
-  __AUTH_CONST.__objc_const: 0x1950
+  __AUTH_CONST.__objc_const: 0x1960
   __AUTH_CONST.__weak_auth_got: 0x40
   __AUTH_CONST.__objc_dictobj: 0x50
-  __AUTH_CONST.__auth_got: 0x1340
+  __AUTH_CONST.__auth_got: 0x1368
   __AUTH.__objc_data: 0x170
   __AUTH.__data: 0x6e0
   __DATA.__objc_ivar: 0x34
-  __DATA.__data: 0xca8
-  __DATA.__bss: 0x6b40
-  __DATA.__common: 0x19
+  __DATA.__data: 0xd48
+  __DATA.__bss: 0x6c40
+  __DATA.__common: 0x49
   __DATA_DIRTY.__objc_data: 0x448
   __DATA_DIRTY.__data: 0x1a88
   __DATA_DIRTY.__bss: 0x1a80

   - /usr/lib/swift/libswift_DarwinFoundation1.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 3875
-  Symbols:   1822
-  CStrings:  461
+  Functions: 3901
+  Symbols:   1825
+  CStrings:  472
 
Symbols:
+ _symbolic _____ 14SuggestedImage15SecureContainerV5ErrorO
+ _symbolic ______p15underlyingError_t s5ErrorP
+ _symbolic _____ySS_____G s18_DictionaryStorageC 14SuggestedImage20WallpaperPosterStyleO
+ _type_layout_string 14SuggestedImage15SecureContainerV5ErrorO
- _objc_msgSend$temporaryDirectory
CStrings:
+ "' security group"
+ "/dev/null/SuggestedImage-container-unavailable"
+ "43D4DAE8-A9AE-4724-A41C-E3ABD4DA6C37"
+ "Cannot exclude directory from backup: %{private}@"
+ "Cannot exclude the trie directory from backup: %{private}@"
+ "Classic 90's Anime"
+ "FedStats Phrases"
+ "Generation run was cancelled mid-stream after %ld image(s); abandoning this request."
+ "Generation run was cancelled; abandoning the remaining pending requests."
+ "Shared secure container is inaccessible; this task must be entitled with the '"
+ "fedstatsPhrases"
+ "specifier"
+ "title"
- "3D figurines style"
- "SuggestedImage-unavailable"
```
