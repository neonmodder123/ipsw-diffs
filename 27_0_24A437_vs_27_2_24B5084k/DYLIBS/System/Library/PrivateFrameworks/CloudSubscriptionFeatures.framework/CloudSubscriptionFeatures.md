## CloudSubscriptionFeatures

> `/System/Library/PrivateFrameworks/CloudSubscriptionFeatures.framework/CloudSubscriptionFeatures`

```diff

-301.24.0.31.0
-  __TEXT.__text: 0x11db80
+301.24.1.3.0
+  __TEXT.__text: 0x11e56c
   __TEXT.__objc_methlist: 0xea4
-  __TEXT.__const: 0xbbd4
+  __TEXT.__const: 0xbda4
   __TEXT.__cstring: 0x4861
-  __TEXT.__oslogstring: 0x767d
+  __TEXT.__oslogstring: 0x76cd
   __TEXT.__gcc_except_tab: 0x60
   __TEXT.__dlopen_cstrs: 0xc4
-  __TEXT.__constg_swiftt: 0x2c44
-  __TEXT.__swift5_typeref: 0x2b80
+  __TEXT.__constg_swiftt: 0x2c24
+  __TEXT.__swift5_typeref: 0x2b6e
   __TEXT.__swift5_builtin: 0xf0
-  __TEXT.__swift5_reflstr: 0x2681
-  __TEXT.__swift5_fieldmd: 0x2ed4
+  __TEXT.__swift5_reflstr: 0x2701
+  __TEXT.__swift5_fieldmd: 0x2f00
   __TEXT.__swift5_assocty: 0x4d0
-  __TEXT.__swift5_proto: 0x88c
+  __TEXT.__swift5_proto: 0x888
   __TEXT.__swift5_types: 0x36c
-  __TEXT.__swift5_capture: 0x18b4
-  __TEXT.__swift5_protos: 0x9c
+  __TEXT.__swift5_capture: 0x1814
+  __TEXT.__swift5_protos: 0x98
   __TEXT.__swift_as_entry: 0x3e4
   __TEXT.__swift_as_ret: 0x3fc
   __TEXT.__swift_as_cont: 0x940
   __TEXT.__swift5_mpenum: 0x18
-  __TEXT.__unwind_info: 0x4478
-  __TEXT.__eh_frame: 0xb838
+  __TEXT.__unwind_info: 0x4488
+  __TEXT.__eh_frame: 0xb808
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_protorefs: 0x48
   __DATA_CONST.__objc_superrefs: 0x28
   __DATA_CONST.__got: 0x578
-  __AUTH_CONST.__const: 0x9800
+  __AUTH_CONST.__const: 0x97d8
   __AUTH_CONST.__cfstring: 0x480
   __AUTH_CONST.__objc_const: 0x35d0
   __AUTH_CONST.__auth_got: 0x1138
   __AUTH.__objc_data: 0x3f0
   __AUTH.__data: 0x7c0
   __DATA.__objc_ivar: 0x38
-  __DATA.__data: 0x17e0
+  __DATA.__data: 0x17d0
   __DATA.__bss: 0xc9c0
   __DATA.__common: 0x8
   __DATA_DIRTY.__objc_data: 0xf28
-  __DATA_DIRTY.__data: 0x1fa0
+  __DATA_DIRTY.__data: 0x1f90
   __DATA_DIRTY.__bss: 0x2be0
   __DATA_DIRTY.__common: 0x48
   - /System/Library/Frameworks/Accounts.framework/Accounts

   - /usr/lib/swift/libswift_DarwinFoundation1.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 5052
-  Symbols:   2059
-  CStrings:  971
+  Functions: 5058
+  Symbols:   2060
+  CStrings:  972
 
Symbols:
+ ___swift_exist.box.addr_destructor.6Tm
+ ___swift_memcpy248_8
+ ___swift_memcpy448_8
+ _associated conformance 25CloudSubscriptionFeatures03AllC18RequestCachePolicyVAA07FeatureefG0AA11ReturnValueAaDP_s23CustomStringConvertible
+ _objc_msgSend$urlStringForKey:
+ _symbolic $s25CloudSubscriptionFeatures25FeatureRequestCachePolicyP
+ _symbolic 10Identifier_____Qy_ 25CloudSubscriptionFeatures25FeatureRequestCachePolicyP
+ _symbolic 11ReturnValue_____Qy_ 25CloudSubscriptionFeatures25FeatureRequestCachePolicyP
+ _symbolic 11ReturnValue_____Qz 25CloudSubscriptionFeatures25FeatureRequestCachePolicyP
+ _symbolic _____ 25CloudSubscriptionFeatures03AllC18RequestCachePolicyV
+ _symbolic ______p 25CloudSubscriptionFeatures21GatewayRequestBuilderP
+ _symbolic ______p_____Sgc 25CloudSubscriptionFeatures14RequestSigningP 10Foundation4DateV
+ _symbolic _____ySS__________G 25CloudSubscriptionFeatures25FeatureRequestCoordinatorV AA03AllcE11CachePolicyV AA0gcE7ServiceV
+ _type_layout_string 25CloudSubscriptionFeatures03AllC18RequestCachePolicyV
- ___swift_exist.box.addr_destructor.9Tm
- ___swift_memcpy240_8
- _associated conformance 25CloudSubscriptionFeatures03AllC12RequestCacheVAA07FeatureeF0AA6ResultAaDP_s23CustomStringConvertible
- _objc_msgSend$_urlStringForKey:
- _symbolic $s25CloudSubscriptionFeatures18URLRequestProviderP
- _symbolic $s25CloudSubscriptionFeatures19FeatureRequestCacheP
- _symbolic 10Identifier_____Qy_ 25CloudSubscriptionFeatures19FeatureRequestCacheP
- _symbolic 6Result_____Qy_ 25CloudSubscriptionFeatures19FeatureRequestCacheP
- _symbolic 6Result_____Qz 25CloudSubscriptionFeatures19FeatureRequestCacheP
- _symbolic _____ 25CloudSubscriptionFeatures03AllC12RequestCacheV
- _symbolic ______p 25CloudSubscriptionFeatures18URLRequestProviderP
- _symbolic _____ySS__________G 25CloudSubscriptionFeatures25FeatureRequestCoordinatorV AA03AllcE5CacheV AA0gcE7ServiceV
- _type_layout_string 25CloudSubscriptionFeatures03AllC12RequestCacheV
CStrings:
+ "[%{public}s] No valid cache value and returning nil for %{public}s"
```
