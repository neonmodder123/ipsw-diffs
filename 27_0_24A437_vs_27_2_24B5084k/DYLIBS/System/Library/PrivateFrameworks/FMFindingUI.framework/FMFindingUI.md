## FMFindingUI

> `/System/Library/PrivateFrameworks/FMFindingUI.framework/FMFindingUI`

```diff

-104.30.6.14.23
-  __TEXT.__text: 0x185d60
+104.31.6.16.8
+  __TEXT.__text: 0x18cd58
   __TEXT.__objc_methlist: 0x25a4
-  __TEXT.__const: 0xdb04
-  __TEXT.__cstring: 0x5465
-  __TEXT.__constg_swiftt: 0x8288
-  __TEXT.__swift5_typeref: 0xa7fe
+  __TEXT.__const: 0xdeb4
+  __TEXT.__cstring: 0x54c5
+  __TEXT.__constg_swiftt: 0x8484
+  __TEXT.__swift5_typeref: 0xb272
   __TEXT.__swift5_builtin: 0x280
-  __TEXT.__swift5_reflstr: 0x6de2
-  __TEXT.__swift5_fieldmd: 0x5368
-  __TEXT.__swift5_assocty: 0x938
-  __TEXT.__oslogstring: 0x6cd0
-  __TEXT.__swift5_proto: 0x624
-  __TEXT.__swift5_types: 0x3dc
-  __TEXT.__swift5_capture: 0x331c
-  __TEXT.__swift5_protos: 0x7c
-  __TEXT.__swift_as_entry: 0x44
-  __TEXT.__swift_as_ret: 0x28
-  __TEXT.__swift_as_cont: 0xb0
+  __TEXT.__swift5_reflstr: 0x6ea2
+  __TEXT.__swift5_fieldmd: 0x5474
+  __TEXT.__swift5_assocty: 0x950
+  __TEXT.__oslogstring: 0x6dc0
+  __TEXT.__swift5_proto: 0x630
+  __TEXT.__swift5_types: 0x3e8
+  __TEXT.__swift5_capture: 0x35f0
+  __TEXT.__swift5_protos: 0x80
+  __TEXT.__swift_as_entry: 0x50
+  __TEXT.__swift_as_ret: 0x34
+  __TEXT.__swift_as_cont: 0xd0
   __TEXT.__swift5_mpenum: 0x30
-  __TEXT.__unwind_info: 0x3bc0
-  __TEXT.__eh_frame: 0x244c
+  __TEXT.__unwind_info: 0x3d10
+  __TEXT.__eh_frame: 0x267c
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x250
-  __DATA_CONST.__objc_classlist: 0x2b0
+  __DATA_CONST.__const: 0x258
+  __DATA_CONST.__objc_classlist: 0x2c0
   __DATA_CONST.__objc_catlist: 0x8
   __DATA_CONST.__objc_protolist: 0x180
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x1dc0
+  __DATA_CONST.__objc_selrefs: 0x1de8
   __DATA_CONST.__objc_protorefs: 0xc0
   __DATA_CONST.__vfx_script_tbx: 0x18
-  __DATA_CONST.__got: 0xae0
-  __AUTH_CONST.__const: 0xe789
-  __AUTH_CONST.__objc_const: 0x11690
-  __AUTH_CONST.__auth_got: 0x16e0
-  __AUTH.__objc_data: 0x8200
-  __AUTH.__data: 0x3220
-  __DATA.__data: 0x49d0
+  __DATA_CONST.__got: 0xb20
+  __AUTH_CONST.__const: 0xede9
+  __AUTH_CONST.__objc_const: 0x119c0
+  __AUTH_CONST.__auth_got: 0x1740
+  __AUTH.__objc_data: 0x82d8
+  __AUTH.__data: 0x33c0
+  __DATA.__data: 0x4b10
   __DATA.__objc_stublist: 0x8
-  __DATA.__bss: 0xb780
+  __DATA.__bss: 0xb830
   __DATA.__common: 0xa08
-  __DATA_DIRTY.__objc_data: 0xac8
+  __DATA_DIRTY.__objc_data: 0xad0
   __DATA_DIRTY.__data: 0x168
   __DATA_DIRTY.__common: 0x38
   - /System/Library/Frameworks/ARKit.framework/ARKit

   - /System/Library/Frameworks/SwiftUI.framework/SwiftUI
   - /System/Library/Frameworks/UIKit.framework/UIKit
   - /System/Library/Frameworks/VideoToolbox.framework/VideoToolbox
+  - /System/Library/PrivateFrameworks/ActivityUIServices.framework/ActivityUIServices
   - /System/Library/PrivateFrameworks/CoreAnalytics.framework/CoreAnalytics
   - /System/Library/PrivateFrameworks/FMCore.framework/FMCore
   - /System/Library/PrivateFrameworks/FindMyBase.framework/FindMyBase
   - /System/Library/PrivateFrameworks/FindMyFeatureFlags.framework/FindMyFeatureFlags
+  - /System/Library/PrivateFrameworks/FrontBoardServices.framework/FrontBoardServices
   - /System/Library/PrivateFrameworks/Proximity.framework/Proximity
   - /System/Library/PrivateFrameworks/ProximityUI.framework/ProximityUI
   - /System/Library/PrivateFrameworks/SPOwner.framework/SPOwner

   - /usr/lib/swift/libswiftCoreMIDI.dylib
   - /usr/lib/swift/libswiftDispatch.dylib
   - /usr/lib/swift/libswiftGLKit.dylib
+  - /usr/lib/swift/libswiftIntents.dylib
   - /usr/lib/swift/libswiftMLCompute.dylib
   - /usr/lib/swift/libswiftMetal.dylib
   - /usr/lib/swift/libswiftMetalKit.dylib

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 6966
-  Symbols:   502
-  CStrings:  988
+  Functions: 7106
+  Symbols:   504
+  CStrings:  993
 
Symbols:
+ _OBJC_CLASS_$_FBSOrientationObserver
+ __swift_FORCE_LOAD_$_swiftIntents
+ _swift_dynamicCastObjCProtocolUnconditional
- _swift_dynamicCastObjCProtocolConditional
CStrings:
+ "FMFindingLocalizedString ORIENTATION_ROTATE_TO_PORTRAIT"
+ "ORIENTATION_ROTATE_TO_PORTRAIT"
+ "Precision VFX - failed to recreate VFXManager after mode change"
+ "Precision VFX - recreating VFX world for mode change"
+ "🧭 FMR1NIContxt%@: delaying niSession start by %fs: throttled"
+ "🧭 FMR1NIContxt%@: delaying niSession start with low preferred update rate by %fs: throttled"
- "Precision VFX - Prepare to remove"
```
