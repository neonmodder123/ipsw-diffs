## PriMLCore

> `/System/Library/PrivateFrameworks/PriMLCore.framework/PriMLCore`

```diff

-42.0.0.0.0
-  __TEXT.__text: 0x1efdc
-  __TEXT.__const: 0xba4
-  __TEXT.__swift5_typeref: 0x4a4
-  __TEXT.__swift5_fieldmd: 0x284
-  __TEXT.__constg_swiftt: 0x524
+44.0.0.0.0
+  __TEXT.__text: 0x1fdf8
+  __TEXT.__const: 0xbd4
+  __TEXT.__swift5_typeref: 0x4c0
+  __TEXT.__swift5_fieldmd: 0x2a0
+  __TEXT.__constg_swiftt: 0x580
   __TEXT.__swift5_builtin: 0x28
-  __TEXT.__swift5_reflstr: 0xee
+  __TEXT.__swift5_reflstr: 0xfb
   __TEXT.__swift5_assocty: 0x48
   __TEXT.__swift5_protos: 0x10
   __TEXT.__swift5_proto: 0x60
-  __TEXT.__swift5_types: 0x4c
+  __TEXT.__swift5_types: 0x50
   __TEXT.__cstring: 0xa6
   __TEXT.__swift5_capture: 0x17c
   __TEXT.__swift_as_entry: 0x80
   __TEXT.__swift_as_ret: 0x84
   __TEXT.__swift_as_cont: 0x114
   __TEXT.__oslogstring: 0x4b
-  __TEXT.__unwind_info: 0x950
+  __TEXT.__unwind_info: 0x968
   __TEXT.__eh_frame: 0x17c0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0xd8
+  __DATA_CONST.__const: 0x100
   __DATA_CONST.__objc_classlist: 0x18
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_selrefs: 0xb0
   __DATA_CONST.__got: 0x0
-  __AUTH_CONST.__const: 0xcb8
-  __AUTH_CONST.__objc_const: 0x348
-  __AUTH_CONST.__auth_got: 0xa10
+  __AUTH_CONST.__const: 0xd70
+  __AUTH_CONST.__objc_const: 0x370
+  __AUTH_CONST.__auth_got: 0xa28
   __AUTH.__data: 0x148
-  __DATA.__data: 0x418
+  __DATA.__data: 0x498
   __DATA.__bss: 0xa80
   __DATA_DIRTY.__data: 0x168
   - /System/Library/Frameworks/Accelerate.framework/Accelerate

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 654
-  Symbols:   370
+  Functions: 670
+  Symbols:   374
   CStrings:  8
 
Symbols:
+ __IVARS__TtC9PriMLCore12SingleResume
+ _symbolic _____ 9PriMLCore12SingleResumeC
+ _symbolic _____yScCyxq_GSgG 9PriMLCore12UnfairLockedC
+ _symbolic q_
```
