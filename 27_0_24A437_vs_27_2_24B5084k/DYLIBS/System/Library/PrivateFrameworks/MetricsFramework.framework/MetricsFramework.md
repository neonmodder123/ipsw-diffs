## MetricsFramework

> `/System/Library/PrivateFrameworks/MetricsFramework.framework/MetricsFramework`

```diff

-3600.49.21.11.1
-  __TEXT.__text: 0x10cc00
+3605.15.1.0.0
+  __TEXT.__text: 0x110084
   __TEXT.__objc_methlist: 0x14
-  __TEXT.__const: 0xd120
-  __TEXT.__swift5_typeref: 0x2ef8
-  __TEXT.__swift5_capture: 0x52c
-  __TEXT.__cstring: 0x8130
-  __TEXT.__oslogstring: 0x6c2c
-  __TEXT.__swift5_reflstr: 0x5868
-  __TEXT.__swift5_assocty: 0xdc0
-  __TEXT.__constg_swiftt: 0x5438
-  __TEXT.__swift5_fieldmd: 0x4e10
-  __TEXT.__swift5_builtin: 0x244
-  __TEXT.__swift5_proto: 0x800
-  __TEXT.__swift5_types: 0x47c
-  __TEXT.__swift_as_entry: 0x444
-  __TEXT.__swift_as_ret: 0x3e8
-  __TEXT.__swift_as_cont: 0x674
+  __TEXT.__const: 0xd240
+  __TEXT.__swift5_typeref: 0x2e8a
+  __TEXT.__swift5_capture: 0x53c
+  __TEXT.__cstring: 0x8750
+  __TEXT.__oslogstring: 0x6bac
+  __TEXT.__swift5_reflstr: 0x5c08
+  __TEXT.__swift5_assocty: 0xe70
+  __TEXT.__constg_swiftt: 0x5640
+  __TEXT.__swift5_fieldmd: 0x4f68
+  __TEXT.__swift5_builtin: 0x26c
+  __TEXT.__swift5_proto: 0x7e4
+  __TEXT.__swift5_types: 0x498
+  __TEXT.__swift_as_entry: 0x470
+  __TEXT.__swift_as_ret: 0x404
+  __TEXT.__swift_as_cont: 0x68c
   __TEXT.__swift5_protos: 0x74
-  __TEXT.__unwind_info: 0x3ad0
-  __TEXT.__eh_frame: 0x76d8
+  __TEXT.__unwind_info: 0x3af0
+  __TEXT.__eh_frame: 0x75e0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x2768
-  __DATA_CONST.__objc_classlist: 0x350
+  __DATA_CONST.__const: 0x2770
+  __DATA_CONST.__objc_classlist: 0x378
   __DATA_CONST.__objc_catlist: 0x8
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0xbb8
+  __DATA_CONST.__objc_selrefs: 0xc08
   __DATA_CONST.__got: 0x0
-  __AUTH_CONST.__const: 0x8650
-  __AUTH_CONST.__cfstring: 0x6440
-  __AUTH_CONST.__objc_const: 0x57a8
-  __AUTH_CONST.__auth_got: 0xfe8
-  __AUTH.__objc_data: 0xea8
-  __AUTH.__data: 0x4880
-  __DATA.__data: 0x20f0
+  __AUTH_CONST.__const: 0x8730
+  __AUTH_CONST.__cfstring: 0x6900
+  __AUTH_CONST.__objc_const: 0x5a78
+  __AUTH_CONST.__auth_got: 0xed8
+  __AUTH.__objc_data: 0xf98
+  __AUTH.__data: 0x4d30
+  __DATA.__data: 0x2098
   __DATA.__common: 0x1b0
-  __DATA.__bss: 0xd000
+  __DATA.__bss: 0xcb80
   __DATA_DIRTY.__objc_data: 0x900
   __DATA_DIRTY.__data: 0x33a0
   __DATA_DIRTY.__bss: 0x2080

   - /usr/lib/libobjc.A.dylib
   - /usr/lib/swift/libswiftAVFoundation.dylib
   - /usr/lib/swift/libswiftAccelerate.dylib
-  - /usr/lib/swift/libswiftAppleArchive.dylib
   - /usr/lib/swift/libswiftCompression.dylib
   - /usr/lib/swift/libswiftCore.dylib
   - /usr/lib/swift/libswiftCoreAudio.dylib

   - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 4889
-  Symbols:   2161
-  CStrings:  1700
+  Functions: 4915
+  Symbols:   2178
+  CStrings:  1743
 
Symbols:
+ _OBJC_CLASS_$_ODDSiriSchemaODDAssistantSiriExtensionsCounts
+ _OBJC_CLASS_$_ODDSiriSchemaODDAssistantSiriExtensionsDigest
+ _OBJC_CLASS_$_ODDSiriSchemaODDAssistantSiriExtensionsDigestsReported
+ _OBJC_CLASS_$_ODDSiriSchemaODDAssistantSiriExtensionsDimensions
+ _OBJC_CLASS_$_ODDSiriSchemaODDLLMSiriTokenCounts
+ __DATA__TtC16MetricsFramework26SiriExtensionsDigestPlugin
+ __DATA__TtC16MetricsFramework28SiriExtensionsDigestReporter
+ __DATA__TtC16MetricsFramework30SiriExtensionsDigestCalculator
+ __DATA__TtC16MetricsFramework32SiriExtensionsDigestDataProvider
+ __DATA__TtC16MetricsFramework32SiriExtensionsDigestSELFReporter
+ __DATA__TtC16MetricsFramework32SiriExtensionsDigestTaskExecutor
+ __IVARS__TtC16MetricsFramework26SiriExtensionsDigestPlugin
+ __IVARS__TtC16MetricsFramework30SiriExtensionsDigestCalculator
+ __IVARS__TtC16MetricsFramework32SiriExtensionsDigestDataProvider
+ __IVARS__TtC16MetricsFramework32SiriExtensionsDigestSELFReporter
+ __METACLASS_DATA__TtC16MetricsFramework26SiriExtensionsDigestPlugin
+ __METACLASS_DATA__TtC16MetricsFramework28SiriExtensionsDigestReporter
+ __METACLASS_DATA__TtC16MetricsFramework30SiriExtensionsDigestCalculator
+ __METACLASS_DATA__TtC16MetricsFramework32SiriExtensionsDigestDataProvider
+ __METACLASS_DATA__TtC16MetricsFramework32SiriExtensionsDigestSELFReporter
+ __METACLASS_DATA__TtC16MetricsFramework32SiriExtensionsDigestTaskExecutor
+ _associated conformance 16MetricsFramework26SiriExtensionsDigestPluginC18lighthouse_runtime010LighthouseF0AaD12DataProvider
+ _associated conformance 16MetricsFramework26SiriExtensionsDigestPluginC18lighthouse_runtime010LighthouseF0AaD6Worker
+ _associated conformance 16MetricsFramework26SiriExtensionsDigestPluginC18lighthouse_runtime010LighthouseF0AaD8Reporter
+ _associated conformance 16MetricsFramework26SiriExtensionsDigestPluginC18lighthouse_runtime12DataProviderAA0I10RecordTypeAdEP_AD0iK0
+ _associated conformance 16MetricsFramework26SiriExtensionsDigestPluginC18lighthouse_runtime6WorkerAA10ResultTypeAdEP_AD0J0
+ _associated conformance 16MetricsFramework26SiriExtensionsDigestPluginC18lighthouse_runtime6WorkerAA14DataRecordTypeAdEP_AD0jK0
+ _associated conformance 16MetricsFramework26SiriExtensionsDigestPluginC18lighthouse_runtime8ReporterAA10ResultTypeAdEP_AD0J0
+ _associated conformance 16MetricsFramework27SiriExtensionsDigestResultsV11DeepThought16BookmarkedResultAA18lighthouse_runtime0J0
+ _associated conformance 16MetricsFramework28SiriExtensionsDigestReporterC18lighthouse_runtime0F0AA10ResultTypeAdEP_AD0I0
+ _associated conformance 16MetricsFramework32SiriExtensionsDigestDataProviderC18lighthouse_runtime0fG0AA0F10RecordTypeAdEP_AD0fJ0
+ _associated conformance 16MetricsFramework32SiriExtensionsDigestTaskExecutorC7SubtaskOSHAASQ
+ _associated conformance 16MetricsFramework32SiriExtensionsDigestTaskExecutorCAA011ODDIMetricsfG4BaseAA7SubtaskAaDP_8RawValueSYSy
+ _associated conformance 16MetricsFramework32SiriExtensionsDigestTaskExecutorCAA011ODDIMetricsfG4BaseAA7SubtaskAaDP_SY
+ _objc_msgSend$addAppPartyTypes:
+ _objc_msgSend$addProviderName:
+ _objc_msgSend$setAssistantSiriExtensionsDigestsReported:
+ _objc_msgSend$setEventStreamResultType:
+ _objc_msgSend$setHasSiriExtensionsEnabled:
+ _objc_msgSend$setInputTokenBucketOdm:
+ _objc_msgSend$setInputTokenBucketPcc:
+ _objc_msgSend$setOutputTokenBucketOdm:
+ _objc_msgSend$setOutputTokenBucketPcc:
+ _objc_msgSend$setPromptType:
+ _objc_msgSend$setTokenBucketScheme:
+ _objc_msgSend$setTokenCounts:
+ _objc_msgSend$setTotalTokenBucketOdm:
+ _objc_msgSend$setTotalTokenBucketPcc:
+ _symbolic SDy_____Say_____GG 16MetricsFramework13EventMetadataV AA020SiriExtensionsDigestA0V
+ _symbolic Say_____G 16MetricsFramework020SiriExtensionsDigestA0V
+ _symbolic Say_____G So28ODDSiriSchemaODDAppPartyTypeV
+ _symbolic Say_____GSg So28ODDSiriSchemaODDAppPartyTypeV
+ _symbolic _____ 16MetricsFramework020SiriExtensionsDigestA0V
+ _symbolic _____ 16MetricsFramework26SiriExtensionsDigestPluginC
+ _symbolic _____ 16MetricsFramework27SiriExtensionsDigestResultsV
+ _symbolic _____ 16MetricsFramework28SiriExtensionsDigestReporterC
+ _symbolic _____ 16MetricsFramework29SiriExtensionsDigestConcluderV
+ _symbolic _____ 16MetricsFramework30SiriExtensionsDigestCalculatorC
+ _symbolic _____ 16MetricsFramework30SiriExtensionsDigestDataRecordV
+ _symbolic _____ 16MetricsFramework32SiriExtensionsDigestDataProviderC
+ _symbolic _____ 16MetricsFramework32SiriExtensionsDigestSELFReporterC
+ _symbolic _____ 16MetricsFramework32SiriExtensionsDigestTaskExecutorC
+ _symbolic _____ 16MetricsFramework32SiriExtensionsDigestTaskExecutorC7SubtaskO
+ _symbolic _____ 16MetricsFramework39SiriExtensionsDigestWorkerConfigurationV
+ _symbolic _____ So013ODDSiriSchemaA20ExtensionRequestTypeV
+ _symbolic _____ So28ODDSiriSchemaODDAppPartyTypeV
+ _symbolic _____3key_Say_____G5valuet 16MetricsFramework13EventMetadataV AA020SiriExtensionsDigestA0V
+ _symbolic _____Sg 12FindMyLocate12ClientTargetV
+ _symbolic _____Sg 16MetricsFramework020SiriExtensionsDigestA0V
+ _symbolic _____Sg So013ODDSiriSchemaA20ExtensionRequestTypeV
+ _symbolic _____y_____G 11DeepThought13BatchReporterC 16MetricsFramework020SiriExtensionsDigestD0C
+ _symbolic _____y_____G 18lighthouse_runtime26LighthouseRuntimeProcessorC 16MetricsFramework26SiriExtensionsDigestPluginC
+ _symbolic _____y_____G s23_ContiguousArrayStorageC 16MetricsFramework020SiriExtensionsDigestD0V
+ _symbolic _____y_____G s23_ContiguousArrayStorageC So28ODDSiriSchemaODDAppPartyTypeV
+ _symbolic _____y_____Say_____GG s18_DictionaryStorageC 16MetricsFramework13EventMetadataV AC020SiriExtensionsDigestC0V
+ _symbolic _____y_____y_____GG 11DeepThought18BookmarkedReporterC AA05BatchD0C 16MetricsFramework020SiriExtensionsDigestD0C
+ _type_layout_string 16MetricsFramework27SiriExtensionsDigestResultsV
- _OBJC_CLASS_$_BMMetadata
- _OBJC_CLASS_$_BMScoreItem
- _OBJC_CLASS_$_BMSiriThirdPartyGenAiEngagementInference
- __DATA__TtC16MetricsFramework10GenAiQuery
- __IVARS__TtC16MetricsFramework10GenAiQuery
- __METACLASS_DATA__TtC16MetricsFramework10GenAiQuery
- ___swift_project_boxed_opaque_existential_1Tm
- __swift_FORCE_LOAD_$_swiftAppleArchive
- __swift_FORCE_LOAD_$_swiftAppleArchive_$_MetricsFramework
- __swift_stdlib_strtod_clocale
- _associated conformance 16MetricsFramework30ThirdPartyGenAiInferenceResultV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLOSHAASQ
- _associated conformance 16MetricsFramework30ThirdPartyGenAiInferenceResultV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLOs0I3KeyAAs23CustomStringConvertible
- _associated conformance 16MetricsFramework30ThirdPartyGenAiInferenceResultV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLOs0I3KeyAAs28CustomDebugStringConvertible
- _associated conformance 16MetricsFramework8MetadataV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLOSHAASQ
- _associated conformance 16MetricsFramework8MetadataV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLOs0D3KeyAAs23CustomStringConvertible
- _associated conformance 16MetricsFramework8MetadataV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLOs0D3KeyAAs28CustomDebugStringConvertible
- _associated conformance 16MetricsFramework9ScoreItemV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLOSHAASQ
- _associated conformance 16MetricsFramework9ScoreItemV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLOs0E3KeyAAs23CustomStringConvertible
- _associated conformance 16MetricsFramework9ScoreItemV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLOs0E3KeyAAs28CustomDebugStringConvertible
- _objc_msgSend$ThirdPartyGenAiEngagementInference
- _objc_msgSend$initWithEventTime:turnId:version:usecase:scores:metadata:
- _objc_msgSend$initWithExplanation:isFollowup:
- _objc_msgSend$initWithType:value:
- _swift_getDynamicType
- _symbolic SDySS_____G 12PoirotBlocks15AnyDatasetValueV
- _symbolic SDySSypG
- _symbolic SS______t 12PoirotBlocks15AnyDatasetValueV
- _symbolic SaySaySSGG
- _symbolic Say_____G 10Foundation11JSONEncoderC16OutputFormattingV
- _symbolic Say_____G 16MetricsFramework9ScoreItemV
- _symbolic Sd
- _symbolic _____ 13ODDIFramework16ODBatchInferenceC
- _symbolic _____ 16MetricsFramework10GenAiQueryC
- _symbolic _____ 16MetricsFramework30ThirdPartyGenAiInferenceResultV
- _symbolic _____ 16MetricsFramework30ThirdPartyGenAiInferenceResultV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLO
- _symbolic _____ 16MetricsFramework8MetadataV
- _symbolic _____ 16MetricsFramework8MetadataV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLO
- _symbolic _____ 16MetricsFramework9ScoreItemV
- _symbolic _____ 16MetricsFramework9ScoreItemV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLO
- _symbolic _____Sg 16MetricsFramework30ThirdPartyGenAiInferenceResultV
- _symbolic _____ySDySS_____GG s23_ContiguousArrayStorageC 12PoirotBlocks15AnyDatasetValueV
- _symbolic _____ySS_____G s18_DictionaryStorageC 12PoirotBlocks15AnyDatasetValueV
- _symbolic _____ySS______tG s23_ContiguousArrayStorageC 12PoirotBlocks15AnyDatasetValueV
- _symbolic _____ySaySSGG s23_ContiguousArrayStorageC
- _symbolic _____ySaySaySSGGG s23_ContiguousArrayStorageC
- _symbolic _____ySo40BMSiriThirdPartyGenAiEngagementInferenceCG 11DeepThought12BiomeDonatorC
- _symbolic _____ySsG s10ArraySliceV
- _symbolic _____y_____G s18EnumeratedSequenceV 12PoirotBlocks12DatasetTableV
- _symbolic _____y_____G s22KeyedDecodingContainerV 16MetricsFramework30ThirdPartyGenAiInferenceResultV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLO
- _symbolic _____y_____G s22KeyedDecodingContainerV 16MetricsFramework8MetadataV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLO
- _symbolic _____y_____G s22KeyedDecodingContainerV 16MetricsFramework9ScoreItemV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLO
- _symbolic _____y_____G s22KeyedEncodingContainerV 16MetricsFramework30ThirdPartyGenAiInferenceResultV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLO
- _symbolic _____y_____G s22KeyedEncodingContainerV 16MetricsFramework8MetadataV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLO
- _symbolic _____y_____G s22KeyedEncodingContainerV 16MetricsFramework9ScoreItemV10CodingKeys33_D0F7A9FBB69195146951456F804E8A73LLO
- _symbolic _____y_____G s23_ContiguousArrayStorageC 10Foundation11JSONEncoderC16OutputFormattingV
- _symbolic _____y_____G s23_ContiguousArrayStorageC 16MetricsFramework30ThirdPartyGenAiInferenceResultV
- _symbolic _____y_____G s23_ContiguousArrayStorageC 16MetricsFramework9ScoreItemV
- _symbolic _____y______G s18EnumeratedSequenceV8IteratorV 12PoirotBlocks12DatasetTableV
- _type_layout_string 16MetricsFramework8MetadataV
- _type_layout_string 16MetricsFramework9ScoreItemV
CStrings:
+ "#ExtensionsUtils: Failed to decode JSON string array: %@"
+ "#ExtensionsUtils: jsonString is nil or its not utf8 encodable."
+ "#PromptType: %s"
+ "#SiriExtensionsDigestCalculator: %ld results"
+ "#SiriExtensionsDigestCalculator: %s (%ld occurrences)"
+ "#SiriExtensionsDigestCalculator: invoked"
+ "#SiriExtensionsDigestConcluder: SQL file could not be loaded"
+ "#SiriExtensionsDigestConcluder: SQL: %s"
+ "#SiriExtensionsDigestSELFReporter: Events Count: %ld"
+ "#SiriExtensionsDigestSELFReporter: Finished Emitting SiriExtensionsDigest SELF events"
+ "#SiriExtensionsDigestSELFReporter: Reporter invoked"
+ "#SiriExtensionsDigestSELFReporter: Results Data %s"
+ "#SiriExtensionsDigestSELFReporter: Total Results Count %ld"
+ "#SiriExtensionsDigestSELFReporter: failed to create client event"
+ "#SiriExtensionsDigestSELFReporter: failed to create digests reported"
+ "#SiriExtensionsDigestSELFReporter: failed to report event: %@"
+ "#SiriExtensionsDigestTaskExecutor: Call SiriExtensionsDigest"
+ "#StreamingResult: %s"
+ "#startOfToday : %s"
+ "AppPartyTypes: %s"
+ "Bookmark Value: %s -  %@"
+ "GAACONFIRMATIONTYPE_"
+ "GAACONFIRMATIONTYPE_EXECUTE_SUGGESTION"
+ "GAAPROVIDERSTREAMINGRESULT_"
+ "INVOCATIONSOURCE_CARPLAY_BANNER_PREPROCESS_SIRIAI"
+ "IncludeCurrentDate: %{bool}d"
+ "LOCALE_ACW_SA"
+ "LOCALE_AFB_AE"
+ "LOCALE_AJP_JO"
+ "LOCALE_AJP_PS"
+ "LOCALE_APC_LB"
+ "LOCALE_APC_SY"
+ "LOCALE_ARS_SA"
+ "LOCALE_ARZ_EG"
+ "LOCALE_AZ_AZ"
+ "LOCALE_BE_BY"
+ "LOCALE_BG_BG"
+ "LOCALE_BN_IN"
+ "LOCALE_ET_EE"
+ "LOCALE_GU_IN"
+ "LOCALE_HI_LATN"
+ "LOCALE_IS_IS"
+ "LOCALE_KN_IN"
+ "LOCALE_ML_IN"
+ "LOCALE_MR_IN"
+ "LOCALE_PA_IN"
+ "LOCALE_SL_SI"
+ "LOCALE_SR_RS"
+ "LOCALE_TA_IN"
+ "LOCALE_TE_IN"
+ "LOCALE_UR_IN"
+ "LOCALE_UZ_UZ"
+ "MetricsFramework/SiriExtensionsDigestReporter.swift"
+ "ODDAPPPARTYTYPE_FIRST_PARTY"
+ "ODDAPPPARTYTYPE_SECOND_PARTY"
+ "ODDAPPPARTYTYPE_THIRD_PARTY"
+ "ODDAPPPARTYTYPE_UNKNOWN"
+ "ODDSIRIEXTENSIONEVENTSTREAMRESULTTYPE_"
+ "ODDSIRIEXTENSIONEVENTSTREAMRESULTTYPE_HANDLED_REQUEST"
+ "ODDSIRIEXTENSIONEVENTSTREAMRESULTTYPE_NEEDS_FOLLOWUP"
+ "ODDSIRIEXTENSIONEVENTSTREAMRESULTTYPE_UNABLE_TO_HANDLE_REQUEST"
+ "ODDSIRIEXTENSIONEVENTSTREAMRESULTTYPE_UNKNOWN"
+ "ODDSIRIEXTENSIONPROMPTTYPE_"
+ "ODDSIRIEXTENSIONPROMPTTYPE_CONFIRMATION_EXECUTE_SUGGESTION"
+ "ODDSIRIEXTENSIONPROMPTTYPE_PROVIDER_ENABLEMENT"
+ "ODDSIRIEXTENSIONPROMPTTYPE_UNKNOWN"
+ "SiriExtensionsDigest"
+ "SiriExtensionsDigestCalculator: "
+ "SiriExtensionsDigestExtension"
+ "SiriExtensionsDigestExtensionIncludeCurrentDate"
+ "SiriExtensionsDigestWorker"
+ "com.apple.siri.SiriExtensionsDigest.worker"
+ "com.apple.siri.SiriExtensionsDigestExtension"
+ "endTime: %s"
+ "input_token_bucket_odm"
+ "input_token_bucket_pcc"
+ "output_token_bucket_odm"
+ "output_token_bucket_pcc"
+ "siriExtensionsDigest"
+ "third_party_streaming_result"
+ "third_party_user_prompt_type"
+ "total_token_bucket_odm"
+ "total_token_bucket_pcc"
- "Could not process data for : %s"
- "Could not process scores."
- "Derived turnIndex: %s"
- "Error process scores datasets."
- "Error processing with ODInference: %@"
- "EvaluationPrompts"
- "Extracted all the utterance columns: %s"
- "Extracting all the rows: %ld"
- "Failed to extract metadata from result: %s"
- "Failed to extract metadata. Values are not compatible."
- "Failed to extract scores from result: %s"
- "Failed to load prompt: %@"
- "FetchUnifiedSiriTurnBlock"
- "Getting all the utterances..."
- "Input received: %s"
- "Mapped index %ld to actual turnId: %s"
- "Mapped results: %s"
- "Model availability: %{bool}d"
- "Model is not avaialble on the Device. Skipping..."
- "No mapping found for index: %ld"
- "No turnId found in result: %s"
- "No turnId mapping found for index: %ld"
- "Nothing to process."
- "Processed turnId: %s"
- "Processing %ld entries"
- "Processing and extracting scores"
- "Processing individual result: %s"
- "Processing turnId: %s, %s"
- "Prompt Instruction : %s"
- "Prompt loaded: %s"
- "Prompt outputColumns : %s"
- "Prompt usecase : %s"
- "Prompt version : %s"
- "Result from the inference call: %s"
- "Total rows in the table: %ld"
- "Truncating long utterance to fit in the context window to a max of %ld tokens"
- "Will write %ld to Biome"
- "Writing to Biome..."
- "userInteractionBooleanClassification"
- "🤖 Processing utterances with ODInference..."
```
