## HeadphoneCommonUIKit

> `/System/Library/PrivateFrameworks/HeadphoneCommonUIKit.framework/HeadphoneCommonUIKit`

```diff

-40.41.1.1.10
-  __TEXT.__text: 0xbea0c
+41.4.0.0.0
+  __TEXT.__text: 0xbeb08
   __TEXT.__objc_methlist: 0x1334
   __TEXT.__const: 0x4754
   __TEXT.__gcc_except_tab: 0x104
-  __TEXT.__cstring: 0x3138
+  __TEXT.__cstring: 0x31f8
   __TEXT.__oslogstring: 0x11e7
   __TEXT.__ustring: 0x1a6
-  __TEXT.__swift5_typeref: 0x3e90
+  __TEXT.__swift5_typeref: 0x3e94
   __TEXT.__swift5_reflstr: 0xadd
   __TEXT.__swift5_assocty: 0x5f0
   __TEXT.__constg_swiftt: 0x2270

   __TEXT.__swift5_builtin: 0x1b8
   __TEXT.__swift5_proto: 0x17c
   __TEXT.__swift5_types: 0x1b4
-  __TEXT.__swift5_capture: 0x25c8
+  __TEXT.__swift5_capture: 0x25d8
   __TEXT.__swift5_protos: 0x28
   __TEXT.__swift_as_entry: 0x54
   __TEXT.__swift_as_ret: 0x40
   __TEXT.__swift_as_cont: 0x78
   __TEXT.__swift5_mpenum: 0x20
-  __TEXT.__unwind_info: 0x1680
+  __TEXT.__unwind_info: 0x1688
   __TEXT.__eh_frame: 0xd68
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0

   __DATA_CONST.__objc_protorefs: 0x40
   __DATA_CONST.__objc_superrefs: 0x40
   __DATA_CONST.__got: 0xa30
-  __AUTH_CONST.__const: 0x74e0
+  __AUTH_CONST.__const: 0x7508
   __AUTH_CONST.__cfstring: 0x1a60
   __AUTH_CONST.__objc_const: 0x3ec0
   __AUTH_CONST.__objc_intobj: 0x30

   - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 3571
-  Symbols:   1982
-  CStrings:  496
+  Functions: 3573
+  Symbols:   1983
+  CStrings:  497
 
Symbols:
+ _symbolic Ig_
CStrings:
+ "/Library/Caches/com.apple.xbs/<UUID>/TemporaryDirectory.<TMP>/Sources/ConnectedAudio_UI/HeadphoneSettings/HeadphoneCommonUIKit/MovieLoopPlayer.swift"
```
