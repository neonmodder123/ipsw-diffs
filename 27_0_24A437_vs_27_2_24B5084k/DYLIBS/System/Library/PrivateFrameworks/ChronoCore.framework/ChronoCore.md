## ChronoCore

> `/System/Library/PrivateFrameworks/ChronoCore.framework/ChronoCore`

```diff

-749.0.2.0.0
-  __TEXT.__text: 0x4243a8
-  __TEXT.__objc_methlist: 0x1f20
-  __TEXT.__const: 0x148d8
-  __TEXT.__cstring: 0x6e5b
-  __TEXT.__oslogstring: 0x15c47
+749.2.4.0.0
+  __TEXT.__text: 0x43716c
+  __TEXT.__objc_methlist: 0x1f28
+  __TEXT.__const: 0x149c8
+  __TEXT.__cstring: 0x6e8b
+  __TEXT.__oslogstring: 0x16347
   __TEXT.__gcc_except_tab: 0x70
   __TEXT.__dlopen_cstrs: 0x7a
-  __TEXT.__constg_swiftt: 0xbc98
-  __TEXT.__swift5_typeref: 0xc393
-  __TEXT.__swift5_reflstr: 0xaa03
-  __TEXT.__swift5_fieldmd: 0x7f58
+  __TEXT.__constg_swiftt: 0xbde4
+  __TEXT.__swift5_typeref: 0xc4b0
+  __TEXT.__swift5_reflstr: 0xabef
+  __TEXT.__swift5_fieldmd: 0x8028
   __TEXT.__swift5_builtin: 0x1b8
   __TEXT.__swift5_assocty: 0x6e8
   __TEXT.__swift5_proto: 0xb74
-  __TEXT.__swift5_types: 0x680
+  __TEXT.__swift5_types: 0x684
   __TEXT.__swift5_protos: 0x230
-  __TEXT.__swift5_capture: 0x556c
-  __TEXT.__swift_as_entry: 0x180
-  __TEXT.__swift_as_ret: 0x16c
-  __TEXT.__swift_as_cont: 0x33c
+  __TEXT.__swift5_capture: 0x5624
+  __TEXT.__swift_as_entry: 0x18c
+  __TEXT.__swift_as_ret: 0x17c
+  __TEXT.__swift_as_cont: 0x368
   __TEXT.__swift5_mpenum: 0x30
-  __TEXT.__unwind_info: 0x7758
-  __TEXT.__eh_frame: 0xc6a8
+  __TEXT.__unwind_info: 0x78c0
+  __TEXT.__eh_frame: 0xcaa0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __TEXT.__objc_methtype: 0x0
   __DATA_CONST.__const: 0x1c0
   __DATA_CONST.__objc_classlist: 0x600
-  __DATA_CONST.__objc_catlist: 0x10
+  __DATA_CONST.__objc_catlist: 0x8
   __DATA_CONST.__objc_protolist: 0x2c8
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x1968
+  __DATA_CONST.__objc_selrefs: 0x1970
   __DATA_CONST.__objc_protorefs: 0x178
   __DATA_CONST.__objc_superrefs: 0x10
-  __DATA_CONST.__got: 0x1df0
-  __AUTH_CONST.__const: 0x13868
+  __DATA_CONST.__got: 0x1e08
+  __AUTH_CONST.__const: 0x13af8
   __AUTH_CONST.__cfstring: 0x60
-  __AUTH_CONST.__objc_const: 0x187c8
-  __AUTH_CONST.__auth_got: 0x4548
-  __AUTH.__objc_data: 0x1138
+  __AUTH_CONST.__objc_const: 0x18960
+  __AUTH_CONST.__auth_got: 0x4578
+  __AUTH.__objc_data: 0x1158
   __AUTH.__data: 0x16d8
   __DATA.__objc_ivar: 0x14
-  __DATA.__data: 0x33c0
-  __DATA.__bss: 0x7a30
+  __DATA.__data: 0x3500
+  __DATA.__bss: 0x7bb0
   __DATA.__common: 0xd0
-  __DATA_DIRTY.__objc_data: 0x3a00
-  __DATA_DIRTY.__data: 0x105b8
-  __DATA_DIRTY.__bss: 0x9c30
+  __DATA_DIRTY.__objc_data: 0x3a08
+  __DATA_DIRTY.__data: 0x105f8
+  __DATA_DIRTY.__bss: 0x9ab0
   __DATA_DIRTY.__common: 0x918
   - /System/Library/Frameworks/AccessoryLiveActivities.framework/AccessoryLiveActivities
   - /System/Library/Frameworks/AccessoryNotifications.framework/AccessoryNotifications

   - /usr/lib/swift/libswift_DarwinFoundation3.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 11048
-  Symbols:   5055
-  CStrings:  1990
+  Functions: 11153
+  Symbols:   5065
+  CStrings:  2012
 
Symbols:
+ __IVARS__TtC10ChronoCore13SmallLRUCache
+ ___swift_closure_destructor.126Tm
+ ___swift_closure_destructor.151Tm
+ ___swift_closure_destructor.169Tm
+ ___swift_closure_destructor.188Tm
+ ___swift_closure_destructor.189Tm
+ ___swift_closure_destructor.211Tm
+ ___swift_closure_destructor.237Tm
+ ___swift_closure_destructor.240Tm
+ ___swift_closure_destructor.38Tm
+ ___swift_closure_destructor.41Tm
+ ___swift_closure_destructor.50Tm
+ _associated conformance 10ChronoCore24MobilePlaceholderServiceC10CacheEventOSHAASQ
+ _symbolic SDyxq_G
+ _symbolic SayxG
+ _symbolic ShySo29CHSWidgetMetricsSpecificationCG
+ _symbolic Shy_____G 10ChronoCore24MobilePlaceholderServiceC10CacheEventO
+ _symbolic _____ 10ChronoCore13SmallLRUCacheC
+ _symbolic _____ 10ChronoCore24MobilePlaceholderServiceC10CacheEventO
+ _symbolic _____ 14ChronoServices9SignpostsV7WidgetsV23PlaceholderLoadSignpostC
+ _symbolic _____ 9ChronoKit30ApplicationAvailabilityMonitorC
+ _symbolic _____3key______5valuet 9ChronoKit14WidgetEntryKeyV AA20EnvironmentModifiersV
+ _symbolic _____Sg 14ChronoServices9SignpostsV7WidgetsV29PlaceholderGenerationSignpostC
+ _symbolic _____Sg_ABt 10Foundation14DateComponentsV
+ _symbolic _____Sg_ABt 9ChronoKit20EnvironmentModifiersV
+ _symbolic ___________t 14ChronoServices16DeviceIdentifierO 0A3Kit19ReloadConfigurationV0F6ReasonO
+ _symbolic ______p 9ChronoKit33ApplicationAvailabilityMonitoringP
+ _symbolic _____ySo29CHSWidgetMetricsSpecificationCG s11_SetStorageC
+ _symbolic _____y_____G s11_SetStorageC 10ChronoCore24MobilePlaceholderServiceC10CacheEventO
+ _symbolic _____y______G 14ChronoServices9SignpostsV24IntervalSignpostProviderV AC7WidgetsV015PlaceholderLoadE0C
+ _symbolic _____y______G 14ChronoServices9SignpostsV24IntervalSignpostProviderV AC7WidgetsV021PlaceholderGenerationE0C
+ _symbolic _____y______Sbt_____G 7Combine18PassthroughSubjectC 10ChronoCore24MobilePlaceholderServiceC10CacheEventO s5NeverO
+ _symbolic _____y______Sbt_____GSg 7Combine12AnyPublisherV 10ChronoCore24MobilePlaceholderServiceC10CacheEventO s5NeverO
+ _symbolic _____y_____y__________G_____GSg 7Combine12AnyPublisherV 9ChronoKit32StatefulStoreReloadVariantsEventV AD11TimelineKeyV AD011WidgetEntryL0V s5NeverO
- -[DASession(CHS) setupForDeferredReconnect]
- _OBJC_CLASS_$_DASession
- __OBJC_$_CATEGORY_DASession_$_CHS
- __OBJC_$_CATEGORY_INSTANCE_METHODS_DASession_$_CHS
- ___swift_closure_destructor.102Tm
- ___swift_closure_destructor.109Tm
- ___swift_closure_destructor.137Tm
- ___swift_closure_destructor.144Tm
- ___swift_closure_destructor.167Tm
- ___swift_closure_destructor.183Tm
- ___swift_closure_destructor.193Tm
- ___swift_closure_destructor.219Tm
- ___swift_closure_destructor.222Tm
- ___swift_closure_destructor.23Tm
- ___swift_closure_destructor.90Tm
- ___swift_closure_destructor.92Tm
- _associated conformance 10ChronoCore24MobilePlaceholderServiceC10CacheEvent33_8629D001E26D3A32393D28729882FCC2LLOSHAASQ
- _objc_msgSend$setupForDeferredReconnect
- _swift_retain_x14
- _symbolic Shy_____G 10ChronoCore24MobilePlaceholderServiceC10CacheEvent33_8629D001E26D3A32393D28729882FCC2LLO
- _symbolic _____ 10ChronoCore24MobilePlaceholderServiceC10CacheEvent33_8629D001E26D3A32393D28729882FCC2LLO
- _symbolic _____ 9ChronoKit25ApplicationRemovalMonitorC
- _symbolic ______p 9ChronoKit28ApplicationRemovalMonitoringP
- _symbolic _____y_____G s11_SetStorageC 10ChronoCore24MobilePlaceholderServiceC10CacheEvent33_8629D001E26D3A32393D28729882FCC2LLO
CStrings:
+ "%{public}s Timeline will reload since its last reload date or the last known significant time change date is > current archive creation date."
+ "Containers awaiting install changed - count = %{public}ld; republishing served extensions"
+ "Device %{public}s changed owning app from %{public}s to %{public}s, invalidating its connection"
+ "Device %{public}s is no longer reported by DeviceAccess, removing it"
+ "Error reconciling devices after app migration: %{public}@"
+ "Failed to add %{public}s to the limited authorizations for accessory %{public}s: %{public}@"
+ "Failed to reconcile DeviceAccess state before replacing %{public}s: %{public}@ - abandoning the migration rather than writing stale state back to DeviceAccess"
+ "Finished replacing %{public}s with %{public}s: updated %{public}s, failed %{public}s"
+ "Generating configured snapshot descriptor with environment modifiers for %{public}s"
+ "Generating configured snapshot descriptor without environment modifiers for %{public}s"
+ "Ignoring app replacement because the source and destination are the same: %{public}s"
+ "Ignoring device reconcile because initial device fetch has not completed"
+ "MacPreviews"
+ "Metrics change has no changes for the precacher"
+ "No accessories affected replacing %{public}s with %{public}s"
+ "Precaching complete: generated %{public}ld, canceled %{public}ld"
+ "Rebuilding remote widget configurations for %{public}s"
+ "Received metrics change"
+ "Reconciled devices after app migration: %{public}ld fetched, %{public}ld added, %{public}ld updated, %{public}ld removed"
+ "Replacing %{public}s with %{public}s across %{public}ld accessory(ies)"
+ "Skipping accessory %{public}s - authorization state %{public}s has no limited allow-list to migrate"
+ "Skipping accessory %{public}s - still registered to %{public}s, so DeviceAccess did not migrate the companion app"
+ "[%{public}s] Denied replaceApplication because the caller lacks the migration entitlement"
+ "reconcileAfterAppMigration()"
- "%{public}s Timeline will reload since it was previously removed from the store and its reload date or the last known significant time change date is > current archive creation date."
- "Received metrics change: %{public}s"
```
