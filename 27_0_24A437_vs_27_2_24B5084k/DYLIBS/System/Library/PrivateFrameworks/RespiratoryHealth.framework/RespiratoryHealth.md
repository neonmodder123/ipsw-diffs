## RespiratoryHealth

> `/System/Library/PrivateFrameworks/RespiratoryHealth.framework/RespiratoryHealth`

```diff

-7027.0.72.2.7
-  __TEXT.__text: 0x4738
-  __TEXT.__objc_methlist: 0x7fc
-  __TEXT.__const: 0x72
-  __TEXT.__cstring: 0xa82
+7027.1.36.2.7
+  __TEXT.__text: 0x6fc8
+  __TEXT.__objc_methlist: 0x834
+  __TEXT.__const: 0x296
+  __TEXT.__cstring: 0xa70
   __TEXT.__oslogstring: 0x40b
-  __TEXT.__unwind_info: 0x1d0
+  __TEXT.__constg_swiftt: 0x2c
+  __TEXT.__swift5_typeref: 0x51
+  __TEXT.__swift5_builtin: 0x14
+  __TEXT.__swift5_reflstr: 0x23
+  __TEXT.__swift5_fieldmd: 0x1c
+  __TEXT.__swift5_assocty: 0x30
+  __TEXT.__swift5_proto: 0x18
+  __TEXT.__swift5_types: 0x4
+  __TEXT.__unwind_info: 0x288
+  __TEXT.__eh_frame: 0x48
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x1d8
+  __DATA_CONST.__const: 0x1e0
   __DATA_CONST.__objc_classlist: 0x40
-  __DATA_CONST.__objc_catlist: 0x18
-  __DATA_CONST.__objc_protolist: 0x40
+  __DATA_CONST.__objc_catlist: 0x20
+  __DATA_CONST.__objc_protolist: 0x50
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_selrefs: 0x680
-  __DATA_CONST.__objc_protorefs: 0x10
+  __DATA_CONST.__objc_protorefs: 0x18
   __DATA_CONST.__objc_superrefs: 0x40
   __DATA_CONST.__objc_arraydata: 0x28
-  __DATA_CONST.__got: 0x1f8
+  __DATA_CONST.__got: 0x228
+  __AUTH_CONST.__const: 0x28
   __AUTH_CONST.__cfstring: 0x8c0
-  __AUTH_CONST.__objc_const: 0x1110
+  __AUTH_CONST.__objc_const: 0x1180
   __AUTH_CONST.__objc_intobj: 0x78
   __AUTH_CONST.__objc_arrayobj: 0x18
-  __AUTH_CONST.__auth_got: 0x0
+  __AUTH_CONST.__auth_got: 0x358
   __AUTH.__objc_data: 0xf0
   __DATA.__objc_ivar: 0x74
-  __DATA.__data: 0x300
-  __DATA.__bss: 0x20
+  __DATA.__data: 0x3f0
+  __DATA.__bss: 0x300
   __DATA_DIRTY.__objc_data: 0x190
   - /System/Library/Frameworks/CoreFoundation.framework/CoreFoundation
   - /System/Library/Frameworks/Foundation.framework/Foundation
   - /System/Library/Frameworks/HealthKit.framework/HealthKit
+  - /System/Library/PrivateFrameworks/HealthFeatures.framework/HealthFeatures
   - /System/Library/PrivateFrameworks/HealthUtilities.framework/HealthUtilities
   - /System/Library/PrivateFrameworks/NanoPreferencesSync.framework/NanoPreferencesSync
   - /System/Library/PrivateFrameworks/NanoRegistry.framework/NanoRegistry

   - /usr/lib/swift/libswiftXPC.dylib
   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswiftos.dylib
-  Functions: 161
-  Symbols:   572
+  Functions: 219
+  Symbols:   617
   CStrings:  93
 
Symbols:
+ _HKFeatureAvailabilityContextOxygenSaturationStoreDemo
+ _MobileGestalt_get_current_device
+ _MobileGestalt_get_deviceSupportsScandium
+ _MobileGestalt_get_deviceSupportsUSLegallyCompliantBloodOxygen
+ __CATEGORY_CLASS_METHODS_HKFeatureAvailabilityRequirementSet_$_RespiratoryHealth
+ __CATEGORY_CLASS_PROPERTIES_HKFeatureAvailabilityRequirementSet_$_RespiratoryHealth
+ __CATEGORY_HKFeatureAvailabilityRequirementSet_$_RespiratoryHealth
+ __OBJC_$_PROTOCOL_INSTANCE_METHODS_HKFeatureAvailabilityRequirementsProviding
+ __OBJC_$_PROTOCOL_METHOD_TYPES_HKFeatureAvailabilityRequirementsProviding
+ __OBJC_LABEL_PROTOCOL_$_HKFeatureAvailabilityRequirementsProviding
+ __OBJC_PROTOCOL_$_HKFeatureAvailabilityRequirementsProviding
+ __swiftEmptyArrayStorage
+ __swiftEmptyDictionarySingleton
+ _associated conformance So28HKFeatureAvailabilityContextaSHSCSQ
+ _associated conformance So28HKFeatureAvailabilityContextas20_SwiftNewtypeWrapperSCSY
+ _associated conformance So28HKFeatureAvailabilityContextas20_SwiftNewtypeWrapperSCs35_HasCustomAnyHashableRepresentation
+ _bzero
+ _memmove
+ _objc_allocWithZone
+ _objc_msgSend$hkrp_oxygenSaturationRecordingCompanionAnalysisRequirementSet
+ _objc_msgSend$hkrp_oxygenSaturationRequirementSet
+ _objc_opt_self
+ _objc_retainAutoreleasedReturnValue
+ _objc_retain_x9
+ _swift_allocObject
+ _swift_arrayDestroy
+ _swift_arrayInitWithCopy
+ _swift_bridgeObjectRelease
+ _swift_bridgeObjectRetain
+ _swift_getExistentialTypeMetadata
+ _swift_getForeignTypeMetadata
+ _swift_getObjCClassMetadata
+ _swift_getTupleTypeMetadata2
+ _swift_getWitnessTable
+ _swift_initStackObject
+ _swift_isUniquelyReferenced_nonNull_bridgeObject
+ _swift_isUniquelyReferenced_nonNull_native
+ _swift_release_x19
+ _swift_release_x20
+ _swift_release_x21
+ _swift_release_x26
+ _swift_retain_x20
+ _swift_setDeallocating
+ _swift_unknownObjectRelease
+ _swift_unknownObjectRetain
+ _symbolic $sSY
+ _symbolic $ss21_ObjectiveCBridgeableP
+ _symbolic SS
+ _symbolic So8NSStringC
+ _symbolic _____ So28HKFeatureAvailabilityContexta
+ _type_layout_string So28HKFeatureAvailabilityContexta
- _HKRPUsageRequirementsForFeatureIdentifier
- _OBJC_CLASS_$_NSDictionary
- _OBJC_CLASS_$_NSMutableDictionary
- _objc_msgSend$dictionaryWithObjects:forKeys:count:
- _objc_msgSend$initWithDictionary:
- _objc_msgSend$setObject:forKeyedSubscript:
CStrings:
+ "OxygenSaturationStoreDemo"
- "DeviceSupportsUSLegallyCompliantBloodOxygen"
```
