## IsolatedCoreAudioClient

> `/System/Library/PrivateFrameworks/IsolatedCoreAudioClient.framework/IsolatedCoreAudioClient`

```diff

-7.49.101.0.0
-  __TEXT.__text: 0x1b538
+7.49.200.0.0
+  __TEXT.__text: 0x1b630
   __TEXT.__objc_methlist: 0x80c
   __TEXT.__const: 0x23ec
   __TEXT.__gcc_except_tab: 0x1a84
-  __TEXT.__cstring: 0xbdf
-  __TEXT.__oslogstring: 0x2de2
+  __TEXT.__cstring: 0xbfe
+  __TEXT.__oslogstring: 0x2e1e
   __TEXT.__unwind_info: 0xdb0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0

   __AUTH_CONST.__auth_got: 0x0
   __DATA.__objc_ivar: 0x54
   __DATA.__data: 0x2a0
-  __DATA.__bss: 0x270
+  __DATA.__bss: 0x280
   __DATA_DIRTY.__objc_data: 0x2d0
   __DATA_DIRTY.__bss: 0x88
   - /System/Library/Frameworks/CoreAudio.framework/CoreAudio

   - /usr/lib/libbsm.0.dylib
   - /usr/lib/libc++.1.dylib
   - /usr/lib/libobjc.A.dylib
-  Functions: 800
-  Symbols:   1806
-  CStrings:  257
+  Functions: 802
+  Symbols:   1808
+  CStrings:  259
 
Symbols:
+ GCC_except_table474
+ GCC_except_table485
+ GCC_except_table488
+ GCC_except_table491
+ GCC_except_table492
+ GCC_except_table510
+ GCC_except_table558
+ GCC_except_table562
+ GCC_except_table574
+ GCC_except_table579
+ GCC_except_table580
+ GCC_except_table593
+ GCC_except_table594
+ GCC_except_table603
+ GCC_except_table604
+ GCC_except_table616
+ GCC_except_table626
+ GCC_except_table660
+ GCC_except_table667
+ GCC_except_table668
+ GCC_except_table671
+ GCC_except_table677
+ GCC_except_table684
+ GCC_except_table685
+ GCC_except_table690
+ GCC_except_table691
+ GCC_except_table698
+ GCC_except_table714
+ GCC_except_table717
+ GCC_except_table720
+ GCC_except_table725
+ GCC_except_table728
+ GCC_except_table731
+ GCC_except_table732
+ GCC_except_table738
+ GCC_except_table744
+ GCC_except_table755
+ GCC_except_table777
+ GCC_except_table778
+ GCC_except_table782
+ GCC_except_table788
+ GCC_except_table800
+ GCC_except_table803
+ GCC_except_table804
+ __ZL16stubUserCallback18CoreAudioTimestampPv
+ __ZL17dummyUserCallback18CoreAudioTimestampPv
- GCC_except_table472
- GCC_except_table482
- GCC_except_table483
- GCC_except_table487
- GCC_except_table490
- GCC_except_table508
- GCC_except_table556
- GCC_except_table560
- GCC_except_table570
- GCC_except_table577
- GCC_except_table578
- GCC_except_table591
- GCC_except_table592
- GCC_except_table601
- GCC_except_table602
- GCC_except_table614
- GCC_except_table624
- GCC_except_table658
- GCC_except_table665
- GCC_except_table666
- GCC_except_table669
- GCC_except_table673
- GCC_except_table682
- GCC_except_table683
- GCC_except_table686
- GCC_except_table689
- GCC_except_table696
- GCC_except_table711
- GCC_except_table712
- GCC_except_table716
- GCC_except_table722
- GCC_except_table723
- GCC_except_table727
- GCC_except_table730
- GCC_except_table736
- GCC_except_table740
- GCC_except_table753
- GCC_except_table775
- GCC_except_table776
- GCC_except_table780
- GCC_except_table786
- GCC_except_table796
- GCC_except_table801
- GCC_except_table802
CStrings:
+ "%25s:%-5d Client is starting IO without setting a callback."
+ "AudioAvailabilityApparatus.cpp"
```
