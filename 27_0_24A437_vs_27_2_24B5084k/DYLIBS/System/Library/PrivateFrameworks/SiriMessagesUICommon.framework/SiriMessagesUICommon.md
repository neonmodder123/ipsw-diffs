## SiriMessagesUICommon

> `/System/Library/PrivateFrameworks/SiriMessagesUICommon.framework/SiriMessagesUICommon`

```diff

-3600.47.22.11.2
-  __TEXT.__text: 0xa3c30
-  __TEXT.__const: 0xb8a4
-  __TEXT.__cstring: 0x3044
-  __TEXT.__swift5_typeref: 0x2154
-  __TEXT.__swift5_reflstr: 0x1ffe
+3605.15.1.0.0
+  __TEXT.__text: 0xa5e94
+  __TEXT.__const: 0xb924
+  __TEXT.__cstring: 0x31d4
+  __TEXT.__swift5_typeref: 0x219a
+  __TEXT.__swift5_reflstr: 0x206e
   __TEXT.__swift5_assocty: 0x258
-  __TEXT.__constg_swiftt: 0x2200
+  __TEXT.__constg_swiftt: 0x2210
   __TEXT.__swift5_builtin: 0x64
-  __TEXT.__swift5_fieldmd: 0x2c58
-  __TEXT.__oslogstring: 0x215a
+  __TEXT.__swift5_fieldmd: 0x2cb8
+  __TEXT.__oslogstring: 0x221a
   __TEXT.__swift5_proto: 0xac8
   __TEXT.__swift5_types: 0x310
-  __TEXT.__swift_as_entry: 0xdc
-  __TEXT.__swift_as_ret: 0xfc
-  __TEXT.__swift_as_cont: 0x158
+  __TEXT.__swift_as_entry: 0xe4
+  __TEXT.__swift_as_ret: 0x104
+  __TEXT.__swift_as_cont: 0x160
   __TEXT.__swift5_capture: 0x4a8
   __TEXT.__swift5_protos: 0x34
   __TEXT.__swift5_mpenum: 0x8
-  __TEXT.__unwind_info: 0x3878
-  __TEXT.__eh_frame: 0x4e40
+  __TEXT.__unwind_info: 0x38d8
+  __TEXT.__eh_frame: 0x4f88
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_selrefs: 0x718
   __DATA_CONST.__got: 0x0
-  __AUTH_CONST.__const: 0x7270
+  __AUTH_CONST.__const: 0x7288
   __AUTH_CONST.__objc_const: 0xbb8
-  __AUTH_CONST.__auth_got: 0x1110
+  __AUTH_CONST.__auth_got: 0x1150
   __AUTH.__objc_data: 0x170
-  __AUTH.__data: 0x8b0
-  __DATA.__data: 0x16f0
+  __AUTH.__data: 0x8b8
+  __DATA.__data: 0x1730
   __DATA.__bss: 0x10df0
   __DATA.__common: 0x50
   __DATA_DIRTY.__objc_data: 0x1c0
-  __DATA_DIRTY.__data: 0x1c30
+  __DATA_DIRTY.__data: 0x1c38
   __DATA_DIRTY.__common: 0x70
   __DATA_DIRTY.__bss: 0x3000
   - /System/Library/Frameworks/AVFAudio.framework/AVFAudio

   - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 5898
-  Symbols:   1734
-  CStrings:  492
+  Functions: 5943
+  Symbols:   1745
+  CStrings:  506
 
Symbols:
+ _OUTLINED_FUNCTION_185
+ _OUTLINED_FUNCTION_186
+ _OUTLINED_FUNCTION_187
+ _OUTLINED_FUNCTION_188
+ _OUTLINED_FUNCTION_189
+ _OUTLINED_FUNCTION_190
+ _OUTLINED_FUNCTION_191
+ _symbolic Say_____G 7ToolKit18ConcreteResolvableO
+ _symbolic _____y_____SaySSGG 10Foundation15ListFormatStyleV AA06StringD0V
+ _symbolic _____y_____SaySSG_G 10Foundation15ListFormatStyleV0B4TypeO AA06StringD0V
+ _symbolic _____y_____SaySSG_G 10Foundation15ListFormatStyleV5WidthO AA06StringD0V
CStrings:
+ "#ReadableMessageTypedValue:readableConversationId Could not extract readableConversationId from entity."
+ "#SentMessageTypedValue conversationId: no conversation identifier"
+ "ACTIVITY_MESSAGE"
+ "DIGITAL_TOUCH_MESSAGE"
+ "HANDWRITTEN_MESSAGE"
+ "LOCATION_REQUEST"
+ "Request"
+ "SCREEN_TIME_REQUEST"
+ "SearchForMessages#OtpMessageInform"
+ "SearchForMessages#ReadAreaCodeCaption"
+ "makeDraftSnippetModel(draft:appBundleIdentifiers:siriLocale:)"
+ "makeSentSnippetModel(sentMessages:siriLocale:)"
+ "nextReadableConversationId"
+ "readableConversationId"
+ "repeatAssociatedEntitiesData"
+ "skipAssociatedEntitiesData"
- "makeDraftSnippetModel(draft:appBundleIdentifiers:)"
- "makeSentSnippetModel(sentMessages:)"
```
