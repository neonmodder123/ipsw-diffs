## CoreUtilsSwift

> `/System/Library/PrivateFrameworks/CoreUtilsSwift.framework/CoreUtilsSwift`

```diff

-900.58.0.0.0
-  __TEXT.__text: 0x920bc
+910.21.0.0.0
+  __TEXT.__text: 0x956ac
   __TEXT.__objc_methlist: 0x14c
-  __TEXT.__const: 0x4f54
-  __TEXT.__swift5_typeref: 0x174c
-  __TEXT.__constg_swiftt: 0x1b50
-  __TEXT.__swift5_reflstr: 0xa9d
-  __TEXT.__swift5_fieldmd: 0x12d8
+  __TEXT.__const: 0x5024
+  __TEXT.__swift5_typeref: 0x1816
+  __TEXT.__constg_swiftt: 0x1ba8
+  __TEXT.__swift5_reflstr: 0xabd
+  __TEXT.__swift5_fieldmd: 0x1324
   __TEXT.__swift5_builtin: 0xb4
-  __TEXT.__swift5_assocty: 0x588
-  __TEXT.__cstring: 0xd8f
-  __TEXT.__swift5_capture: 0x874
-  __TEXT.__swift5_proto: 0x2fc
-  __TEXT.__swift5_types: 0x1d0
+  __TEXT.__swift5_assocty: 0x5a0
+  __TEXT.__cstring: 0xdaf
+  __TEXT.__swift5_capture: 0x8a4
+  __TEXT.__swift5_proto: 0x308
+  __TEXT.__swift5_types: 0x1d4
   __TEXT.__swift_as_entry: 0x108
   __TEXT.__swift_as_ret: 0xf8
   __TEXT.__swift_as_cont: 0x1c0
   __TEXT.__swift5_mpenum: 0x28
   __TEXT.__swift5_protos: 0x30
-  __TEXT.__oslogstring: 0x9d0
-  __TEXT.__unwind_info: 0x2a20
-  __TEXT.__eh_frame: 0x6640
+  __TEXT.__oslogstring: 0xa10
+  __TEXT.__unwind_info: 0x2ad8
+  __TEXT.__eh_frame: 0x6700
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x380
-  __DATA_CONST.__objc_classlist: 0xb0
+  __DATA_CONST.__const: 0x388
+  __DATA_CONST.__objc_classlist: 0xb8
   __DATA_CONST.__objc_protolist: 0x20
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x1e8
+  __DATA_CONST.__objc_selrefs: 0x1f0
   __DATA_CONST.__objc_protorefs: 0x10
   __DATA_CONST.__got: 0x0
-  __AUTH_CONST.__const: 0x3838
-  __AUTH_CONST.__objc_const: 0x1af0
-  __AUTH_CONST.__auth_got: 0x1638
-  __AUTH.__objc_data: 0x48
-  __AUTH.__data: 0x408
-  __DATA.__data: 0x15d0
-  __DATA.__bss: 0x4f90
+  __AUTH_CONST.__const: 0x38e8
+  __AUTH_CONST.__objc_const: 0x1be0
+  __AUTH_CONST.__auth_got: 0x1668
+  __AUTH.__objc_data: 0x98
+  __AUTH.__data: 0x528
+  __DATA.__data: 0x1650
+  __DATA.__bss: 0x5090
+  __DATA.__common: 0x8
   __DATA_DIRTY.__objc_data: 0x140
-  __DATA_DIRTY.__data: 0xff8
+  __DATA_DIRTY.__data: 0xfd8
   __DATA_DIRTY.__common: 0x8
   - /System/Library/Frameworks/Combine.framework/Combine
   - /System/Library/Frameworks/CoreFoundation.framework/CoreFoundation

   - /usr/lib/swift/libswift_DarwinFoundation2.dylib
   - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswiftos.dylib
-  Functions: 2916
-  Symbols:   1020
-  CStrings:  170
+  Functions: 2975
+  Symbols:   1039
+  CStrings:  172
 
Symbols:
+ __DATA__TtC14CoreUtilsSwift26CUDarwinNotificationStream
+ __IVARS__TtC14CoreUtilsSwift26CUDarwinNotificationStream
+ __METACLASS_DATA__TtC14CoreUtilsSwift26CUDarwinNotificationStream
+ ___swift_closure_destructor.43Tm
+ ___swift_project_boxed_opaque_existential_0Tm
+ ___unnamed_43
+ ___unnamed_46
+ ___unnamed_56
+ ___unnamed_60
+ ___unnamed_67
+ ___unnamed_68
+ ___unnamed_69
+ ___unnamed_78
+ ___unnamed_81
+ ___unnamed_86
+ _associated conformance 14CoreUtilsSwift26CUDarwinNotificationStreamCAA15CUEventReporterAA5EventAaDP_s23CustomStringConvertible
+ _objc_msgSend$encodeObjectAsDispatchData:flags:error:
+ _swift_getObjCClassFromMetadata
+ _swift_task_deinitOnExecutor
+ _symbolic SDy__________yAA_GG s6UInt64V ScS12ContinuationV
+ _symbolic _____ 14CoreUtilsSwift26CUDarwinNotificationStreamC
+ _symbolic _____3key______yAA_G5valuet s6UInt64V ScS12ContinuationV
+ _symbolic _____SgXw 14CoreUtilsSwift26CUDarwinNotificationStreamC
+ _symbolic _____y______G ScS12ContinuationV s6UInt64V
+ _symbolic _____y______GSg ScS12ContinuationV s6UInt64V
+ _symbolic _____y_______G ScS12ContinuationV11YieldResultO s6UInt64V
+ _symbolic _____y_______GSg ScS12ContinuationV11YieldResultO s6UInt64V
+ _symbolic _____y__________yAB_GG s18_DictionaryStorageC s6UInt64V ScS12ContinuationV
+ _symbolic _____yt______pIeggIrzo_ 14CoreUtilsSwift26CUDarwinNotificationStreamC s5ErrorP
+ _symbolic _____yt______pIggIrzo_ 14CoreUtilsSwift26CUDarwinNotificationStreamC s5ErrorP
- ___swift_closure_destructor.37Tm
- ___unnamed_37
- ___unnamed_40
- ___unnamed_51
- ___unnamed_55
- ___unnamed_57
- ___unnamed_63
- ___unnamed_64
- ___unnamed_70
- ___unnamed_77
- ___unnamed_82
CStrings:
+ "### notify_register_dispatch failed: name=%s, status=%u"
+ "CoreUtilsSwift/MiscUtils.swift"
```
