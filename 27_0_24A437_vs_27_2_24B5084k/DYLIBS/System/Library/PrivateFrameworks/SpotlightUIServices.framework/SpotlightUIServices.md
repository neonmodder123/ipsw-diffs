## SpotlightUIServices

> `/System/Library/PrivateFrameworks/SpotlightUIServices.framework/SpotlightUIServices`

```diff

-236.0.21.105.0
-  __TEXT.__text: 0x53b60
+250.1.2.0.0
+  __TEXT.__text: 0x5452c
   __TEXT.__objc_methlist: 0x4d70
-  __TEXT.__const: 0xbf4
-  __TEXT.__cstring: 0x25e0
+  __TEXT.__const: 0xbe4
+  __TEXT.__cstring: 0x2680
   __TEXT.__ustring: 0x38
   __TEXT.__oslogstring: 0xa5b
   __TEXT.__gcc_except_tab: 0x324
-  __TEXT.__swift5_typeref: 0x470
+  __TEXT.__swift5_typeref: 0x47c
   __TEXT.__constg_swiftt: 0x68c
   __TEXT.__swift5_reflstr: 0x205
   __TEXT.__swift5_fieldmd: 0x248

   __TEXT.__swift_as_cont: 0x80
   __TEXT.__swift5_assocty: 0xa8
   __TEXT.__swift5_builtin: 0x28
-  __TEXT.__unwind_info: 0x1518
-  __TEXT.__eh_frame: 0x660
+  __TEXT.__unwind_info: 0x1528
+  __TEXT.__eh_frame: 0x6a0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__const: 0x5d0
   __DATA_CONST.__objc_classlist: 0x2f8
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x2e98
+  __DATA_CONST.__objc_selrefs: 0x2ea0
   __DATA_CONST.__objc_superrefs: 0x258
   __DATA_CONST.__objc_arraydata: 0x48
-  __DATA_CONST.__got: 0x14e8
+  __DATA_CONST.__got: 0x1508
   __AUTH_CONST.__const: 0xd70
-  __AUTH_CONST.__cfstring: 0x29c0
+  __AUTH_CONST.__cfstring: 0x29e0
   __AUTH_CONST.__objc_const: 0x7278
   __AUTH_CONST.__objc_arrayobj: 0x48
   __AUTH_CONST.__objc_doubleobj: 0x10

   __AUTH.__objc_data: 0x868
   __AUTH.__data: 0x188
   __DATA.__objc_ivar: 0x4c0
-  __DATA.__data: 0x1d8
+  __DATA.__data: 0x1e8
   __DATA.__bss: 0xa90
   __DATA.__common: 0x10
   __DATA_DIRTY.__objc_data: 0x1bb0
-  __DATA_DIRTY.__data: 0x2b0
+  __DATA_DIRTY.__data: 0x2a8
   __DATA_DIRTY.__bss: 0x1d8
   __DATA_DIRTY.__common: 0x28
   - /System/Library/Frameworks/Contacts.framework/Contacts

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 2181
-  Symbols:   4905
-  CStrings:  466
+  Functions: 2185
+  Symbols:   4908
+  CStrings:  469
 
Symbols:
+ _objc_msgSend$initWithSearchResult:
+ _swift_isUniquelyReferenced_nonNull_bridgeObject
+ _symbolic _____Sg_ABt 32AssistantActionSuggestionSupport16SemanticIdentityO
CStrings:
+ "com.apple.spotlight.conversationResumption"
+ "com.apple.spotlight.delete.pasteboard.item"
+ "title section to resume conversations you've made with siri AI"
```
