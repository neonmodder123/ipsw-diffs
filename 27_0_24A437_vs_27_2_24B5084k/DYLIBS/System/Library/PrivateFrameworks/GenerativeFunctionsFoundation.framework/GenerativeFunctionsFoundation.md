## GenerativeFunctionsFoundation

> `/System/Library/PrivateFrameworks/GenerativeFunctionsFoundation.framework/GenerativeFunctionsFoundation`

```diff

-291.6.0.5.102
-  __TEXT.__text: 0x76848
-  __TEXT.__const: 0x12e70
-  __TEXT.__swift5_typeref: 0x405e
-  __TEXT.__cstring: 0x2a4d
-  __TEXT.__constg_swiftt: 0x3354
-  __TEXT.__swift5_reflstr: 0x12f3
-  __TEXT.__swift5_fieldmd: 0x3378
+297.6.0.5.0
+  __TEXT.__text: 0x79ba8
+  __TEXT.__const: 0x13a30
+  __TEXT.__swift5_typeref: 0x4230
+  __TEXT.__cstring: 0x2b7d
+  __TEXT.__constg_swiftt: 0x34c8
+  __TEXT.__swift5_reflstr: 0x13c3
+  __TEXT.__swift5_fieldmd: 0x353c
   __TEXT.__swift5_builtin: 0xb4
   __TEXT.__swift5_assocty: 0x150
-  __TEXT.__swift5_proto: 0x120c
-  __TEXT.__swift5_types: 0x50c
+  __TEXT.__swift5_proto: 0x12bc
+  __TEXT.__swift5_types: 0x538
   __TEXT.__swift5_mpenum: 0x48
   __TEXT.__swift5_protos: 0x94
   __TEXT.__oslogstring: 0x292

   __TEXT.__swift_as_ret: 0x24
   __TEXT.__swift_as_cont: 0x34
   __TEXT.__swift5_capture: 0x58
-  __TEXT.__unwind_info: 0x30e8
-  __TEXT.__eh_frame: 0x3854
+  __TEXT.__unwind_info: 0x3288
+  __TEXT.__eh_frame: 0x39dc
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_methname: 0x0

   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_selrefs: 0x48
   __DATA_CONST.__got: 0x0
-  __AUTH_CONST.__const: 0x8b40
+  __AUTH_CONST.__const: 0x8fc8
   __AUTH_CONST.__auth_got: 0xc98
   __AUTH.__data: 0x460
-  __DATA.__data: 0x2eb8
-  __DATA.__bss: 0x23b80
+  __DATA.__data: 0x3080
+  __DATA.__bss: 0x25180
   __DATA.__common: 0x18
   __DATA_DIRTY.__data: 0x1390
   __DATA_DIRTY.__bss: 0x200

   - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 5383
+  Functions: 5566
   Symbols:   142
-  CStrings:  236
+  CStrings:  244
 
CStrings:
+ "Prompt injection guardrail rejected request. "
+ "The prompt-injection guardrail rejected the request"
+ "classifierUnavailable"
+ "maliciousContentIds"
+ "promptInjectionRejected"
+ "tokenLimitExceeded"
+ "untrustedContentNotExtractable"
+ "unverifiedContent"
```
