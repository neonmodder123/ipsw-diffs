## ActivitySharing

> `/System/Library/PrivateFrameworks/ActivitySharing.framework/ActivitySharing`

```diff

-2027.0.13.0.0
-  __TEXT.__text: 0x436c8
-  __TEXT.__objc_methlist: 0x4adc
-  __TEXT.__const: 0x2c8
-  __TEXT.__cstring: 0x2f95
+2027.1.7.0.0
+  __TEXT.__text: 0x43e9c
+  __TEXT.__objc_methlist: 0x4b24
+  __TEXT.__const: 0x2d8
+  __TEXT.__cstring: 0x3003
   __TEXT.__oslogstring: 0x1df4
   __TEXT.__gcc_except_tab: 0x94
-  __TEXT.__unwind_info: 0xdc0
+  __TEXT.__unwind_info: 0xdc8
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_catlist: 0x28
   __DATA_CONST.__objc_protolist: 0x40
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x25f8
+  __DATA_CONST.__objc_selrefs: 0x2630
   __DATA_CONST.__objc_protorefs: 0x8
   __DATA_CONST.__objc_superrefs: 0x168
   __DATA_CONST.__objc_arraydata: 0x100
   __DATA_CONST.__got: 0x350
   __AUTH_CONST.__const: 0x660
-  __AUTH_CONST.__cfstring: 0x3aa0
-  __AUTH_CONST.__objc_const: 0x81f0
+  __AUTH_CONST.__cfstring: 0x3ba0
+  __AUTH_CONST.__objc_const: 0x8270
   __AUTH_CONST.__objc_intobj: 0x2d0
   __AUTH_CONST.__objc_arrayobj: 0xf0
   __AUTH_CONST.__objc_doubleobj: 0x10
   __AUTH_CONST.__auth_got: 0x0
   __AUTH.__objc_data: 0x5a0
-  __DATA.__objc_ivar: 0x538
+  __DATA.__objc_ivar: 0x540
   __DATA.__data: 0x350
   __DATA.__bss: 0x20
   __DATA_DIRTY.__objc_data: 0xaa0

   - /usr/lib/libMobileGestalt.dylib
   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libobjc.A.dylib
-  Functions: 1880
-  Symbols:   3861
-  CStrings:  682
+  Functions: 1887
+  Symbols:   3874
+  CStrings:  690
 
Symbols:
+ -[ASCodableCloudKitWorkout hasSeymourSExerciseIdentifier]
+ -[ASCodableCloudKitWorkout hasSeymourSWorkoutIdentifier]
+ -[ASCodableCloudKitWorkout setSeymourSExerciseIdentifier:]
+ -[ASCodableCloudKitWorkout setSeymourSWorkoutIdentifier:]
+ -[ASCodableCloudKitWorkout seymourSExerciseIdentifier]
+ -[ASCodableCloudKitWorkout seymourSWorkoutIdentifier]
+ GCC_except_table16
+ OBJC_IVAR_$_ASCodableCloudKitWorkout._seymourSExerciseIdentifier
+ OBJC_IVAR_$_ASCodableCloudKitWorkout._seymourSWorkoutIdentifier
+ __CreateFitnessPlusWorkout
+ _objc_msgSend$setSeymourSExerciseIdentifier:
+ _objc_msgSend$setSeymourSWorkoutIdentifier:
+ _objc_msgSend$setWithObjects:
CStrings:
+ "1537521179"
+ "1546163570"
+ "1567714086"
+ "breeze"
+ "guidedWalk"
+ "seymourSExerciseIdentifier"
+ "seymourSWorkoutIdentifier"
+ "video"
+ "\x88"
- "\x86"
```
