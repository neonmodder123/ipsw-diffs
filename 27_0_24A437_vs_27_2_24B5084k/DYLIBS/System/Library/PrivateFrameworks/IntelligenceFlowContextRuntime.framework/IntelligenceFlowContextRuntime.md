## IntelligenceFlowContextRuntime

> `/System/Library/PrivateFrameworks/IntelligenceFlowContextRuntime.framework/IntelligenceFlowContextRuntime`

```diff

-3600.156.4.501.4
-  __TEXT.__text: 0x11581c
+3605.14.3.501.4
+  __TEXT.__text: 0x117cd0
   __TEXT.__objc_methlist: 0x6d4
-  __TEXT.__const: 0x4a90
-  __TEXT.__swift5_typeref: 0x2bbe
-  __TEXT.__swift5_fieldmd: 0x1258
-  __TEXT.__constg_swiftt: 0x1aec
+  __TEXT.__const: 0x4b60
+  __TEXT.__swift5_typeref: 0x2c0a
+  __TEXT.__swift5_fieldmd: 0x12a8
+  __TEXT.__constg_swiftt: 0x1b4c
   __TEXT.__swift5_builtin: 0xdc
-  __TEXT.__swift5_reflstr: 0xd11
+  __TEXT.__swift5_reflstr: 0xd3f
   __TEXT.__swift5_assocty: 0x220
   __TEXT.__swift5_protos: 0x44
   __TEXT.__swift5_proto: 0x1fc
-  __TEXT.__swift5_types: 0x1ec
-  __TEXT.__cstring: 0x1dec
-  __TEXT.__oslogstring: 0x423b
+  __TEXT.__swift5_types: 0x1f4
+  __TEXT.__cstring: 0x1f4c
+  __TEXT.__oslogstring: 0x43db
   __TEXT.__swift_as_entry: 0x3ac
   __TEXT.__swift_as_ret: 0x3fc
   __TEXT.__swift_as_cont: 0x6ac
-  __TEXT.__swift5_capture: 0x1800
+  __TEXT.__swift5_capture: 0x16a0
   __TEXT.__swift5_mpenum: 0x18
-  __TEXT.__unwind_info: 0x3230
-  __TEXT.__eh_frame: 0x8ae4
+  __TEXT.__unwind_info: 0x32b8
+  __TEXT.__eh_frame: 0x8d74
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
   __DATA_CONST.__const: 0x140
-  __DATA_CONST.__objc_classlist: 0x100
+  __DATA_CONST.__objc_classlist: 0x108
   __DATA_CONST.__objc_protolist: 0x98
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0xae0
+  __DATA_CONST.__objc_selrefs: 0xae8
   __DATA_CONST.__objc_protorefs: 0x58
   __DATA_CONST.__got: 0x10a8
-  __AUTH_CONST.__const: 0x5498
+  __AUTH_CONST.__const: 0x5478
   __AUTH_CONST.__cfstring: 0xc0
-  __AUTH_CONST.__objc_const: 0x1e40
-  __AUTH_CONST.__auth_got: 0x2708
+  __AUTH_CONST.__objc_const: 0x1ef8
+  __AUTH_CONST.__auth_got: 0x2758
   __AUTH.__objc_data: 0x348
-  __AUTH.__data: 0x600
-  __DATA.__data: 0xce0
+  __AUTH.__data: 0x6a0
+  __DATA.__data: 0xd40
   __DATA.__bss: 0x1400
-  __DATA.__common: 0x38
+  __DATA.__common: 0x40
   __DATA_DIRTY.__objc_data: 0x4f0
   __DATA_DIRTY.__data: 0x3158
   __DATA_DIRTY.__bss: 0xd80

   - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 5036
-  Symbols:   331
-  CStrings:  410
+  Functions: 5057
+  Symbols:   332
+  CStrings:  424
 
Symbols:
+ _OBJC_CLASS_$_LSBundleRecord
CStrings:
+ "[AppNameResolver] Unable to resolve app name for %{public}s: %@"
+ "[UserContextFetcher] Dropping non-allow-listed scene %{public}s"
+ "[UserContextFetcher] Window %{public}s (bundle=%{public}s) carries content from %{public}ld Siri-excluded region(s) belonging to [%{sensitive}s]; redacted screenshot and dropped snapshot image data"
+ "[UserContextFetcher] Window %{public}s redaction failed; withholding screenshot"
+ "com.apple.PineBoard"
+ "com.apple.RealityChrome"
+ "com.apple.RealityControlCenter"
+ "com.apple.RealityHUD"
+ "com.apple.RealityHUD.HandSceneLeft"
+ "com.apple.RealityHUD.HandSceneRight"
+ "com.apple.RealityLauncher"
+ "com.apple.Spotlight"
+ "com.apple.SurfBoard"
+ "com.apple.camera"
+ "iOS System Interface"
- "com.apple.TVSystemUIService"
```
