## AXTapToSpeakTime

> `/System/Library/PrivateFrameworks/AXTapToSpeakTime.framework/AXTapToSpeakTime`

```diff

-3240.9.0.0.0
-  __TEXT.__text: 0x6be8
-  __TEXT.__objc_methlist: 0x87c
+3245.7.1.0.0
+  __TEXT.__text: 0x6c74
+  __TEXT.__objc_methlist: 0x88c
   __TEXT.__const: 0xb8
   __TEXT.__gcc_except_tab: 0xe0
   __TEXT.__oslogstring: 0xdfc
   __TEXT.__cstring: 0x900
-  __TEXT.__unwind_info: 0x270
+  __TEXT.__unwind_info: 0x278
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_classlist: 0x28
   __DATA_CONST.__objc_protolist: 0x18
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x810
+  __DATA_CONST.__objc_selrefs: 0x820
   __DATA_CONST.__objc_superrefs: 0x20
   __DATA_CONST.__objc_arraydata: 0x50
   __DATA_CONST.__got: 0x1e0
   __AUTH_CONST.__const: 0x100
   __AUTH_CONST.__cfstring: 0xae0
-  __AUTH_CONST.__objc_const: 0x910
+  __AUTH_CONST.__objc_const: 0x940
   __AUTH_CONST.__objc_intobj: 0x48
   __AUTH_CONST.__objc_arrayobj: 0x60
   __AUTH_CONST.__auth_got: 0x0
   __AUTH.__objc_data: 0x190
-  __DATA.__objc_ivar: 0x5c
+  __DATA.__objc_ivar: 0x60
   __DATA.__data: 0x120
   __DATA.__bss: 0x88
   - /System/Library/Frameworks/AVFAudio.framework/AVFAudio

   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 165
-  Symbols:   662
+  Functions: 167
+  Symbols:   667
   CStrings:  182
 
Symbols:
+ -[AXTapticChimeAsset _initWithChimeSoundType:audioFilePath:hapticsFilePath:isHourly:]
+ -[AXTapticChimeAsset isHourlyChime]
+ -[AXTapticChimesScheduler _scheduleChimeAudioPlayerForAsset:atDate:]
+ GCC_except_table145
+ GCC_except_table74
+ GCC_except_table91
+ GCC_except_table95
+ _OBJC_IVAR_$_AXTapticChimeAsset._isHourlyChime
+ _objc_msgSend$_initWithChimeSoundType:audioFilePath:hapticsFilePath:isHourly:
+ _objc_msgSend$_scheduleChimeAudioPlayerForAsset:atDate:
+ _objc_retain_x23
- -[AXTapticChimeAsset _initWithChimeSoundType:audioFilePath:hapticsFilePath:]
- GCC_except_table144
- GCC_except_table73
- GCC_except_table90
- GCC_except_table94
- _objc_msgSend$_initWithChimeSoundType:audioFilePath:hapticsFilePath:
```
