## NanoPassKitUI

> `/System/Library/PrivateFrameworks/NanoPassKitUI.framework/NanoPassKitUI`

```diff

-1347.0.0.0.0
-  __TEXT.__text: 0x26234
+1353.0.0.0.0
+  __TEXT.__text: 0x261e0
   __TEXT.__objc_methlist: 0x267c
   __TEXT.__const: 0x362
   __TEXT.__cstring: 0x1621

   __TEXT.__swift5_reflstr: 0x42
   __TEXT.__swift5_fieldmd: 0x50
   __TEXT.__swift5_types: 0x8
-  __TEXT.__unwind_info: 0x898
+  __TEXT.__unwind_info: 0x8a0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_catlist: 0x38
   __DATA_CONST.__objc_protolist: 0x30
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x1f38
+  __DATA_CONST.__objc_selrefs: 0x1f30
   __DATA_CONST.__objc_protorefs: 0x8
   __DATA_CONST.__objc_superrefs: 0xe8
   __DATA_CONST.__objc_arraydata: 0x108
   __DATA_CONST.__got: 0x618
   __AUTH_CONST.__const: 0x2a0
   __AUTH_CONST.__cfstring: 0x1460
-  __AUTH_CONST.__objc_const: 0x4728
+  __AUTH_CONST.__objc_const: 0x4738
   __AUTH_CONST.__objc_doubleobj: 0x50
   __AUTH_CONST.__objc_arrayobj: 0x120
   __AUTH_CONST.__auth_got: 0x4d0

   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 946
-  Symbols:   2627
+  Functions: 945
+  Symbols:   2625
   CStrings:  263
 
Symbols:
+ -[PKPass(NanoPassKitUI) npkHasBankLogoCardArt]
+ -[PKPass(NanoPassKitUI) npkNeedsCardArtTopRightText]
+ -[PKPass(NanoPassKitUI) npkNeedsForegroundLabelsWithDynamicCardArtInUse:]
+ _objc_msgSend$npkHasBankLogoCardArt
+ _objc_msgSend$npkNeedsCardArtTopRightText
+ _objc_msgSend$npkNeedsForegroundLabelsWithDynamicCardArtInUse:
- +[NPKPaymentPassView _hasBankLogoForPassBundle:]
- -[NPKDynamicFaceViewAdapter allowsForegroundLabels]
- -[NPKPaymentPassView _foregroundLabelsNeeded]
- -[NPKPaymentPassView _topRightLabelNeeded]
- _objc_msgSend$_foregroundLabelsNeeded
- _objc_msgSend$_hasBankLogoForPassBundle:
- _objc_msgSend$_topRightLabelNeeded
- _objc_msgSend$allowsForegroundLabels
```
