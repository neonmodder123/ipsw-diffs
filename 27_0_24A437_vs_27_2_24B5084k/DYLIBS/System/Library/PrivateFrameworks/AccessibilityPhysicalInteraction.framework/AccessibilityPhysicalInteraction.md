## AccessibilityPhysicalInteraction

> `/System/Library/PrivateFrameworks/AccessibilityPhysicalInteraction.framework/AccessibilityPhysicalInteraction`

```diff

-3240.9.0.0.0
-  __TEXT.__text: 0x1e5d4
-  __TEXT.__objc_methlist: 0x20f4
+3245.7.1.0.0
+  __TEXT.__text: 0x1e74c
+  __TEXT.__objc_methlist: 0x20fc
   __TEXT.__const: 0x6ed
   __TEXT.__dlopen_cstrs: 0xb6
   __TEXT.__constg_swiftt: 0x8c
   __TEXT.__swift5_typeref: 0xc0
   __TEXT.__swift5_builtin: 0x14
   __TEXT.__swift5_types: 0x10
+  __TEXT.__cstring: 0x9bd
   __TEXT.__swift5_fieldmd: 0x30
   __TEXT.__swift5_proto: 0x30
   __TEXT.__swift_as_entry: 0x68
   __TEXT.__swift_as_cont: 0x70
-  __TEXT.__cstring: 0x931
-  __TEXT.__oslogstring: 0x654
+  __TEXT.__oslogstring: 0x730
   __TEXT.__swift5_capture: 0xa8
   __TEXT.__swift_as_ret: 0x10
   __TEXT.__gcc_except_tab: 0x230
-  __TEXT.__unwind_info: 0xad0
+  __TEXT.__unwind_info: 0xac8
   __TEXT.__eh_frame: 0x5d0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0

   __DATA_CONST.__objc_catlist: 0x18
   __DATA_CONST.__objc_protolist: 0x38
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x18b8
+  __DATA_CONST.__objc_selrefs: 0x18c0
   __DATA_CONST.__objc_superrefs: 0x90
   __DATA_CONST.__objc_arraydata: 0x58
-  __DATA_CONST.__got: 0x588
+  __DATA_CONST.__got: 0x590
   __AUTH_CONST.__const: 0x660
-  __AUTH_CONST.__cfstring: 0x7c0
+  __AUTH_CONST.__cfstring: 0x7e0
   __AUTH_CONST.__objc_const: 0x28d0
   __AUTH_CONST.__objc_intobj: 0x48
   __AUTH_CONST.__objc_dictobj: 0x28
   __AUTH_CONST.__objc_doubleobj: 0x50
   __AUTH_CONST.__objc_arrayobj: 0x18
-  __AUTH_CONST.__auth_got: 0x878
+  __AUTH_CONST.__auth_got: 0x888
   __AUTH.__objc_data: 0x730
   __DATA.__objc_ivar: 0x1e0
   __DATA.__data: 0x338

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 973
+  Functions: 975
   Symbols:   2277
-  CStrings:  142
+  CStrings:  147
 
CStrings:
+ "AA: StartFlow - AssistiveAccess feature action handled; attempting to enter ClarityBoard."
+ "AA: StartFlow - toggleAssistiveAccess called (AssistiveTouch/Back Tap/Action Button); attempting to enter ClarityBoard."
+ "Error occurred trying to toggle live recognition: %s"
+ "Reader is already frontmost - No op"
+ "com.apple.accessibility.AccessibilityReader"
```
