## HearingModeUI

> `/System/Library/PrivateFrameworks/HearingModeUI.framework/HearingModeUI`

### Sections with Same Size but Changed Content

- `__TEXT.__swift5_reflstr`

```diff

-40.41.1.1.10
-  __TEXT.__text: 0x4a24c
+41.4.0.0.0
+  __TEXT.__text: 0x4a2bc
   __TEXT.__objc_methlist: 0x1730
-  __TEXT.__const: 0x2d44
-  __TEXT.__cstring: 0x4886
+  __TEXT.__const: 0x2d64
+  __TEXT.__cstring: 0x4786
   __TEXT.__gcc_except_tab: 0x244
   __TEXT.__ustring: 0x352
   __TEXT.__oslogstring: 0x85a
-  __TEXT.__swift5_typeref: 0x1556
-  __TEXT.__swift5_capture: 0x3a4
+  __TEXT.__swift5_typeref: 0x155c
+  __TEXT.__swift5_capture: 0x3e4
   __TEXT.__swift5_reflstr: 0xc2e
   __TEXT.__swift5_assocty: 0x240
-  __TEXT.__constg_swiftt: 0x1c64
-  __TEXT.__swift5_fieldmd: 0xc48
+  __TEXT.__constg_swiftt: 0x1c4c
+  __TEXT.__swift5_fieldmd: 0xc3c
   __TEXT.__swift5_builtin: 0x1b8
   __TEXT.__swift5_proto: 0x1dc
   __TEXT.__swift5_types: 0x128

   __TEXT.__swift_as_ret: 0x1c
   __TEXT.__swift_as_cont: 0x20
   __TEXT.__swift5_mpenum: 0x8
-  __TEXT.__unwind_info: 0x1410
+  __TEXT.__unwind_info: 0x1420
   __TEXT.__eh_frame: 0x358
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0

   __DATA_CONST.__objc_catlist: 0x10
   __DATA_CONST.__objc_protolist: 0x88
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x12e0
+  __DATA_CONST.__objc_selrefs: 0x12f0
   __DATA_CONST.__objc_protorefs: 0x40
   __DATA_CONST.__objc_superrefs: 0x58
   __DATA_CONST.__got: 0x590
-  __AUTH_CONST.__const: 0x1ec8
+  __AUTH_CONST.__const: 0x1f68
   __AUTH_CONST.__cfstring: 0x1ee0
-  __AUTH_CONST.__objc_const: 0x4370
-  __AUTH_CONST.__auth_got: 0xcf0
-  __AUTH.__objc_data: 0x2268
-  __AUTH.__data: 0xb80
+  __AUTH_CONST.__objc_const: 0x4350
+  __AUTH_CONST.__auth_got: 0xd08
+  __AUTH.__objc_data: 0x2248
+  __AUTH.__data: 0xb70
   __DATA.__objc_ivar: 0x11c
   __DATA.__data: 0xce0
   __DATA.__common: 0x2d8

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 2111
-  Symbols:   1759
-  CStrings:  499
+  Functions: 2118
+  Symbols:   1765
+  CStrings:  498
 
Symbols:
+ _objc_msgSend$replaceCurrentItemWithPlayerItem:
+ _objc_msgSend$setActionAtItemEnd:
+ _swift_isEscapingClosureAtFileLocation
+ _swift_retain_x24
+ _swift_retain_x26
+ _symbolic Ig_
+ _symbolic So14AVPlayerLooperCSg
- _symbolic So14AVPlayerLooperC
CStrings:
+ "MEDIA_ASSIST_FOOTER_DEVICE_NAME"
+ "MEDIA_ASSIST_FOOTER_MARKETING_NAME"
+ "MEDIA_ASSIST_TEST_RESULTS_DETAIL"
+ "init(frame:urlForResource:bounds:loops:)"
- " Sound. You can make changes in the Hearing Assistance settings of your "
- "Media Adjustments will be used instead of Headphone Accommodations for \""
- "Media Adjustments will be used instead of Headphone Accommodations for your "
- "Your test results are being used to make music, video, calls and FaceTime easier for you to hear.\n\nIt may take time to get used to the way your "
- "init(frame:urlForResource:bounds:)"
```
