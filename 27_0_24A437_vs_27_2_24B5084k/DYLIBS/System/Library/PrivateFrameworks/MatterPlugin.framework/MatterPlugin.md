## MatterPlugin

> `/System/Library/PrivateFrameworks/MatterPlugin.framework/MatterPlugin`

```diff

-86.1.1.0.0
-  __TEXT.__text: 0x4abc0
+89.0.0.0.0
+  __TEXT.__text: 0x4af1c
   __TEXT.__objc_methlist: 0x4a3c
   __TEXT.__const: 0x110
   __TEXT.__cstring: 0x1413
-  __TEXT.__oslogstring: 0x5eb8
-  __TEXT.__gcc_except_tab: 0x1b18
+  __TEXT.__oslogstring: 0x5f5f
+  __TEXT.__gcc_except_tab: 0x1bac
   __TEXT.__unwind_info: 0x1288
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0

   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libbsm.0.dylib
   - /usr/lib/libobjc.A.dylib
-  Functions: 1721
+  Functions: 1726
   Symbols:   3569
-  CStrings:  626
+  CStrings:  629
 
Symbols:
+ ___block_descriptor_72_e8_32s40s48s56s64r_e5_v8?0lr64l8s32l8s40l8s48l8s56l8
+ ___block_descriptor_88_e8_32s40s48s56s64s72s80r_e5_v8?0lr80l8s32l8s40l8s48l8s56l8s64l8s72l8
- ___block_descriptor_48_e8_32s40r_e5_v8?0lr40l8s32l8
- ___block_descriptor_80_e8_32s40s48s56s64s72r_e5_v8?0lr72l8s32l8s40l8s48l8s56l8s64l8
CStrings:
+ "%@ denying invokeCommands: unexpected batch entry class"
+ "%@ denying invokeCommands: unexpected command entry class"
+ "%@ denying invokeCommands: unexpected commands class"
```
