## SymptomNetworkDiagnostics

> `/System/Library/PrivateFrameworks/SymptomNetworkDiagnostics.framework/SymptomNetworkDiagnostics`

```diff

-2394.0.4.0.0
-  __TEXT.__text: 0x14114
+2394.40.15.0.0
+  __TEXT.__text: 0x138c4
   __TEXT.__objc_methlist: 0x290
   __TEXT.__cstring: 0x1c8
-  __TEXT.__const: 0xdf0
-  __TEXT.__constg_swiftt: 0x5cc
-  __TEXT.__swift5_typeref: 0x3dd
-  __TEXT.__swift5_reflstr: 0x549
-  __TEXT.__swift5_fieldmd: 0x770
+  __TEXT.__const: 0xf10
+  __TEXT.__swift5_typeref: 0x3e9
+  __TEXT.__swift5_reflstr: 0x569
+  __TEXT.__swift5_assocty: 0x90
+  __TEXT.__constg_swiftt: 0x5e8
+  __TEXT.__swift5_fieldmd: 0x7b0
   __TEXT.__swift5_protos: 0xc
-  __TEXT.__swift5_proto: 0x7c
-  __TEXT.__swift5_types: 0x58
+  __TEXT.__swift5_proto: 0x90
+  __TEXT.__swift5_types: 0x5c
   __TEXT.__swift5_capture: 0x164
-  __TEXT.__oslogstring: 0x90b
+  __TEXT.__oslogstring: 0x8aa
   __TEXT.__swift_as_entry: 0x44
   __TEXT.__swift_as_ret: 0x44
   __TEXT.__swift_as_cont: 0x7c
-  __TEXT.__swift5_assocty: 0x78
   __TEXT.__swift5_builtin: 0x14
-  __TEXT.__unwind_info: 0x6f0
-  __TEXT.__eh_frame: 0x638
+  __TEXT.__unwind_info: 0x710
+  __TEXT.__eh_frame: 0x6a8
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0xd0
+  __DATA_CONST.__const: 0xc8
   __DATA_CONST.__objc_classlist: 0x18
   __DATA_CONST.__objc_protolist: 0x38
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x208
+  __DATA_CONST.__objc_selrefs: 0x200
   __DATA_CONST.__objc_protorefs: 0x20
   __DATA_CONST.__got: 0x0
-  __AUTH_CONST.__const: 0xc90
+  __AUTH_CONST.__const: 0xce0
   __AUTH_CONST.__objc_const: 0x5a8
   __AUTH_CONST.__auth_got: 0x698
   __AUTH.__data: 0x250
-  __DATA.__data: 0x270
-  __DATA.__bss: 0xaf0
+  __DATA.__data: 0x280
+  __DATA.__bss: 0xd70
   __DATA_DIRTY.__objc_data: 0x358
   __DATA_DIRTY.__data: 0x690
   __DATA_DIRTY.__bss: 0x2a0

   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
-  Functions: 704
+  Functions: 714
   Symbols:   337
-  CStrings:  61
+  CStrings:  59
 
Symbols:
+ _associated conformance 25SymptomNetworkDiagnostics0bC15RadioTechnologyOSHAASQ
+ _symbolic _____ 25SymptomNetworkDiagnostics0bC15RadioTechnologyO
+ _symbolic _____ s5UInt8V
- _objc_msgSend$decodeBoolForKey:
- _swift_bridgeObjectRetain_n
- _symbolic _____Sg 25SymptomNetworkDiagnostics0bC7PayloadO
CStrings:
- "Failed to synthesize payload for event: %s"
- "Synthetically generating event %s at %f"
```
