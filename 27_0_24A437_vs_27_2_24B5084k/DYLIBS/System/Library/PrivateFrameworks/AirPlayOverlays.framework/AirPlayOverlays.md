## AirPlayOverlays

> `/System/Library/PrivateFrameworks/AirPlayOverlays.framework/AirPlayOverlays`

```diff

-799.3.0.0.0
-  __TEXT.__text: 0x8b24
-  __TEXT.__objc_methlist: 0x584
+807.2.0.0.0
+  __TEXT.__text: 0x8710
+  __TEXT.__objc_methlist: 0x51c
   __TEXT.__const: 0x32c
   __TEXT.__cstring: 0x271
   __TEXT.__oslogstring: 0x30e

   __TEXT.__swift_as_cont: 0x8
   __TEXT.__swift5_protos: 0x4
   __TEXT.__swift5_builtin: 0x28
-  __TEXT.__unwind_info: 0x328
+  __TEXT.__unwind_info: 0x308
   __TEXT.__eh_frame: 0x88
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x88
+  __DATA_CONST.__const: 0xd0
   __DATA_CONST.__objc_classlist: 0x50
   __DATA_CONST.__objc_protolist: 0x50
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x250
+  __DATA_CONST.__objc_selrefs: 0x210
   __DATA_CONST.__objc_protorefs: 0x28
   __DATA_CONST.__got: 0xb0
   __AUTH_CONST.__const: 0x328
   __AUTH_CONST.__cfstring: 0x40
-  __AUTH_CONST.__objc_const: 0x1398
-  __AUTH_CONST.__auth_got: 0x430
+  __AUTH_CONST.__objc_const: 0x1308
+  __AUTH_CONST.__auth_got: 0x438
   __AUTH.__objc_data: 0x378
   __AUTH.__data: 0x328
   __DATA.__data: 0x358

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 272
-  Symbols:   348
+  Functions: 264
+  Symbols:   341
   CStrings:  33
 
Symbols:
+ _objc_msgSend$initWithStatusBarType:statusBarSize:leftSideClimateControls:rightSideClimateControls:
+ _objc_retain_x8
- _objc_msgSend$initWithIdentifier:data:rect:
- _objc_msgSend$leading
- _objc_msgSend$position
- _objc_msgSend$primary
- _objc_msgSend$primaryClimateControls
- _objc_msgSend$secondaryClimateControls
- _objc_msgSend$statusBarSize
- _objc_msgSend$statusBarType
- _objc_msgSend$trailing
```
