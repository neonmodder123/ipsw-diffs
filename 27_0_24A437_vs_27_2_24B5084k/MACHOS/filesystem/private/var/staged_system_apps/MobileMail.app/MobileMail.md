## MobileMail

> `/private/var/staged_system_apps/MobileMail.app/MobileMail`

### Sections with Same Size but Changed Content

- `__TEXT.__ustring`
- `__TEXT.__dlopen_cstrs`
- `__TEXT.__swift5_mpenum`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_intobj`
- `__DATA_CONST.__objc_doubleobj`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA_CONST.__objc_dictobj`

```diff

-3901.100.1.2.14
-  __TEXT.__text: 0x61a65c
-  __TEXT.__auth_stubs: 0x8090
-  __TEXT.__objc_stubs: 0x467a0
-  __TEXT.__objc_methlist: 0x2708c
-  __TEXT.__const: 0x15d80
-  __TEXT.__gcc_except_tab: 0x55f74
-  __TEXT.__objc_methname: 0x69b47
-  __TEXT.__cstring: 0x19574
-  __TEXT.__objc_classname: 0x64da
-  __TEXT.__objc_methtype: 0x12ad5
-  __TEXT.__oslogstring: 0x1a8f4
+3901.200.34.0.0
+  __TEXT.__text: 0x62750c
+  __TEXT.__auth_stubs: 0x8290
+  __TEXT.__objc_stubs: 0x46900
+  __TEXT.__objc_methlist: 0x26c2c
+  __TEXT.__gcc_except_tab: 0x56180
+  __TEXT.__objc_methname: 0x699c7
+  __TEXT.__cstring: 0x197c4
+  __TEXT.__objc_classname: 0x639a
+  __TEXT.__objc_methtype: 0x12838
+  __TEXT.__const: 0x163b0
+  __TEXT.__oslogstring: 0x1ac92
   __TEXT.__ustring: 0xace
   __TEXT.__dlopen_cstrs: 0x2cc
-  __TEXT.__swift5_typeref: 0x10708
-  __TEXT.__swift5_capture: 0x9454
-  __TEXT.__constg_swiftt: 0x4dd0
-  __TEXT.__swift5_reflstr: 0x3e5f
-  __TEXT.__swift5_assocty: 0x18b0
-  __TEXT.__swift5_fieldmd: 0x3a04
-  __TEXT.__swift5_builtin: 0x3e8
-  __TEXT.__swift5_protos: 0x4c
-  __TEXT.__swift5_proto: 0xba8
-  __TEXT.__swift5_types: 0x5f0
-  __TEXT.__swift_as_entry: 0x5e8
-  __TEXT.__swift_as_ret: 0x668
-  __TEXT.__swift_as_cont: 0xa78
+  __TEXT.__constg_swiftt: 0x4f38
+  __TEXT.__swift5_typeref: 0x10898
+  __TEXT.__swift5_builtin: 0x410
+  __TEXT.__swift5_reflstr: 0x3ecd
+  __TEXT.__swift5_fieldmd: 0x3ad4
+  __TEXT.__swift5_assocty: 0x1910
+  __TEXT.__swift5_proto: 0xbe0
+  __TEXT.__swift5_types: 0x604
+  __TEXT.__swift_as_entry: 0x618
+  __TEXT.__swift_as_ret: 0x69c
+  __TEXT.__swift5_capture: 0x9604
+  __TEXT.__swift_as_cont: 0xaec
   __TEXT.__swift5_mpenum: 0x20
-  __TEXT.__unwind_info: 0x1aa28
-  __TEXT.__eh_frame: 0x58a4
-  __DATA_CONST.__const: 0x260a8
-  __DATA_CONST.__cfstring: 0xe980
-  __DATA_CONST.__objc_classlist: 0xed0
+  __TEXT.__swift5_protos: 0x54
+  __TEXT.__unwind_info: 0x1ab60
+  __TEXT.__eh_frame: 0x5a74
+  __DATA_CONST.__const: 0x267c8
+  __DATA_CONST.__cfstring: 0xe940
+  __DATA_CONST.__objc_classlist: 0xea8
   __DATA_CONST.__objc_catlist: 0xf0
-  __DATA_CONST.__objc_protolist: 0xbf8
+  __DATA_CONST.__objc_protolist: 0xbc0
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_protorefs: 0x298
-  __DATA_CONST.__objc_superrefs: 0xa58
+  __DATA_CONST.__objc_protorefs: 0x290
+  __DATA_CONST.__objc_superrefs: 0xa40
+  __DATA_CONST.__objc_arraydata: 0x500
+  __DATA_CONST.__objc_dictobj: 0xa0
   __DATA_CONST.__objc_intobj: 0x978
   __DATA_CONST.__objc_doubleobj: 0x290
-  __DATA_CONST.__objc_arraydata: 0x500
   __DATA_CONST.__objc_arrayobj: 0x198
-  __DATA_CONST.__objc_dictobj: 0xa0
-  __DATA_CONST.__auth_got: 0x4058
-  __DATA_CONST.__got: 0x4200
-  __DATA_CONST.__auth_ptr: 0x2040
-  __DATA.__objc_const: 0x381b0
-  __DATA.__objc_selrefs: 0x15bc0
-  __DATA.__objc_ivar: 0x1fec
-  __DATA.__objc_data: 0xc6c8
-  __DATA.__data: 0x10a30
-  __DATA.__bss: 0x16648
-  __DATA.__common: 0x938
+  __DATA_CONST.__auth_got: 0x4158
+  __DATA_CONST.__got: 0x4278
+  __DATA_CONST.__auth_ptr: 0x2068
+  __DATA.__objc_const: 0x37968
+  __DATA.__objc_selrefs: 0x15b20
+  __DATA.__objc_ivar: 0x1fdc
+  __DATA.__objc_data: 0xc470
+  __DATA.__data: 0x109c0
+  __DATA.__bss: 0x16c38
+  __DATA.__common: 0x958
   - /System/Library/Frameworks/Accounts.framework/Accounts
   - /System/Library/Frameworks/AppIntents.framework/AppIntents
   - /System/Library/Frameworks/Charts.framework/Charts

   - /System/Library/PrivateFrameworks/GraphicsServices.framework/GraphicsServices
   - /System/Library/PrivateFrameworks/HybridSearch.framework/HybridSearch
   - /System/Library/PrivateFrameworks/HybridSearchAdapter.framework/HybridSearchAdapter
+  - /System/Library/PrivateFrameworks/HybridSearchResultRanker.framework/HybridSearchResultRanker
   - /System/Library/PrivateFrameworks/MIME.framework/MIME
   - /System/Library/PrivateFrameworks/MailKit.framework/MailKit
   - /System/Library/PrivateFrameworks/MailServices.framework/MailServices

   - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 26720
-  Symbols:   4997
-  CStrings:  22392
+  Functions: 26906
+  Symbols:   5046
+  CStrings:  22359
 
Symbols:
+ _$s10Foundation11JSONEncoderC16OutputFormattingV10sortedKeysAEvgZ
+ _$s10Foundation11JSONEncoderC16OutputFormattingV13prettyPrintedAEvgZ
+ _$s10Foundation11JSONEncoderC16OutputFormattingVMa
+ _$s10Foundation11JSONEncoderC16OutputFormattingVs10SetAlgebraAAMc
+ _$s10Foundation11JSONEncoderC16outputFormattingAC06OutputD0VvsTj
+ _$s10Foundation11JSONEncoderC20DateEncodingStrategyO7iso8601yA2EmFWC
+ _$s10Foundation11JSONEncoderC20DateEncodingStrategyOMa
+ _$s10Foundation11JSONEncoderC20dateEncodingStrategyAC04DatedE0OvsTj
+ _$s10Foundation11JSONEncoderC6encodeyAA4DataVxKSERzlFTj
+ _$s10Foundation11JSONEncoderCACycfC
+ _$s10Foundation3URLV22appendingPathComponentyACSSF
+ _$s10Foundation3URLV22appendingPathExtensionyACSSF
+ _$s10Foundation4DataV5write2to7optionsyAA3URLV_So20NSDataWritingOptionsVtKF
+ _$s19HybridSearchAdapter04MailB6ClientV18captureL1ForGsTool5query5limit0aB12ResultRanker0iJ11DumpPayloadVSS_SitYaKF
+ _$s19HybridSearchAdapter04MailB6ClientV18captureL1ForGsTool5query5limit0aB12ResultRanker0iJ11DumpPayloadVSS_SitYaKFTu
+ _$s24HybridSearchResultRanker17GsToolDumpPayloadVMa
+ _$s24HybridSearchResultRanker17GsToolDumpPayloadVSEAAMc
+ _$s7Instants5ClockPTl
+ _$sScA15unownedExecutorScevgTq
+ _$sScAMp
+ _$sSceySceBecfC
+ _$sScg4next9isolationxSgScA_pSgYi_tYaKF
+ _$sScg4next9isolationxSgScA_pSgYi_tYaKFTu
+ _$sScg9cancelAllyyF
+ _$sSiSzsMc
+ _$sSq6MailUISSRszlE12isNilOrEmptySbvg
+ _$ss13ManagedBufferC6create15minimumCapacity16makingHeaderWithAByxq_GSi_xAFKXEtKFZ
+ _$ss13ManagedBufferCMa
+ _$ss13ManagedBufferCMo
+ _$ss15ContinuousClockVMa
+ _$ss15ContinuousClockVs0B0sMc
+ _$ss15InstantProtocolP8advanced2byx8DurationQz_tFTj
+ _$ss21withThrowingTaskGroup2of9returning9isolation4bodyq_xm_q_mScA_pSgYiq_Scgyxs5Error_pGzYaKXEtYaKs8SendableRzr0_lF
+ _$ss21withThrowingTaskGroup2of9returning9isolation4bodyq_xm_q_mScA_pSgYiq_Scgyxs5Error_pGzYaKXEtYaKs8SendableRzr0_lFTu
+ _$ss5ClockP3now7InstantQzvgTj
+ _$ss5ClockP5sleep5until9tolerancey7InstantQz_8DurationQzSgtYaKFTj
+ _$ss5ClockP5sleep5until9tolerancey7InstantQz_8DurationQzSgtYaKFTjTu
+ _$ss5ClockP7InstantAB_s0B8ProtocolTn
+ _$ss5ClockPss010ContinuousA0VRszrlE10continuousADvgZ
+ _$ss5ClockTL
+ _$ss6UInt32VMn
+ _$ss8DurationV7secondsyABxSzRzlFZ
+ _ShouldUpdateAccessibilityIdentifier
+ _UIAccessibilityTraitNone
+ _UIAccessibilityTraitNotEnabled
+ _os_variant_has_internal_content
+ _swift_defaultActor_deallocate
+ _swift_defaultActor_destroy
+ _swift_defaultActor_initialize
CStrings:
+ "!\xf0\xf0B"
+ "#SiriMail ComposeMessageIntent: preserving reused compose's selected account isDefault: %{bool}d"
+ "#SiriMail ComposeMessageIntent: reusing blank on-screen compose in place"
+ "#SiriMail pinExistingDocumentIdentifier: reused compose controller missing documentID; returning fallback identifier"
+ "$defaultActor"
+ "%p: Skipping fetched-results report for a stale set of search futures."
+ ") ---\n"
+ "--- L1 Ranking Dump (gstool ranking dump-l1 --limit "
+ ".json --limit "
+ ".json when capture succeeded.\n"
+ "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/Developer/SDKs/iPhoneOS27.2.Internal.sdk/System/Library/PrivateFrameworks/Email.framework/Headers/EMContentRequestOptions.h"
+ "/AppleInternal/Library/Frameworks/ContextStagingIntents.framework/ContextStagingIntents"
+ "/Library/Caches/com.apple.xbs/<UUID>/TemporaryDirectory.<TMP>/Sources/Mail/MobileMail/Source/Conversation View/Conversation Header View/StaticConversationSenderHeaderView.swift"
+ "/Library/Caches/com.apple.xbs/<UUID>/TemporaryDirectory.<TMP>/Sources/Mail/MobileMail/Source/Message List/Mailbox Filter/View/MFMailboxFilterCriteriaBarButtonItem.h"
+ "/Library/Caches/com.apple.xbs/<UUID>/TemporaryDirectory.<TMP>/Sources/Mail/MobileMail/Source/MobileMailUI/Search/SearchCollectionViewCellRegistrationFactory.swift"
+ "/Library/Caches/com.apple.xbs/<UUID>/TemporaryDirectory.<TMP>/Sources/Mail/MobileMail/Source/Status/Modern/MailStatusMailboxActivityObserver.h"
+ "<%@: %p> Could not resolve visible items for the restoration state"
+ "<%@: %p> No message list item for orbed index path: %{public}@"
+ "@\"MailSplitViewControllerCoordinator\""
+ "Attached as "
+ "Attempt to add empty suggestion to recent list"
+ "Capturing the L1 ranking dump, query length %{public}ld"
+ "Could not write the L1 ranking dump: %{public}s"
+ "Dock tapped while a transition is in flight, presenting after animation is finished."
+ "Dropped the L1 ranking dump: a later report superseded it"
+ "Dropped the L1 ranking dump: the report was not filed"
+ "HIDE_RELATED_MESSAGES_COUNT%1$lu"
+ "NO_RELATED_MESSAGES"
+ "Note: captured UNSCOPED — mailbox scope and suggestions are NOT applied. On a scoped "
+ "Replay with: gstool ranking replay-l2 "
+ "SHOW_RELATED_MESSAGES_COUNT%1$lu"
+ "SearchRankingDump"
+ "T@\"<EFCancelable>\",&,N,V_relatedMessagesCountToken"
+ "T@\"MailSplitViewControllerCoordinator\",&,N,V_coordinator"
+ "TB,N,GisSubjectVisible,V_subjectVisible"
+ "TB,N,V_usesBarMinimization"
+ "TB,N,V_usesBarMinimizationForHeaderView"
+ "TB,R,N,GisBarMinimizationEnabled"
+ "TQ,N,V_relatedMessagesCount"
+ "Wrote the L1 ranking dump, %{public}ld bytes"
+ "[ContextStaging] Failed to load ContextStagingIntents.framework: %{public}s"
+ "_TtC10MobileMailP33_2190D88703EE98958110D55910D4AF9811L1DumpStore"
+ "_beginUsingNavigationBarMinimization"
+ "_endUsingNavigationBarMinimization"
+ "_fetchGLPStatsAndUpdateIndexState:"
+ "_isCardActionDisabled"
+ "_loadContextStagingIntentsFrameworkIfInternal"
+ "_relatedMessagesCount"
+ "_relatedMessagesCountToken"
+ "_renderedCellMessageListItemAtIndexPath:"
+ "_resolvedMessageListItemAtIndexPath:"
+ "_resolvedMessageListItemsAtIndexPaths:"
+ "_setNavigationBarSafeAreaAdjustment:"
+ "_shouldUseBarMinimizationForHeaderView"
+ "_subjectVisible"
+ "_updateAccessibilityLabelsIfNeeded"
+ "_updateAccessibilityTraits"
+ "_updateBarMinimizationForCurrentScrollPosition"
+ "_updateRelatedMessagesCountAsync"
+ "_usesBarMinimization"
+ "_usesBarMinimizationForHeaderView"
+ "actuallyVisibleIndexPaths"
+ "barMinimizationEnabled"
+ "cancelL1DumpRequest:"
+ "clearConfiguration"
+ "contentsOfDirectoryAtURL:includingPropertiesForKeys:options:error:"
+ "isBarMinimizationEnabled"
+ "isDefaultDeliveryAccount"
+ "json"
+ "latestOffered"
+ "moveItemAtURL:toURL:error:"
+ "mui_updateAccessibilityIdentifierHasHighlighted"
+ "navigationBarMinimization"
+ "profileConnectionDidReceiveAllowCloudSyncChangedNotification:userInfo:"
+ "q64@0:8@16@24@32B40B44@48@?56"
+ "relatedMessagesActionVisible"
+ "relatedMessagesCount"
+ "relatedMessagesCountToken"
+ "search this holds results the reporter did not see, and may omit ones they did.\n"
+ "sendingAccountProxy"
+ "setBackground:"
+ "setMinimizationBehavior:"
+ "setRelatedMessagesCount:"
+ "setRelatedMessagesCountToken:"
+ "setSubjectVisible:"
+ "setUsesBarMinimization:"
+ "setUsesBarMinimizationForHeaderView:"
+ "smartActionButtonFrameInWindow"
+ "subjectVisible"
+ "the capture did not finish within its budget"
+ "updateUsesBarMinimizationForHeaderViewIfNeeded"
+ "usesBarMinimization"
+ "usesBarMinimizationForHeaderView"
+ "v32@?0@\"NSNumber\"8Q16^B24"
+ "written"
+ "\x91"
- "!\xf0\xf02"
- "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/Developer/SDKs/iPhoneOS27.0.Internal.sdk/System/Library/PrivateFrameworks/Email.framework/Headers/EMContentRequestOptions.h"
- "/Library/Caches/com.apple.xbs/<UUID>/TemporaryDirectory.<TMP>/Sources/Mail/MobileMail/MFMailboxFilterCriteriaBarButtonItem.h"
- "/Library/Caches/com.apple.xbs/<UUID>/TemporaryDirectory.<TMP>/Sources/Mail/MobileMail/MailStatusMailboxActivityObserver.h"
- "/Library/Caches/com.apple.xbs/<UUID>/TemporaryDirectory.<TMP>/Sources/Mail/MobileMail/MobileMailUI/Search/SearchCollectionViewCellRegistrationFactory.swift"
- "<%@: %p> Fetching visible items for the restoration state failed with error: %@"
- "@\"<DimmingViewDelegate>\""
- "@\"<ECMailbox>\"16@0:8"
- "@\"<ECMailbox>\"24@0:8q16"
- "@\"<EDDeliveryAccount>\"16@0:8"
- "@\"<EDReceivingAccount>\"16@?0@\"NSString\"8"
- "@\"ACAccount\"16@0:8"
- "@\"AccountLinkButton\""
- "@\"UIView\"24@0:8@\"NSString\"16"
- "@\"UIView<PSHeaderFooterView>\"24@0:8@\"PSSpecifier\"16"
- "@\"UIViewController\"24@0:8@\"NSString\"16"
- "@16@?0@\"<EDAccount>\"8"
- "@24@0:8@\"NSString\"16"
- "AccountLinkButton"
- "AccountLinkButtonView"
- "B32@0:8@?16@?24"
- "B32@0:8@?<v@?@\"<UIViewControllerTransitionCoordinatorContext>\">16@?<v@?@\"<UIViewControllerTransitionCoordinatorContext>\">24"
- "B40@0:8@\"UIView\"16@?<v@?@\"<UIViewControllerTransitionCoordinatorContext>\">24@?<v@?@\"<UIViewControllerTransitionCoordinatorContext>\">32"
- "B40@0:8@16@?24@?32"
- "DimmingView"
- "ECAccountPropertyProviding"
- "ECMailAccount"
- "EDAccount"
- "EDReceivingAccount"
- "HIDE_RELATED_MESSAGES"
- "MCSNotJunk"
- "MFNullAnimationTransitionCoordinator"
- "MobileMail.ConversationSenderHeaderViewLayoutInformation"
- "MobileMail/ConversationSenderHeaderViewLayoutInformation.swift"
- "PSHeaderFooterView"
- "SHOW_RELATED_MESSAGES"
- "T@\"<DimmingViewDelegate>\",W,N,V_delegate"
- "T@\"<ECMailbox>\",R,N"
- "T@\"<EFCancelable>\",&,N,V_hasRelatedMessagesCountToken"
- "T@\"ACAccount\",R,N"
- "T@\"UIView\",R,N"
- "T@\"UIView\",W,N,V_containerView"
- "TB,N,V_hasRelatedMessages"
- "TB,R,N,GisAnimated"
- "TB,R,N,GisCancelled"
- "TB,R,N,GisInteractive"
- "Td,N,R,VcontentInset"
- "Td,N,R,VstatusBarHeight"
- "T{CGAffineTransform=dddddd},R,N"
- "UIViewControllerTransitionCoordinator"
- "UIViewControllerTransitionCoordinatorContext"
- "_TtC10MobileMail45ConversationSenderHeaderViewLayoutInformation"
- "_addAlongsideAnimation:"
- "_addAlongsideCompletion:"
- "_alongsideAnimations"
- "_alongsideCompletions"
- "_applyBlocks:"
- "_containerView"
- "_fetchGLPStatsForInternalSuggestionsRow"
- "_focusedMessageAtNormalizedFocalPoint:"
- "_goToAccountURL"
- "_hasRelatedMessages"
- "_hasRelatedMessagesCountToken"
- "_height"
- "_helpURL"
- "_linkButton"
- "_performBlockAfterCATransactionCommits:"
- "_runAlongsideAnimations"
- "_runAlongsideCompletionsAfterCommit"
- "_transitionIsInFlight"
- "_updateHasRelatedMessagesAsync"
- "accountLinkButtonTitle"
- "accountLinkButtonURL"
- "accountLinkButtonViewHeight"
- "accountURL"
- "actuallyVisibleItemIDs"
- "altDSID"
- "animateAlongsideTransitionInView:animation:completion:"
- "animated"
- "canAuthenticateWithCurrentCredentials"
- "canToggleRelatedMessages"
- "completionCurve"
- "completionVelocity"
- "containsMailboxWithURL:"
- "contentRectForBounds:"
- "d32@0:8d16@\"UITableView\"24"
- "d32@0:8d16@24"
- "dimmingViewWasTapped:"
- "drawRect:"
- "fill"
- "hasRelatedMessages"
- "hasRelatedMessagesCountToken"
- "initWithSpecialDestination:"
- "initWithSpecifier:"
- "initWithStatusBarHeight:contentInset:"
- "initialize"
- "initiallyInteractive"
- "interactive"
- "isRotating"
- "mailboxForType:"
- "notifyWhenInteractionEndsUsingBlock:"
- "percentComplete"
- "preferredHeightForWidth:inTableView:"
- "presentationStyle"
- "primaryiCloudAccount"
- "refreshContentsWithSpecifier:"
- "roundedRectBezierPath:withRoundedCorners:withCornerRadius:"
- "setAccountFinderBlock:"
- "setContainerView:"
- "setDeliveryAccount:"
- "setHasRelatedMessages:"
- "setHasRelatedMessagesCountToken:"
- "setTitleColor:forState:"
- "setTitleShadowColor:forState:"
- "setTransparent:"
- "smtpIdentifier"
- "statusBarHeight"
- "systemAccount"
- "targetTransform"
- "titleColorForState:"
- "titleRectForContentRect:"
- "titleShadowColorForState:"
- "transform3D"
- "transitionDuration"
- "transparent"
- "v24@0:8@\"<EDDeliveryAccount>\"16"
- "v24@0:8@\"PSSpecifier\"16"
- "v24@0:8@?<v@?@\"<UIViewControllerTransitionCoordinatorContext>\">16"
- "v64@0:8@16@24@32B40B44@48@?56"
```
