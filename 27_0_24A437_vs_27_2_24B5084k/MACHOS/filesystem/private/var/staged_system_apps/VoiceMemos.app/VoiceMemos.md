## VoiceMemos

> `/private/var/staged_system_apps/VoiceMemos.app/VoiceMemos`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_classname`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift5_protos`
- `__TEXT.__swift5_mpenum`
- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_catlist2`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_intobj`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA.__objc_stublist`

```diff

-1438.1.0.0.0
-  __TEXT.__text: 0x1f9f0c
-  __TEXT.__auth_stubs: 0x5c90
-  __TEXT.__objc_stubs: 0x22500
-  __TEXT.__objc_methlist: 0x11eac
-  __TEXT.__objc_methname: 0x33eb9
-  __TEXT.__cstring: 0xbf69
+1443.0.0.0.0
+  __TEXT.__text: 0x1f9068
+  __TEXT.__auth_stubs: 0x5c50
+  __TEXT.__objc_stubs: 0x225c0
+  __TEXT.__objc_methlist: 0x11f1c
+  __TEXT.__objc_methname: 0x34099
+  __TEXT.__cstring: 0xc039
   __TEXT.__objc_classname: 0x395a
-  __TEXT.__const: 0x15c44
-  __TEXT.__objc_methtype: 0x817a
-  __TEXT.__oslogstring: 0x49d2
-  __TEXT.__gcc_except_tab: 0x1ec8
-  __TEXT.__swift5_typeref: 0xa24c
-  __TEXT.__constg_swiftt: 0xabd8
+  __TEXT.__const: 0x15bc4
+  __TEXT.__objc_methtype: 0x818a
+  __TEXT.__oslogstring: 0x4992
+  __TEXT.__gcc_except_tab: 0x1ee4
+  __TEXT.__swift5_typeref: 0xa228
+  __TEXT.__constg_swiftt: 0xabb8
   __TEXT.__swift5_builtin: 0x474
-  __TEXT.__swift5_reflstr: 0x69a0
-  __TEXT.__swift5_fieldmd: 0x5adc
+  __TEXT.__swift5_reflstr: 0x69d0
+  __TEXT.__swift5_fieldmd: 0x5ae8
   __TEXT.__swift5_assocty: 0xbe8
   __TEXT.__swift5_proto: 0xb48
-  __TEXT.__swift5_types: 0x66c
+  __TEXT.__swift5_types: 0x668
   __TEXT.__swift5_protos: 0xfc
-  __TEXT.__swift5_capture: 0x1c58
-  __TEXT.__swift_as_entry: 0x3ec
-  __TEXT.__swift_as_cont: 0x7f8
-  __TEXT.__swift_as_ret: 0x400
+  __TEXT.__swift5_capture: 0x1c54
+  __TEXT.__swift_as_entry: 0x3e0
+  __TEXT.__swift_as_cont: 0x7e0
+  __TEXT.__swift_as_ret: 0x3f4
   __TEXT.__swift5_mpenum: 0x8c
-  __TEXT.__unwind_info: 0x97d8
-  __TEXT.__eh_frame: 0xaac0
+  __TEXT.__unwind_info: 0x9770
+  __TEXT.__eh_frame: 0xa92c
   __DATA_CONST.__const: 0xd6d0
-  __DATA_CONST.__cfstring: 0x38c0
-  __DATA_CONST.__objc_classlist: 0x8c0
+  __DATA_CONST.__cfstring: 0x39c0
+  __DATA_CONST.__objc_classlist: 0x8b8
   __DATA_CONST.__objc_catlist: 0x88
   __DATA_CONST.__objc_catlist2: 0x8
-  __DATA_CONST.__objc_protolist: 0x4e8
+  __DATA_CONST.__objc_protolist: 0x4f0
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0x1c8
   __DATA_CONST.__objc_superrefs: 0x318

   __DATA_CONST.__objc_arraydata: 0xb0
   __DATA_CONST.__objc_arrayobj: 0xa8
   __DATA_CONST.__objc_doubleobj: 0xa0
-  __DATA_CONST.__auth_got: 0x2e60
-  __DATA_CONST.__got: 0x1d30
-  __DATA_CONST.__auth_ptr: 0x21c0
-  __DATA.__objc_const: 0x1e140
-  __DATA.__objc_selrefs: 0xaac8
-  __DATA.__objc_ivar: 0xe98
-  __DATA.__objc_data: 0xa1d0
-  __DATA.__data: 0xeb10
+  __DATA_CONST.__auth_got: 0x2e40
+  __DATA_CONST.__got: 0x1d18
+  __DATA_CONST.__auth_ptr: 0x21a8
+  __DATA.__objc_const: 0x1e190
+  __DATA.__objc_selrefs: 0xab10
+  __DATA.__objc_ivar: 0xea0
+  __DATA.__objc_data: 0xa348
+  __DATA.__data: 0xe988
   __DATA.__objc_stublist: 0x18
   __DATA.__bss: 0x15650
   __DATA.__common: 0x658

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 13465
-  Symbols:   3024
-  CStrings:  10350
+  Functions: 13458
+  Symbols:   3016
+  CStrings:  10373
 
Symbols:
+ _$s7SwiftUI4ViewP31AppleMediaServicesUIKitInternalE22significantChangeSheet11isPresented010presentingC10Controller07warmingC7Content07waitingcQ00iJ11Description7appIcon0tU10Descriptor7version_QrAA7BindingVySbG_So06UIViewO0CAD011SignificantjcQ0VATSSSo6ISIconCSo07ISImageV0CSgSSys6ResultOySbs5Error_pGctF
+ _$s7SwiftUI4ViewP31AppleMediaServicesUIKitInternalE22significantChangeSheet11isPresented010presentingC10Controller07warmingC7Content07waitingcQ00iJ11Description7appIcon0tU10Descriptor7version_QrAA7BindingVySbG_So06UIViewO0CAD011SignificantjcQ0VATSSSo6ISIconCSo07ISImageV0CSgSSys6ResultOySbs5Error_pGctFQOMQ
+ _OBJC_CLASS_$_OBBulletedListItemLinkButton
- _$s10Foundation23LocalizedStringResourceV17BundleDescriptionO4mainyA2EmFWC
- _$s10Foundation23LocalizedStringResourceV17BundleDescriptionOMa
- _$s10Foundation23LocalizedStringResourceV_5table6locale6bundle7commentACSSAAE17LocalizationValueV_SSSgAA6LocaleVAC17BundleDescriptionOs06StaticC0VSgtcfC
- _$s7SwiftUI4ViewP31AppleMediaServicesUIKitInternalE22significantChangeSheet11isPresented010presentingC10Controller07warmingC7Content07waitingcQ00iJ11Description7appIcon7version_QrAA7BindingVySbG_So06UIViewO0CAD011SignificantjcQ0VASSSAA5ImageVSSys6ResultOySbs5Error_pGctF
- _$s7SwiftUI4ViewP31AppleMediaServicesUIKitInternalE22significantChangeSheet11isPresented010presentingC10Controller07warmingC7Content07waitingcQ00iJ11Description7appIcon7version_QrAA7BindingVySbG_So06UIViewO0CAD011SignificantjcQ0VASSSAA5ImageVSSys6ResultOySbs5Error_pGctFQOMQ
- _$s7SwiftUI5ImageV02uiC0ACSo7UIImageC_tcfC
- _$s7SwiftUI5ImageVMn
- _$s7SwiftUI5ImageVN
- _$sSo8UIButtonC5UIKitE13ConfigurationV015preferredSymbolC8ForImageSo07UIImageeC0CSgvs
- _OBJC_CLASS_$_ISImageDescriptor
- _OBJC_CLASS_$__UIScrollPocketInteraction
CStrings:
+ "%s -- Cannot begin replace / resume yet - recording view is not presented."
+ "%s -- must not prepare for playback while recording."
+ "@\"<RCPlatterTimeViewInteractionDelegate>\""
+ "@\"<RCPlatterTimeViewStateDelegate>\""
+ "RCPlatterTimeViewInteractionDelegate"
+ "RCPlatterTimeViewStateDelegate"
+ "RCWaveformRenderer.mm"
+ "STUDIO_VOICE_SETTING"
+ "STUDIO_VOICE_SLIDER"
+ "STUDIO_VOICE_TITLE"
+ "STUDIO_VOICE_TOGGLE_SETTING"
+ "STUDIO_VOICE_TOGGLE_SETTING_DESCRIPTION"
+ "Studio Voice slider"
+ "T@\"<RCPlatterTimeViewInteractionDelegate>\",W,N,V_interactionDelegate"
+ "T@\"<RCPlatterTimeViewStateDelegate>\",W,N,V_stateDelegate"
+ "T@\"NSHashTable\",&,N,V__sizeClassObservers"
+ "T@\"RCRecordingViewAppStateUIModel\",&,N,V_appStateUIModel"
+ "TB,N,VforcePauseUpdates"
+ "VoiceMemos.RecordingViewLayoutPresenter"
+ "WELCOME_ITEM_1_DESCRIPTION"
+ "WELCOME_ITEM_1_LEARN_MORE_LINK"
+ "WELCOME_ITEM_1_TITLE"
+ "WELCOME_ITEM_2_DESCRIPTION"
+ "WELCOME_ITEM_2_TITLE"
+ "WELCOME_ITEM_3_DESCRIPTION"
+ "WELCOME_ITEM_3_TITLE"
+ "["
+ "[%@] Tried to draw on non-main thread."
+ "[%@] Tried to render on non-main thread."
+ "__sizeClassObservers"
+ "_appStateUIModel"
+ "_interactionDelegate"
+ "_playheadFractionOfBoundsWidth"
+ "_playheadInsetForBoundsWidth:"
+ "_playheadTimeForBeginTime:contentVisibleDuration:"
+ "_sizeClassObservers"
+ "_stateDelegate"
+ "_viewProperties"
+ "addBulletedListItemWithTitle:description:image:linkButton:"
+ "addSizeClassObserver:"
+ "forcePauseUpdates"
+ "invalidateDetentsIfNeeded"
+ "linkButton"
+ "person.spatialaudio.fill"
+ "removeSizeClassObserver:"
+ "setContentInsetAdjustmentBehavior:"
+ "setForcePauseUpdates:"
+ "setInteractionDelegate:"
+ "setStateDelegate:"
+ "setStudioVoiceEnabled:ofRecording:"
+ "set_sizeClassObservers:"
+ "stateDelegate"
+ "viewSafeAreaInsetsDidChange"
- "%s -- cannot set studio voice enabled for UUID %@ that does not match audio player UUID %@"
- "-[RCMainControllerHelper setStudioVoiceEnabled:forUUID:]"
- "@\"<RCPlatterTimeViewDelegate>\""
- "Failed to render the app icon for the image descriptor: %@. Falling back to Welcome_AppIcon"
- "K"
- "Localizable-Studio"
- "RCPlatterTimeViewDelegate"
- "STUDIO_SOUND_SETTING"
- "STUDIO_SOUND_SLIDER"
- "STUDIO_SOUND_TITLE"
- "STUDIO_SOUND_TOGGLE_SETTING"
- "STUDIO_SOUND_TOGGLE_SETTING_DESCRIPTION"
- "Studio Sound slider"
- "T@\"<RCPlatterTimeViewDelegate>\",W,N,V_delegate"
- "T@\"NSHashTable\",&,N,V__observers"
- "WELCOME_ITEM_DESCRIPTION"
- "WELCOME_ITEM_TITLE"
- "_TtC10VoiceMemos21RCToggleButtonFactory"
- "__observers"
- "_views"
- "createPlaybackSettingsButton"
- "createTranscriptionButton"
- "didUpdateRecordingViewDisplayStyle"
- "displayScale"
- "getCGImageForImageDescriptor:completion:"
- "initWithCGImage:scale:orientation:"
- "initWithScrollView:edge:style:"
- "initWithSize:scale:"
- "set_observers:"
- "v16@?0^{CGImage=}8"
```
