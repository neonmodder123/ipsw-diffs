## BookEPUB

> `/private/var/staged_system_apps/Books.app/Frameworks/BookEPUB.framework/BookEPUB`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift5_mpenum`
- `__TEXT.__swift5_protos`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift_as_ret`
- `__TEXT.__swift_as_cont`
- `__DATA_CONST.__cfstring`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_dictobj`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA_CONST.__got`

```diff

-6655.0.0.0.0
-  __TEXT.__text: 0x27fd44
-  __TEXT.__auth_stubs: 0x3e30
-  __TEXT.__objc_stubs: 0xa960
+6713.0.0.0.0
+  __TEXT.__text: 0x281fe0
+  __TEXT.__auth_stubs: 0x3e60
+  __TEXT.__objc_stubs: 0xaa60
   __TEXT.__objc_methlist: 0x58a8
-  __TEXT.__cstring: 0x9081
-  __TEXT.__objc_classname: 0x1dde
-  __TEXT.__objc_methname: 0x14578
-  __TEXT.__objc_methtype: 0x44d4
-  __TEXT.__const: 0x1e1b0
-  __TEXT.__gcc_except_tab: 0x1fb0
-  __TEXT.__oslogstring: 0xc873
+  __TEXT.__cstring: 0x9191
+  __TEXT.__objc_classname: 0x1dfe
+  __TEXT.__objc_methname: 0x145e8
+  __TEXT.__objc_methtype: 0x44e4
+  __TEXT.__const: 0x1e180
+  __TEXT.__gcc_except_tab: 0x1fe4
+  __TEXT.__oslogstring: 0xc963
   __TEXT.__ustring: 0x322be
-  __TEXT.__swift5_typeref: 0x66c0
-  __TEXT.__constg_swiftt: 0x9524
-  __TEXT.__swift5_reflstr: 0x6acd
-  __TEXT.__swift5_fieldmd: 0x5d6c
+  __TEXT.__swift5_typeref: 0x668e
+  __TEXT.__constg_swiftt: 0x952c
+  __TEXT.__swift5_reflstr: 0x6a7d
+  __TEXT.__swift5_fieldmd: 0x5d68
   __TEXT.__swift5_builtin: 0x230
   __TEXT.__swift5_assocty: 0x770
   __TEXT.__swift5_proto: 0x7d8
-  __TEXT.__swift5_types: 0x4c4
+  __TEXT.__swift5_types: 0x4cc
   __TEXT.__swift5_mpenum: 0x38
   __TEXT.__swift5_protos: 0x16c
-  __TEXT.__swift5_capture: 0x2380
+  __TEXT.__swift5_capture: 0x24fc
   __TEXT.__swift_as_entry: 0x110
   __TEXT.__swift_as_ret: 0x110
   __TEXT.__swift_as_cont: 0x258
-  __TEXT.__unwind_info: 0x7010
-  __TEXT.__eh_frame: 0x537c
-  __DATA_CONST.__const: 0x15750
+  __TEXT.__unwind_info: 0x7038
+  __TEXT.__eh_frame: 0x534c
+  __DATA_CONST.__const: 0x15c80
   __DATA_CONST.__cfstring: 0x5660
-  __DATA_CONST.__objc_classlist: 0x528
+  __DATA_CONST.__objc_classlist: 0x530
   __DATA_CONST.__objc_catlist: 0x80
   __DATA_CONST.__objc_protolist: 0x218
   __DATA_CONST.__objc_imageinfo: 0x8

   __DATA_CONST.__objc_arraydata: 0x88
   __DATA_CONST.__objc_dictobj: 0x50
   __DATA_CONST.__objc_arrayobj: 0x18
-  __DATA_CONST.__auth_got: 0x1f30
+  __DATA_CONST.__auth_got: 0x1f48
   __DATA_CONST.__got: 0xdf8
-  __DATA_CONST.__auth_ptr: 0x12f0
-  __DATA.__objc_const: 0x10d08
-  __DATA.__objc_selrefs: 0x4278
+  __DATA_CONST.__auth_ptr: 0x12f8
+  __DATA.__objc_const: 0x10d48
+  __DATA.__objc_selrefs: 0x42c0
   __DATA.__objc_ivar: 0x2ac
-  __DATA.__objc_data: 0x5680
-  __DATA.__data: 0xcde8
-  __DATA.__bss: 0xe020
+  __DATA.__objc_data: 0x5618
+  __DATA.__data: 0xce78
+  __DATA.__bss: 0xe010
   __DATA.__common: 0xe38
   - /System/Library/Frameworks/CFNetwork.framework/CFNetwork
   - /System/Library/Frameworks/Combine.framework/Combine

   - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 11362
+  Functions: 11410
   Symbols:   1473
-  CStrings:  5823
+  CStrings:  5840
 
CStrings:
+ "%@ pages left in chapter"
+ "1 page left in chapter"
+ "@\"UIView\"20@0:8B16"
+ "Double tap to show undo page skip menu"
+ "Last page in chapter"
+ "Skipping #fontReg of #fontFamily '%{public}@' because webView:%{public}@ requires a reload"
+ "Timed out after %.0fs waiting for #fontReg of #fontFamily '%{public}@' on webView:%{public}@ -- failing it so deferred registrations can drain"
+ "_TtC8BookEPUB15ChapterProgress"
+ "_hostViewForSelectionHandleWithTrailingHandle:"
+ "_selectedTextRangeWillChangeToRange:"
+ "_selectionIsFromActivationBehavior"
+ "arrow.uturn.backward"
+ "arrow.uturn.forward"
+ "bottomEdgeEffect"
+ "constant"
+ "initWithUnsignedInteger:"
+ "insertSubview:aboveSubview:"
+ "interactionShouldReceiveTouchesInDescendantViews:"
+ "kREIPageLabelSettledOffset"
+ "kREIScrubberToPageLabelSpacing"
+ "performWithoutAnimation:"
+ "screen"
+ "setAccessibilityHint:"
+ "setAccessibilityValue:"
+ "setMaximumFractionDigits:"
+ "setMenu:"
+ "setPrimaryAction:"
- "TQ,N,R"
- "installedAnchor"
- "isActive"
- "navigationController"
- "pageLabelExternalConstraint"
- "preferredScreenEdgesDeferringSystemGestures"
- "toolbar"
- "toolbarSuperviewObservation"
- "viewIsAppearing:"
- "viewWillDisappear:"
```
