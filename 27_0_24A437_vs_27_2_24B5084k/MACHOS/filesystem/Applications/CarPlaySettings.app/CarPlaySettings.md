## CarPlaySettings

> `/Applications/CarPlaySettings.app/CarPlaySettings`

### Sections with Same Size but Changed Content

- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_dictobj`

```diff

-581.7.2.0.0
-  __TEXT.__text: 0xd9714
-  __TEXT.__auth_stubs: 0x3760
-  __TEXT.__objc_stubs: 0x9b60
-  __TEXT.__objc_methlist: 0x61e4
-  __TEXT.__const: 0x7034
-  __TEXT.__objc_methname: 0x130b5
-  __TEXT.__oslogstring: 0x3841
-  __TEXT.__cstring: 0x4164
-  __TEXT.__objc_classname: 0x155f
-  __TEXT.__objc_methtype: 0x6012
-  __TEXT.__gcc_except_tab: 0xc24
+591.2.0.0.0
+  __TEXT.__text: 0xd456c
+  __TEXT.__auth_stubs: 0x3980
+  __TEXT.__objc_stubs: 0x9be0
+  __TEXT.__objc_methlist: 0x61bc
+  __TEXT.__const: 0x6f54
+  __TEXT.__objc_methname: 0x13025
+  __TEXT.__oslogstring: 0x3d61
+  __TEXT.__cstring: 0x4434
+  __TEXT.__objc_classname: 0x151f
+  __TEXT.__objc_methtype: 0x5f92
+  __TEXT.__gcc_except_tab: 0xc04
   __TEXT.__ustring: 0x40
-  __TEXT.__swift5_typeref: 0xfb48
-  __TEXT.__swift5_capture: 0x15e0
-  __TEXT.__constg_swiftt: 0x2954
-  __TEXT.__swift5_reflstr: 0x1cc3
-  __TEXT.__swift5_assocty: 0x790
-  __TEXT.__swift5_fieldmd: 0x19f0
-  __TEXT.__swift5_proto: 0x1f8
-  __TEXT.__swift5_types: 0x204
-  __TEXT.__swift_as_entry: 0x118
-  __TEXT.__swift_as_ret: 0xe8
-  __TEXT.__swift_as_cont: 0x14c
-  __TEXT.__swift5_protos: 0x10
-  __TEXT.__swift5_builtin: 0xc8
-  __TEXT.__unwind_info: 0x3628
-  __TEXT.__eh_frame: 0x27c8
-  __DATA_CONST.__const: 0x5cf8
-  __DATA_CONST.__cfstring: 0x3520
-  __DATA_CONST.__objc_classlist: 0x3e0
+  __TEXT.__swift5_typeref: 0xd080
+  __TEXT.__swift5_capture: 0x1488
+  __TEXT.__constg_swiftt: 0x2908
+  __TEXT.__swift5_reflstr: 0x1c63
+  __TEXT.__swift5_assocty: 0x7c0
+  __TEXT.__swift5_fieldmd: 0x1a60
+  __TEXT.__swift5_proto: 0x210
+  __TEXT.__swift5_types: 0x220
+  __TEXT.__swift_as_entry: 0xe0
+  __TEXT.__swift_as_ret: 0x9c
+  __TEXT.__swift_as_cont: 0xf8
+  __TEXT.__swift5_protos: 0x8
+  __TEXT.__swift5_builtin: 0xdc
+  __TEXT.__swift5_mpenum: 0x8
+  __TEXT.__unwind_info: 0x3510
+  __TEXT.__eh_frame: 0x2408
+  __DATA_CONST.__const: 0x5d28
+  __DATA_CONST.__cfstring: 0x4140
+  __DATA_CONST.__objc_classlist: 0x3d8
   __DATA_CONST.__objc_catlist: 0x10
   __DATA_CONST.__objc_protolist: 0x248
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0xf0
-  __DATA_CONST.__objc_superrefs: 0x268
-  __DATA_CONST.__objc_arraydata: 0x420
+  __DATA_CONST.__objc_superrefs: 0x260
+  __DATA_CONST.__objc_arraydata: 0x8b0
   __DATA_CONST.__objc_dictobj: 0x28
-  __DATA_CONST.__auth_got: 0x1bc0
-  __DATA_CONST.__got: 0xf98
-  __DATA_CONST.__auth_ptr: 0xd70
-  __DATA.__objc_const: 0x14d48
-  __DATA.__objc_selrefs: 0x3f40
-  __DATA.__objc_ivar: 0x480
-  __DATA.__objc_data: 0x3958
-  __DATA.__data: 0x5098
-  __DATA.__bss: 0x4948
-  __DATA.__common: 0x140
+  __DATA_CONST.__auth_got: 0x1cd0
+  __DATA_CONST.__got: 0xf78
+  __DATA_CONST.__auth_ptr: 0xe78
+  __DATA.__objc_const: 0x14d00
+  __DATA.__objc_selrefs: 0x3f50
+  __DATA.__objc_ivar: 0x474
+  __DATA.__objc_data: 0x39e8
+  __DATA.__data: 0x4cb8
+  __DATA.__bss: 0x4c38
+  __DATA.__common: 0x138
   - /System/Library/Frameworks/Combine.framework/Combine
   - /System/Library/Frameworks/CoreFoundation.framework/CoreFoundation
   - /System/Library/Frameworks/CoreGraphics.framework/CoreGraphics

   - /System/Library/PrivateFrameworks/AccessibilityUIUtilities.framework/AccessibilityUIUtilities
   - /System/Library/PrivateFrameworks/AssistantServices.framework/AssistantServices
   - /System/Library/PrivateFrameworks/BaseBoard.framework/BaseBoard
-  - /System/Library/PrivateFrameworks/CAFCombine.framework/CAFCombine
   - /System/Library/PrivateFrameworks/CAFUI.framework/CAFUI
   - /System/Library/PrivateFrameworks/CarAccessoryFramework.framework/CarAccessoryFramework
   - /System/Library/PrivateFrameworks/CarAssetUtils.framework/CarAssetUtils
   - /System/Library/PrivateFrameworks/CarKit.framework/CarKit
+  - /System/Library/PrivateFrameworks/CarPlayAsset.framework/CarPlayAsset
   - /System/Library/PrivateFrameworks/CarPlayAssetUI.framework/CarPlayAssetUI
   - /System/Library/PrivateFrameworks/CarPlayServices.framework/CarPlayServices
   - /System/Library/PrivateFrameworks/CarPlayUI.framework/CarPlayUI

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 4896
-  Symbols:   1691
-  CStrings:  4220
+  Functions: 4818
+  Symbols:   1752
+  CStrings:  4342
 
Symbols:
+ _$s13CarAssetUtils15CAUAssetLibraryC20featureConfigurationAA010CAUFeatureG0VyFTj
+ _$s13CarAssetUtils16ThemePickerStyleO7galleryyAcA20GalleryConfigurationVcACmFWC
+ _$s13CarAssetUtils16ThemePickerStyleO8standardyAcA21StandardConfigurationVcACmFWC
+ _$s13CarAssetUtils16ThemePickerStyleOMa
+ _$s13CarAssetUtils16ThemePickerStyleOMn
+ _$s13CarAssetUtils18ThemeConfigurationV16themePickerStyleAA0dgH0OSgvg
+ _$s13CarAssetUtils18ThemeConfigurationVMa
+ _$s13CarAssetUtils18ThemeConfigurationVMn
+ _$s13CarAssetUtils26CAUSettingsUIConfigurationV18themeConfigurationAA05ThemeG0VSgvg
+ _$s14AnimatableData7SwiftUI0A0PTl
+ _$s5Cache7SwiftUI6LayoutPTl
+ _$s5UIKit25UIBackgroundConfigurationV5clearACyFZ
+ _$s5UIKit25UIBackgroundConfigurationVMa
+ _$s7SwiftUI10AnimatableMp
+ _$s7SwiftUI10AnimatableP05_makeC05value6inputsyAA11_GraphValueVyxGz_AA01_G6InputsVtFZTq
+ _$s7SwiftUI10AnimatableP0C4DataAC_AA16VectorArithmeticTn
+ _$s7SwiftUI10AnimatableP14animatableData0cE0QzvMTq
+ _$s7SwiftUI10AnimatableP14animatableData0cE0QzvgTq
+ _$s7SwiftUI10AnimatableP14animatableData0cE0QzvsTq
+ _$s7SwiftUI10AnimatablePA2A05EmptyC4DataV0cE0RtzrlE010animatableE0AEvM
+ _$s7SwiftUI10AnimatablePA2A05EmptyC4DataV0cE0RtzrlE05_makeC05value6inputsyAA11_GraphValueVyxGz_AA01_I6InputsVtFZ
+ _$s7SwiftUI10ScrollViewVMa
+ _$s7SwiftUI11_LayoutRootVMn
+ _$s7SwiftUI13LayoutSubviewV12sizeThatFitsySo6CGSizeVAA16ProposedViewSizeVF
+ _$s7SwiftUI13LayoutSubviewV5place2at6anchor8proposalySo7CGPointV_AA9UnitPointVAA16ProposedViewSizeVtF
+ _$s7SwiftUI13LayoutSubviewVMa
+ _$s7SwiftUI13LayoutSubviewVMn
+ _$s7SwiftUI14LayoutSubviewsVMa
+ _$s7SwiftUI14LayoutSubviewsVMn
+ _$s7SwiftUI14LayoutSubviewsVSTAAMc
+ _$s7SwiftUI14LayoutSubviewsVSlAAMc
+ _$s7SwiftUI15ForegroundStyleVMn
+ _$s7SwiftUI16ProposedViewSizeV11unspecifiedACvgZ
+ _$s7SwiftUI16RoundedRectangleV4path2inAA4PathVSo6CGRectV_tF
+ _$s7SwiftUI16ScrollViewReaderV7contentACyxGxAA0cD5ProxyVc_tcfC
+ _$s7SwiftUI16ScrollViewReaderVMa
+ _$s7SwiftUI16ScrollViewReaderVyxGAA0D0AAMc
+ _$s7SwiftUI19EmptyAnimatableDataVAA16VectorArithmeticAAMc
+ _$s7SwiftUI19EmptyAnimatableDataVMn
+ _$s7SwiftUI19EmptyAnimatableDataVN
+ _$s7SwiftUI4ViewPAAE2idyQrqd__SHRd__lF
+ _$s7SwiftUI5ShapeP12sizeThatFitsySo6CGSizeVAA16ProposedViewSizeVFTq
+ _$s7SwiftUI5ShapeP23layoutDirectionBehaviorAA06LayouteF0OvgTq
+ _$s7SwiftUI5ShapeP4path2inAA4PathVSo6CGRectV_tFTq
+ _$s7SwiftUI5ShapeP4roleAA0C4RoleOvgZTq
+ _$s7SwiftUI5ShapePAA10AnimatableTb
+ _$s7SwiftUI5ShapePAA4ViewTb
+ _$s7SwiftUI5ShapePAAE12sizeThatFitsySo6CGSizeVAA16ProposedViewSizeVF
+ _$s7SwiftUI5ShapePAAE13_makeViewList4view6inputsAA01_eF7OutputsVAA11_GraphValueVyxG_AA01_eF6InputsVtFZ
+ _$s7SwiftUI5ShapePAAE23layoutDirectionBehaviorAA06LayouteF0Ovg
+ _$s7SwiftUI5ShapePAAE4bodyAA01_C4ViewVyxAA15ForegroundStyleVGvg
+ _$s7SwiftUI5ShapePAAE4roleAA0C4RoleOvgZ
+ _$s7SwiftUI5ShapePAAE9_makeView4view6inputsAA01_E7OutputsVAA11_GraphValueVyxG_AA01_E6InputsVtFZ
+ _$s7SwiftUI5StateV12wrappedValueACyxGx_tcfC
+ _$s7SwiftUI5StateV14projectedValueAA7BindingVyxGvg
+ _$s7SwiftUI6HStackV9alignment7spacing7contentACyxGAA17VerticalAlignmentV_12CoreGraphics7CGFloatVSgxyXEtcfC
+ _$s7SwiftUI6HStackVMa
+ _$s7SwiftUI6IDViewVMa
+ _$s7SwiftUI6IDViewVyxq_GAA4ViewAAMc
+ _$s7SwiftUI6LayoutMp
+ _$s7SwiftUI6LayoutP05_makeC4View4root6inputs4bodyAA01_E7OutputsVAA11_GraphValueVyxG_AA01_E6InputsVAA01_e4ListI0VAA01_J0V_ANtXEtFZTq
+ _$s7SwiftUI6LayoutP11updateCache_8subviewsy0E0Qzz_AA0C8SubviewsVtFTq
+ _$s7SwiftUI6LayoutP12sizeThatFits8proposal8subviews5cacheSo6CGSizeVAA16ProposedViewSizeV_AA0C8SubviewsV5CacheQzztFTq
+ _$s7SwiftUI6LayoutP13placeSubviews2in8proposal8subviews5cacheySo6CGRectV_AA16ProposedViewSizeVAA0cE0V5CacheQzztFTq
+ _$s7SwiftUI6LayoutP16layoutPropertiesAA0cE0VvgZTq
+ _$s7SwiftUI6LayoutP17explicitAlignment2of2in8proposal8subviews5cache12CoreGraphics7CGFloatVSgAA010HorizontalE0V_So6CGRectVAA16ProposedViewSizeVAA0C8SubviewsV5CacheQzztFTq
+ _$s7SwiftUI6LayoutP17explicitAlignment2of2in8proposal8subviews5cache12CoreGraphics7CGFloatVSgAA08VerticalE0V_So6CGRectVAA16ProposedViewSizeVAA0C8SubviewsV5CacheQzztFTq
+ _$s7SwiftUI6LayoutP7spacing8subviews5cacheAA11ViewSpacingVAA0C8SubviewsV_5CacheQzztFTq
+ _$s7SwiftUI6LayoutP9makeCache8subviews0E0QzAA0C8SubviewsV_tFTq
+ _$s7SwiftUI6LayoutPAA10AnimatableTb
+ _$s7SwiftUI6LayoutPAAE05_makeC4View4root6inputs4bodyAA01_E7OutputsVAA11_GraphValueVyxG_AA01_E6InputsVAA01_e4ListI0VAA01_J0V_ANtXEtFZ
+ _$s7SwiftUI6LayoutPAAE11updateCache_8subviewsy0E0Qzz_AA0C8SubviewsVtF
+ _$s7SwiftUI6LayoutPAAE16layoutPropertiesAA0cE0VvgZ
+ _$s7SwiftUI6LayoutPAAE17explicitAlignment2of2in8proposal8subviews5cache12CoreGraphics7CGFloatVSgAA010HorizontalE0V_So6CGRectVAA16ProposedViewSizeVAA0C8SubviewsV5CacheQzztF
+ _$s7SwiftUI6LayoutPAAE17explicitAlignment2of2in8proposal8subviews5cache12CoreGraphics7CGFloatVSgAA08VerticalE0V_So6CGRectVAA16ProposedViewSizeVAA0C8SubviewsV5CacheQzztF
+ _$s7SwiftUI6LayoutPAAE7spacing8subviews5cacheAA11ViewSpacingVAA0C8SubviewsV_5CacheQzztF
+ _$s7SwiftUI6LayoutPAAyt5CacheRtzrlE04makeD08subviewsyAA0C8SubviewsV_tF
+ _$s7SwiftUI9UnitPointV3topACvgZ
+ _$s8AllCasess12CaseIterablePTl
+ _$sSayxSicig
+ _$sSh15minimumCapacityShyxGSi_tcfC
+ _$sSksSx5IndexRpzSnyABG7IndicesRtzSiAA_6StrideRTzrlE7indicesACvg
+ _$sSl10startIndex0B0QzvgTj
+ _$sSl5countSivgTj
+ _$sSl8distance4from2toSi5IndexQz_AEtFTj
+ _$sSl8endIndex0B0QzvgTj
+ _$sSl9formIndex5aftery0B0Qzz_tFTj
+ _$sSlsE10firstIndex5where0B0QzSgSb7ElementQzKXE_tKF
+ _$sSly7ElementQz5IndexQzcirTj
+ _$sSo6UIMenuC5UIKitE5title8subtitle5image24preferredImageVisibility10identifier7options0F11ElementSize8childrenABSS_SSSgSo7UIImageCSgSo0akgH0VSo0A10IdentifieraSgSo0A7OptionsVSo0akL0VSaySo0aK0CGtcfC
+ _$sSo8UIButtonC5UIKitE13ConfigurationV10backgroundAC012UIBackgroundC0Vvs
+ _$sSo8UIButtonC5UIKitE13ConfigurationV12imagePadding12CoreGraphics7CGFloatVvs
+ _$sSo8UIButtonC5UIKitE13ConfigurationV13contentInsetsSo017NSDirectionalEdgeE0Vvs
+ _$sSo8UIButtonC5UIKitE13ConfigurationV14imagePlacementSo21NSDirectionalRectEdgeVvs
+ _$sSo8UIButtonC5UIKitE13ConfigurationV5imageSo7UIImageCSgvs
+ _$sSo8UIButtonC5UIKitE13ConfigurationV5plainAEyFZ
+ _$sSo8UIButtonC5UIKitE13ConfigurationV5titleSSSgvs
+ _$sSo8UIButtonC5UIKitE13ConfigurationVMa
+ _$sSo8UIButtonC5UIKitE13configuration13primaryActionA2bCE13ConfigurationV_So8UIActionCSgtcfC
+ _$sSq12CarPlayAssets23CustomStringConvertibleRzlE11descriptionSSvg
+ _$ss11_StringGutsV4growyySiF
+ _$ss12CaseIterableMp
+ _$ss12CaseIterableP8AllCasesAB_SlTn
+ _$ss12CaseIterableP8allCases03AllD0QzvgZTq
+ _$ss12IdentifiableP2id2IDQzvgTj
+ _$ss16IndexingIteratorVMn
+ _$ss18EnumeratedSequenceV8IteratorVMn
+ _OBJC_CLASS_$_UIImageSymbolConfiguration
+ _OBJC_CLASS_$_UIMenu
- _$s10Foundation4UUIDVACycfC
- _$s13CarAssetUtils15CAUAssetLibraryC10CAFCombineE20featureConfigurationAA010CAUFeatureH0VyF
- _$s13CarAssetUtils26CAUSettingsUIConfigurationV20galleryConfigurationAA07GalleryG0VSgvg
- _$s13CarAssetUtils26CAUSettingsUIConfigurationV21standardConfigurationAA08StandardG0VSgvg
- _$s13CarAssetUtils26CAUSettingsUIConfigurationVs23CustomStringConvertibleAAMc
- _$s25ObjectWillChangePublisher7Combine010ObservableA0PTl
- _$s7Combine16ObservableObjectMp
- _$s7Combine16ObservableObjectP0C19WillChangePublisherAC_AA0F0Tn
- _$s7Combine16ObservableObjectP16objectWillChange0ceF9PublisherQzvgTq
- _$s7Combine16ObservableObjectP7SwiftUIE16environmentStores15WritableKeyPathCyAD17EnvironmentValuesVxSgGvgZ
- _$s7Combine16ObservableObjectPA2A0bC9PublisherC0c10WillChangeD0RtzrlE06objecteF0AEvg
- _$s7Combine25ObservableObjectPublisherCAA0D0AAWP
- _$s7Combine25ObservableObjectPublisherCMn
- _$s7Combine9PublishedV12initialValueACyxGx_tcfC
- _$s7Combine9PublishedV14projectedValueAC9PublisherVyx_Gvg
- _$s7Combine9PublishedV14projectedValueAC9PublisherVyx_Gvs
- _$s7Combine9PublishedV18_enclosingInstance7wrapped7storagexqd___s24ReferenceWritableKeyPathCyqd__xGAHyqd__ACyxGGtcRld__CluiMZ
- _$s7Combine9PublishedV18_enclosingInstance7wrapped7storagexqd___s24ReferenceWritableKeyPathCyqd__xGAHyqd__ACyxGGtcRld__CluigZ
- _$s7Combine9PublishedV18_enclosingInstance7wrapped7storagexqd___s24ReferenceWritableKeyPathCyqd__xGAHyqd__ACyxGGtcRld__CluisZ
- _$s7Combine9PublishedVMa
- _$s7Combine9PublishedVMn
- _$s7SwiftUI13DismissActionVMa
- _$s7SwiftUI13DismissActionVMn
- _$s7SwiftUI17EnvironmentObjectV14projectedValueAC7WrapperVyx_Gvg
- _$s7SwiftUI17EnvironmentObjectV5errors5NeverOyF
- _$s7SwiftUI17EnvironmentObjectVACyxGycfC
- _$s7SwiftUI17EnvironmentObjectVMa
- _$s7SwiftUI17EnvironmentObjectVMn
- _$s7SwiftUI17EnvironmentValuesV7dismissAA13DismissActionVvg
- _$s7SwiftUI17EnvironmentValuesV7dismissAA13DismissActionVvpMV
- _$s7SwiftUI17SwitchToggleStyleVAA0dE0AAMc
- _$s7SwiftUI17SwitchToggleStyleVACycfC
- _$s7SwiftUI17SwitchToggleStyleVMa
- _$s7SwiftUI17SwitchToggleStyleVMn
- _$s7SwiftUI23_GeometryActionModifierVyxGAA04ViewE0AAMc
- _$s7SwiftUI4ViewPAAE11toggleStyleyQrqd__AA06ToggleE0Rd__lF
- _$s7SwiftUI4ViewPAAE11toggleStyleyQrqd__AA06ToggleE0Rd__lFQOMQ
- _$s7SwiftUI5ColorV_3red5green4blue7opacityA2C13RGBColorSpaceO_S4dtcfC
- _$s7SwiftUI6ToggleV4isOn5labelACyxGAA7BindingVySbG_xyXEtcfC
- _$s7SwiftUI6ToggleVMn
- _$s7SwiftUI6ToggleVyxGAA4ViewAAMc
- _$sScC6resume8throwingyq_n_tF
- _$sSo17OS_dispatch_queueC7Combine9Scheduler8DispatchMc
- _$sSo17OS_dispatch_queueC8DispatchE16SchedulerOptionsVMa
- _$sSo17OS_dispatch_queueC8DispatchE16SchedulerOptionsVMn
- _$sSq14CarPlayAssetUIs23CustomStringConvertibleRzlE11descriptionSSvg
- _$ss18EnumeratedSequenceVyxGSksSkRzrlMc
- _swift_unknownObjectWeakAssign
CStrings:
+ " (no theme data for display "
+ "$__lazy_storage_$_ambientLightSyncSpecifier"
+ "$__lazy_storage_$_model"
+ "$__lazy_storage_$_resetSpecifier"
+ "%{public}s theme for drive mode %{public}s"
+ "5073"
+ "50730"
+ "507300"
+ "5074"
+ "50740"
+ "507400"
+ "5075"
+ "50750"
+ "507500"
+ "5076"
+ "50760"
+ "507600"
+ "50761"
+ "507610"
+ "5076100"
+ "5077"
+ "50770"
+ "507700"
+ "5078"
+ "50780"
+ "507800"
+ "5080"
+ "50800"
+ "508000"
+ "5090"
+ "50900"
+ "509000"
+ "50901"
+ "509010"
+ "5090100"
+ "5091"
+ "50910"
+ "509100"
+ "5091000"
+ "50911"
+ "509110"
+ "5091100"
+ "50912"
+ "509120"
+ "5091200"
+ "50913"
+ "509130"
+ "5091300"
+ "50914"
+ "509140"
+ "5091400"
+ "50915"
+ "509150"
+ "5091500"
+ "5092"
+ "50920"
+ "509200"
+ "5093"
+ "50930"
+ "509300"
+ "5094"
+ "50940"
+ "509400"
+ "5095"
+ "50950"
+ "509500"
+ "5097"
+ "50970"
+ "509700"
+ "5099"
+ "50990"
+ "509900"
+ "5100"
+ "51000"
+ "510000"
+ "51001"
+ "510010"
+ "5100100"
+ "ACCESSIBILITY_DRIVE_MODE_THEME_STYLE"
+ "ACCESSIBILITY_MORE_OPTIONS"
+ "AMBIENT_LIGHTING"
+ "AMBIENT_LIGHT_SYNC_FOOTER"
+ "AMBIENT_LIGHT_SYNC_WITH_THEME"
+ "Active drive mode %{public}s (display name %{public}s)"
+ "Added drive mode style button to navigation bar - automatic: %{bool,public}d"
+ "Added more options button to navigation bar"
+ "Ambient light sync %{public}s"
+ "Ambient light sync section footer"
+ "Ambient light sync switch title"
+ "Ambient lighting section title"
+ "CARThemePanel viewDidLoad"
+ "CARThemeSelectorModel"
+ "CARThemeSettingsPanel"
+ "CarPlaySettings.CARThemeSettingsPanel"
+ "CarPlaySettings/CARThemePanel.swift"
+ "CarPlaySettings/CARThemeSettingsPanel.swift"
+ "Cleared the current drive mode theme configuration"
+ "Clearing the theme configuration for drive mode %{public}s"
+ "Cluster layout %{public}s not found; falling back to %{public}s"
+ "Console layout %{public}s not found; falling back to %{public}s"
+ "Creating gallery selector with configuration: %{public}s"
+ "Creating standard selector with configuration: %{public}s"
+ "DRIVE_MODE_THEME_AUTOMATIC"
+ "DRIVE_MODE_THEME_AUTOMATIC_SUBTITLE"
+ "DRIVE_MODE_THEME_CUSTOM"
+ "DRIVE_MODE_THEME_CUSTOM_SUBTITLE"
+ "Display has no layouts - cluster: %{public}s (%{public}ld layouts), console: %{public}s (%{public}ld layouts)"
+ "Drive mode linked state: %{public}s"
+ "Drive mode theme style button accessibility label"
+ "DriveModeThemeStyleButton"
+ "Drop 10"
+ "Drop 10 (Pre-release)"
+ "Drop 7.3"
+ "Drop 7.4"
+ "Drop 7.4 +1"
+ "Drop 7.4 +1+AirPlay"
+ "Drop 7.4 +1-2"
+ "Drop 7.4 +1.1+AirPlay"
+ "Drop 7.5"
+ "Drop 7.6"
+ "Drop 8"
+ "Drop 9"
+ "Drop 9.0.1"
+ "Drop 9.1"
+ "Drop 9.10"
+ "Drop 9.11"
+ "Drop 9.12"
+ "Drop 9.13"
+ "Drop 9.14"
+ "Drop 9.15"
+ "Drop 9.2"
+ "Drop 9.3"
+ "Drop 9.4"
+ "Drop 9.5 or 9.6"
+ "Drop 9.7 or 9.8"
+ "Drop 9.9"
+ "Failed to clear the current drive mode theme configuration: %{public}s"
+ "Failed to read the active drive mode, defaulting to %{public}s: %{public}s"
+ "Failed to read the linked state, defaulting to linked: %{public}s"
+ "Failed to set linked state: %{public}s"
+ "Menu item subtitle: the theme follows the active drive mode"
+ "Menu item subtitle: the theme stays as the user set it"
+ "Menu item: the theme follows the active drive mode"
+ "Menu item: the theme stays as the user set it"
+ "More options button accessibility label"
+ "No car session; cannot build the theme selector"
+ "No console wallpaper matching %{public}s; using the console layout's first, %{public}s"
+ "No palette %{public}s on layout %{public}s; using the layout default"
+ "No panel controller or theme data; cannot push theme settings"
+ "No theme data for console display %{public}s; falling back to layout %{public}s"
+ "No themePickerStyle configured; creating the modal selector"
+ "No wallpaper %{public}s on layout %{public}s; using the layout default"
+ "Pushing theme settings panel"
+ "RESET_CURRENT_THEME"
+ "RESET_CURRENT_THEME_FOOTER"
+ "Reset current theme button title"
+ "Reset current theme section footer"
+ "ResetCurrentThemeButton"
+ "Set %ld right bar button items"
+ "Theme settings navigation title"
+ "ThemeMoreOptionsButton"
+ "Unknown themePickerStyle in the asset configuration; creating the modal selector"
+ "_TtC15CarPlaySettings13CARThemePanel"
+ "_TtC15CarPlaySettings21CARThemeSelectorModel"
+ "_offersDriveModeThemeAssociation"
+ "_setCustomViewContentType:"
+ "chevron.up.chevron.down"
+ "clusterFallbackLayout"
+ "clusterThemeManager:didUpdateExtraAssetsURL:assetVersion:"
+ "configurationWithPointSize:"
+ "consoleFallbackLayout"
+ "currentThemeManagerData"
+ "iconManager:failedToOpenFolder:"
+ "initWithCustomView:"
+ "initWithRed:green:blue:alpha:"
+ "isDefault"
+ "model"
+ "setAccessibilityValue:"
+ "setMenu:"
+ "setSharesBackground:"
+ "setShowsMenuAsPrimaryAction:"
+ "systemImageNamed:withConfiguration:"
+ "v40@0:8@\"CRSUIClusterThemeManager\"16@\"NSSecurityScopedURLWrapper\"24Q32"
+ "viewIfLoaded"
- "@\"_TtC15CarPlaySettings30CARThemeSelectorViewController\""
- "ACCESSIBILITY_LINK_DRIVE_MODE"
- "ACCESSIBILITY_RESET"
- "ACCESSIBILITY_UNLINK_DRIVE_MODE"
- "AMBIENT_LIGHT_SYNC"
- "Added link/unlink button to navigation bar - button: %@"
- "Added reset button to navigation bar - button: %@"
- "Alert message explaining theme unlink behavior"
- "Alert title for unlinking theme from drive mode"
- "CARThemePanel"
- "CARThemeSelectorViewController viewDidLoad"
- "CarPlaySettings.CARThemeSelectorViewController"
- "CarPlaySettings/CARThemeSelectorViewController.swift"
- "Creating gallery selector with galleryConfiguration: %{public}s"
- "Creating standard/modal selector with standardConfiguration: %{public}s"
- "Failed to check for current drive mode configuration: %@"
- "Failed to clear current drive mode theme configuration: %@"
- "Failed to get active drive mode or display name, defaulting to Normal: %@"
- "Failed to get initial linked state, defaulting to true: %@"
- "Failed to set linked state: %@"
- "Initialized active drive mode: %@ (display name: %@)"
- "Initialized drive mode linked state: %@"
- "Link drive mode button accessibility label"
- "Linked theme to drive mode: %@"
- "Optional<DriveModeThemeActionsService>"
- "Requesting to clear current drive mode theme configuration for: %@"
- "Reset button accessibility label"
- "Set %ld right bar button items on %@"
- "Successfully cleared current drive mode theme configuration"
- "T@\"_TtC15CarPlaySettings30CARThemeSelectorViewController\",&,N,V_selectorViewController"
- "TB,N,V_cellBoolValue"
- "THEME_WILL_NO_LONGER_CHANGE_IN_DRIVE_MODE"
- "ThemeSelectorStyle"
- "UNLINK_THEME_WITH_DRIVE_MODE"
- "Unlink button title"
- "Unlink drive mode button accessibility label"
- "Unlinked theme from drive mode: %@"
- "[AmbientLight] Toggle: %{public}@"
- "_TtC15CarPlaySettings14DriveModeState"
- "_TtC15CarPlaySettings28DriveModeThemeActionsService"
- "_TtC15CarPlaySettings30CARThemeSelectorViewController"
- "_cellBoolValue"
- "_hasDriveModeConfigurationOverride"
- "_selectorViewController"
- "cancellables"
- "clusterThemeManager:didUpdateExtraAssetsURL:"
- "displayName(for:)"
- "driveModeState"
- "hasDriveModeConfigurationOverride:"
- "imageNamed:"
- "initWithBarButtonSystemItem:target:action:"
- "initWithImage:style:target:action:"
- "initWithTitle:style:target:action:"
- "linkUnlinkButtonTapped"
- "navigationController or topViewController is nil!"
- "resetButtonTapped"
- "selectorViewController"
- "setCellBoolValue:"
- "setSelectorViewController:"
- "shouldUseGallerySelector: %{public}s -> %{bool,public}d"
- "themeSelectorStyle"
- "v32@0:8@\"CRSUIClusterThemeManager\"16@\"NSSecurityScopedURLWrapper\"24"
```
