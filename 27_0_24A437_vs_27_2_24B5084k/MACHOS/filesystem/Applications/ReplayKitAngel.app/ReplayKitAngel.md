## ReplayKitAngel

> `/Applications/ReplayKitAngel.app/ReplayKitAngel`

### Sections with Same Size but Changed Content

- `__TEXT.__gcc_except_tab`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_intobj`

```diff

-740.63.1.2.0
-  __TEXT.__text: 0x3e42c
-  __TEXT.__auth_stubs: 0x12a0
-  __TEXT.__objc_stubs: 0x26a0
-  __TEXT.__objc_methlist: 0x188c
-  __TEXT.__const: 0x1e34
+765.9.1.0.0
+  __TEXT.__text: 0x445d8
+  __TEXT.__auth_stubs: 0x1340
+  __TEXT.__objc_stubs: 0x2740
+  __TEXT.__objc_methlist: 0x18e4
+  __TEXT.__const: 0x2054
   __TEXT.__gcc_except_tab: 0xa0
-  __TEXT.__oslogstring: 0x1cd5
-  __TEXT.__cstring: 0x1ff1
-  __TEXT.__objc_methname: 0x5123
-  __TEXT.__objc_classname: 0x5ae
-  __TEXT.__objc_methtype: 0x1b1d
-  __TEXT.__swift5_typeref: 0x8ec
-  __TEXT.__swift5_capture: 0x318
-  __TEXT.__swift5_reflstr: 0xb53
-  __TEXT.__swift5_assocty: 0x1c8
-  __TEXT.__constg_swiftt: 0x1268
-  __TEXT.__swift5_fieldmd: 0x9a4
-  __TEXT.__swift5_builtin: 0xb4
-  __TEXT.__swift5_proto: 0x114
-  __TEXT.__swift5_types: 0xdc
-  __TEXT.__swift_as_entry: 0x18
-  __TEXT.__swift_as_ret: 0x1c
-  __TEXT.__swift_as_cont: 0x3c
-  __TEXT.__unwind_info: 0xe48
-  __TEXT.__eh_frame: 0x4e8
-  __DATA_CONST.__const: 0x1b40
-  __DATA_CONST.__cfstring: 0x320
-  __DATA_CONST.__objc_classlist: 0xb0
+  __TEXT.__oslogstring: 0x1f65
+  __TEXT.__cstring: 0x20e1
+  __TEXT.__objc_methname: 0x5255
+  __TEXT.__objc_classname: 0x60e
+  __TEXT.__objc_methtype: 0x1aed
+  __TEXT.__swift5_typeref: 0x94c
+  __TEXT.__swift5_capture: 0x3b4
+  __TEXT.__swift5_reflstr: 0xc03
+  __TEXT.__swift5_assocty: 0x1f8
+  __TEXT.__constg_swiftt: 0x1418
+  __TEXT.__swift5_fieldmd: 0xa5c
+  __TEXT.__swift5_builtin: 0xc8
+  __TEXT.__swift5_proto: 0x124
+  __TEXT.__swift5_types: 0xf0
+  __TEXT.__swift_as_entry: 0x1c
+  __TEXT.__swift_as_ret: 0x2c
+  __TEXT.__swift_as_cont: 0x78
+  __TEXT.__unwind_info: 0xf90
+  __TEXT.__eh_frame: 0x7d0
+  __DATA_CONST.__const: 0x1dc0
+  __DATA_CONST.__cfstring: 0x340
+  __DATA_CONST.__objc_classlist: 0xb8
   __DATA_CONST.__objc_protolist: 0xe8
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0x60
   __DATA_CONST.__objc_superrefs: 0x30
   __DATA_CONST.__objc_intobj: 0x78
-  __DATA_CONST.__auth_got: 0x960
-  __DATA_CONST.__got: 0x3c8
-  __DATA_CONST.__auth_ptr: 0x400
-  __DATA.__objc_const: 0x5dd8
-  __DATA.__objc_selrefs: 0x11b8
+  __DATA_CONST.__auth_got: 0x9b0
+  __DATA_CONST.__got: 0x3d8
+  __DATA_CONST.__auth_ptr: 0x408
+  __DATA.__objc_const: 0x63f8
+  __DATA.__objc_selrefs: 0x11e0
   __DATA.__objc_ivar: 0x8c
-  __DATA.__objc_data: 0x16c0
-  __DATA.__data: 0x1208
-  __DATA.__bss: 0x2498
-  __DATA.__common: 0x180
+  __DATA.__objc_data: 0x18b8
+  __DATA.__data: 0x12c8
+  __DATA.__bss: 0x26b8
+  __DATA.__common: 0x198
   - /System/Library/Frameworks/AVFoundation.framework/AVFoundation
   - /System/Library/Frameworks/ActivityKit.framework/ActivityKit
   - /System/Library/Frameworks/Combine.framework/Combine

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 1511
-  Symbols:   579
-  CStrings:  1314
+  Functions: 1612
+  Symbols:   592
+  CStrings:  1343
 
Symbols:
+ _$s6TipKit0A0PAAE16resetEligibilityyyYaF
+ _$s6TipKit0A0PAAE16resetEligibilityyyYaFTu
+ _$s6TipKit0A0PAAE7optionsSayAA0A6Option_pGvg
+ _$s6TipKit0A23UIPopoverViewControllerC10imageStyle7SwiftUI05ShapeG0_pSgvs
+ _$s6TipKit4TipsO19ConfigurationOptionV16DisplayFrequencyV9immediateAGvgZ
+ _$s6TipKit4TipsO19ConfigurationOptionV16DisplayFrequencyVMa
+ _$s6TipKit4TipsO19ConfigurationOptionV16displayFrequencyyA2E07DisplayG0VFZ
+ _$s6TipKit4TipsO19ConfigurationOptionVMa
+ _$s6TipKit4TipsO19ConfigurationOptionVMn
+ _$s6TipKit4TipsO6ActionV2id5title7performAESSSg_xyyctcSyRzlufC
+ _$s6TipKit4TipsO6ActionV2idSSvg
+ _$s6TipKit4TipsO6ActionV4with5indexAESi_tF
+ _$s6TipKit4TipsO6ActionVMa
+ _$s6TipKit4TipsO6ActionVMn
+ _$s7SwiftUI5ColorV02uiC0ACSo7UIColorC_tcfC
+ _$s7SwiftUI5ColorVAA10ShapeStyleAAWP
+ _$s7SwiftUI5ColorVN
+ _AVControlCenterVideoEffectReactions
+ _AVControlCenterVideoEffectRingLight
+ _AVControlCenterVideoEffectsModuleSetRingLightOnboardingTipResponseForBundleID
+ _OBJC_CLASS_$_RPFeatureFlagUtility
+ _UILayoutFittingCompressedSize
+ _swift_arrayInitWithCopy
+ _swift_arrayInitWithTakeBackToFront
+ _swift_arrayInitWithTakeFrontToBack
+ _swift_isEscapingClosureAtFileLocation
+ _swift_release_x28
+ _swift_retain_x28
- _$s6TipKit0A0PAAE6statusAA4TipsO6StatusOvg
- _$s6TipKit4TipsO14OptionsBuilderV15buildExpressionyQrxAA0A6OptionRzlFZ
- _$s6TipKit4TipsO14OptionsBuilderV15buildExpressionyQrxAA0A6OptionRzlFZQOMQ
- _$s6TipKit4TipsO14OptionsBuilderV16buildFinalResultySayAA0A6Option_pGxAaGRzlFZ
- _$s6TipKit4TipsO14OptionsBuilderV17buildPartialBlock5firstQrx_tAA0A6OptionRzlFZ
- _$s6TipKit4TipsO14OptionsBuilderV17buildPartialBlock5firstQrx_tAA0A6OptionRzlFZQOMQ
- _$s6TipKit4TipsO15MaxDisplayCountVAA0A6OptionAAWP
- _$s6TipKit4TipsO15MaxDisplayCountVMa
- _$s6TipKit4TipsO15MaxDisplayCountVMn
- _$s6TipKit4TipsO15MaxDisplayCountVyAESicfC
- _$s6TipKit4TipsO18InvalidationReasonO20displayCountExceededyA2EmFWC
- _$s6TipKit4TipsO18InvalidationReasonOMa
- _$s6TipKit4TipsO6StatusO11invalidatedyAeC18InvalidationReasonOcAEmFWC
- _$s6TipKit4TipsO6StatusO2eeoiySbAE_AEtFZ
- _swift_getOpaqueTypeConformance2
CStrings:
+ " [ERROR] %{public}s:%d failed to create remote alert handle"
+ " [ERROR] %{public}s:%d no application name or bundle identifier"
+ " [ERROR] %{public}s:%d no configuration identifier for videoEffect=%@"
+ " [ERROR] %{public}s:%d no video effect"
+ " [ERROR] %{public}s:%d unrecognized video effect"
+ " [INFO] %{public}s:%d app=%@ bundleId=%@ videoEffect=%@"
+ " [INFO] %{public}s:%d feature flag disabled"
+ "%s already reported %ld, ignoring response=%ld"
+ "%s end returned for activity=%s"
+ "%s id=%s"
+ "%s no bundleID to report response for"
+ "%s response=%ld"
+ "%s sessionID=%s currentSessionID=%s"
+ "+shouldShowEdgeLightTip"
+ "-[RPAngelConnectionManager showRemoteAlertOfType:application:bundleID:]"
+ "-[RPRemoteAlertManager createRemoteHandleForApplication:bundleID:videoEffect:]"
+ "EDGE_LIGHT_TIP_BUTTON_TITLE"
+ "EDGE_LIGHT_TIP_DESCRIPTION"
+ "EDGE_LIGHT_TIP_TITLE"
+ "Error configuring tips %s"
+ "ReplayKitAngel"
+ "ReplayKitAngel.RPEdgeLightTipViewController"
+ "VB40@0:8@\"NSString\"16@\"NSString\"24@\"NSString\"32"
+ "VB40@0:8@16@24@32"
+ "_TtC14ReplayKitAngel28RPEdgeLightTipViewController"
+ "_rpLocalizableTableName"
+ "_rpLocalizedStringFromFrameworkBundleWithKey:tableName:"
+ "configurationIdentifier"
+ "createRemoteHandleForApplication:bundleID:videoEffect:"
+ "edge-light-tip-alert"
+ "edgeLightEnabled"
+ "finishSession session.end returned for sessionID=%s"
+ "getStatusBarAssertion"
+ "handleTipAction(_:)"
+ "hasPresentedTip"
+ "monitorEdgeLightTipStatus()"
+ "reportTipResponse(_:)"
+ "reportedTipResponse"
+ "showRemoteAlertOfType:application:bundleID:"
+ "systemLayoutSizeFittingSize:withHorizontalFittingPriority:verticalFittingPriority:"
+ "unrecognized configuration identifier %s"
- " [INFO] %{public}s:%d app=%@ bundleId=%@"
- "-[RPAngelConnectionManager dismissReactionsTipForApplication:bundleID:]"
- "-[RPAngelConnectionManager showReactionsTipForApplication:bundleID:]"
- "-[RPRemoteAlertManager createRemoteHandleForApplication:bundleID:]"
- "VB32@0:8@\"NSString\"16@\"NSString\"24"
- "VB32@0:8@16@24"
- "Vv32@0:8@\"NSString\"16@\"NSString\"24"
- "Vv32@0:8@16@24"
- "createRemoteHandleForApplication:bundleID:"
- "dismissReactionsTipForApplication:bundleID:"
- "showReactionsTipForApplication:bundleID:"
- "tip invalidated removing scene"
```
