## SiriMailSnippetProviderPlugin

> `/System/Library/FlowTools/SnippetService/ResponsePlugins/SiriMailSnippetProviderPlugin.bundle/SiriMailSnippetProviderPlugin`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__constg_swiftt`
- `__TEXT.__swift5_fieldmd`
- `__TEXT.__swift5_types`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift_as_ret`
- `__TEXT.__swift_as_cont`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__got`
- `__DATA.__objc_const`
- `__DATA.__objc_selrefs`

```diff

-3600.23.24.0.0
-  __TEXT.__text: 0xc79c
-  __TEXT.__auth_stubs: 0xb30
+3605.13.1.0.0
+  __TEXT.__text: 0xd3c0
+  __TEXT.__auth_stubs: 0xb40
   __TEXT.__objc_stubs: 0x160
   __TEXT.__objc_methlist: 0x154
-  __TEXT.__const: 0x348
-  __TEXT.__swift5_typeref: 0x184
+  __TEXT.__const: 0x3d0
   __TEXT.__constg_swiftt: 0xd0
+  __TEXT.__swift5_typeref: 0x1ac
   __TEXT.__swift5_fieldmd: 0x8c
   __TEXT.__cstring: 0xbd
   __TEXT.__swift5_types: 0x14

   __TEXT.__objc_methtype: 0x101
   __TEXT.__swift5_reflstr: 0x2c
   __TEXT.__swift5_assocty: 0x18
-  __TEXT.__oslogstring: 0xa11
+  __TEXT.__oslogstring: 0xaa1
   __TEXT.__swift5_proto: 0x8
   __TEXT.__swift_as_entry: 0x1c
   __TEXT.__swift_as_ret: 0x10
   __TEXT.__swift_as_cont: 0xc
-  __TEXT.__unwind_info: 0x218
-  __TEXT.__eh_frame: 0x1e0
-  __DATA_CONST.__const: 0x140
+  __TEXT.__unwind_info: 0x238
+  __TEXT.__eh_frame: 0x200
+  __DATA_CONST.__const: 0x1b0
   __DATA_CONST.__objc_classlist: 0x8
   __DATA_CONST.__objc_protolist: 0x50
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0x28
-  __DATA_CONST.__auth_got: 0x5a0
+  __DATA_CONST.__auth_got: 0x5a8
   __DATA_CONST.__got: 0x170
-  __DATA_CONST.__auth_ptr: 0x140
+  __DATA_CONST.__auth_ptr: 0x158
   __DATA.__objc_const: 0x218
   __DATA.__objc_selrefs: 0x118
-  __DATA.__data: 0x4f0
-  __DATA.__common: 0x18
+  __DATA.__data: 0x510
   __DATA.__bss: 0x100
   - /System/Library/Frameworks/Contacts.framework/Contacts
   - /System/Library/Frameworks/Foundation.framework/Foundation

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 218
-  Symbols:   114
-  CStrings:  98
+  Functions: 252
+  Symbols:   113
+  CStrings:  99
 
Symbols:
+ _SiriMailSnippetProviderPluginImplementationVersionNumber
+ _SiriMailSnippetProviderPluginImplementationVersionString
+ _swift_lookUpClassMethod
+ _swift_release_x22
- _objc_release_x27
- _objc_retain_x26
- _swift_release_x24
- _swift_release_x26
- _swift_release_x28
CStrings:
+ "#SiriMailSnippetProviderPlugin shouldPreferStaticView - draft has no renderable content, preferring static display representation"
```
