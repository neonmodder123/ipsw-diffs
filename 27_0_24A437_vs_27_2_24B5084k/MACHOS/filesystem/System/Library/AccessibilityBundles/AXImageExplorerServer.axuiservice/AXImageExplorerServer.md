## AXImageExplorerServer

> `/System/Library/AccessibilityBundles/AXImageExplorerServer.axuiservice/AXImageExplorerServer`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_fieldmd`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift5_types`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift5_protos`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__got`
- `__DATA_CONST.__auth_ptr`
- `__DATA.__objc_const`
- `__DATA.__objc_data`

```diff

-3240.9.0.0.0
-  __TEXT.__text: 0x53e78
+3245.7.1.0.0
+  __TEXT.__text: 0x548b0
   __TEXT.__auth_stubs: 0x2a40
-  __TEXT.__objc_stubs: 0xba0
+  __TEXT.__objc_stubs: 0xc00
   __TEXT.__objc_methlist: 0x41c
-  __TEXT.__const: 0x2824
-  __TEXT.__constg_swiftt: 0x8c8
-  __TEXT.__swift5_typeref: 0x6d88
+  __TEXT.__const: 0x2804
+  __TEXT.__constg_swiftt: 0x8d0
+  __TEXT.__swift5_typeref: 0x6c12
   __TEXT.__swift5_builtin: 0x64
   __TEXT.__swift5_reflstr: 0x6a7
   __TEXT.__swift5_fieldmd: 0x5d4
   __TEXT.__swift5_assocty: 0x258
-  __TEXT.__cstring: 0x12bb
-  __TEXT.__swift5_capture: 0x430
+  __TEXT.__cstring: 0x128b
+  __TEXT.__swift5_capture: 0x41c
   __TEXT.__swift5_proto: 0xcc
   __TEXT.__swift5_types: 0x78
   __TEXT.__objc_classname: 0x225
-  __TEXT.__objc_methname: 0x1481
+  __TEXT.__objc_methname: 0x14c1
   __TEXT.__objc_methtype: 0x681
-  __TEXT.__oslogstring: 0x1399
+  __TEXT.__oslogstring: 0x1419
   __TEXT.__swift_as_entry: 0x88
-  __TEXT.__swift_as_ret: 0xb4
-  __TEXT.__swift_as_cont: 0x1b0
+  __TEXT.__swift_as_ret: 0xb8
+  __TEXT.__swift_as_cont: 0x1b8
   __TEXT.__swift5_protos: 0x4
-  __TEXT.__unwind_info: 0x11e8
-  __TEXT.__eh_frame: 0x23fc
-  __DATA_CONST.__const: 0x1218
+  __TEXT.__unwind_info: 0x1200
+  __TEXT.__eh_frame: 0x2494
+  __DATA_CONST.__const: 0x11f0
   __DATA_CONST.__objc_classlist: 0x38
   __DATA_CONST.__objc_protolist: 0x40
   __DATA_CONST.__objc_imageinfo: 0x8

   __DATA_CONST.__got: 0x9d8
   __DATA_CONST.__auth_ptr: 0x7e0
   __DATA.__objc_const: 0xa20
-  __DATA.__objc_selrefs: 0x4e8
+  __DATA.__objc_selrefs: 0x4f8
   __DATA.__objc_data: 0x340
-  __DATA.__data: 0x1e50
+  __DATA.__data: 0x1e20
   __DATA.__bss: 0x1ae8
   __DATA.__common: 0x18
   - /System/Library/Frameworks/AVFAudio.framework/AVFAudio

   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
   Functions: 1331
-  Symbols:   259
-  CStrings:  475
+  Symbols:   261
+  CStrings:  478
 
Symbols:
+ _AVAudioSessionCategoryPlayback
+ _AVAudioSessionModeDefault
CStrings:
+ "Answer rejected by output safety guardrail. %@"
+ "[AXImageExplorerAudioInputManager] Failed to configure audio session: %@."
+ "setCategory:mode:options:error:"
+ "setShouldProcessRemotely:"
- "IMAGE_EXPLORER_REPORT_A_CONCERN_ACTION"
```
