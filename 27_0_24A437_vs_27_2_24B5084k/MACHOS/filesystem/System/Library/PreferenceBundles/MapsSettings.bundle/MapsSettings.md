## MapsSettings

> `/System/Library/PreferenceBundles/MapsSettings.bundle/MapsSettings`

### Sections with Same Size but Changed Content

- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA_CONST.__objc_floatobj`

```diff

-2972.30.6.12.58
-  __TEXT.__text: 0x3c98c
-  __TEXT.__auth_stubs: 0xf20
-  __TEXT.__objc_stubs: 0x8160
-  __TEXT.__objc_methlist: 0x3bcc
-  __TEXT.__const: 0x5ba
-  __TEXT.__cstring: 0xce72
-  __TEXT.__oslogstring: 0x1350
+2972.31.6.17.21
+  __TEXT.__text: 0x7975c
+  __TEXT.__auth_stubs: 0x21f0
+  __TEXT.__objc_stubs: 0x8240
+  __TEXT.__objc_methlist: 0x3ad4
+  __TEXT.__const: 0x3d70
+  __TEXT.__cstring: 0xe390
+  __TEXT.__oslogstring: 0x1475
   __TEXT.__gcc_except_tab: 0x6c8
-  __TEXT.__objc_classname: 0x787
-  __TEXT.__objc_methname: 0xbfc9
-  __TEXT.__objc_methtype: 0x177e
-  __TEXT.__constg_swiftt: 0x8c
-  __TEXT.__swift5_typeref: 0xea
-  __TEXT.__swift5_fieldmd: 0x2c
-  __TEXT.__swift5_reflstr: 0x47
-  __TEXT.__swift5_assocty: 0x30
-  __TEXT.__swift5_capture: 0x20
-  __TEXT.__swift5_proto: 0x8
-  __TEXT.__swift5_types: 0x8
-  __TEXT.__swift_as_entry: 0x4
-  __TEXT.__swift_as_cont: 0x4
-  __TEXT.__unwind_info: 0xf70
-  __TEXT.__eh_frame: 0x98
-  __DATA_CONST.__const: 0x19778
-  __DATA_CONST.__cfstring: 0xa4c0
-  __DATA_CONST.__objc_classlist: 0x1b8
+  __TEXT.__objc_classname: 0xb8a
+  __TEXT.__objc_methname: 0xc7d5
+  __TEXT.__objc_methtype: 0x193e
+  __TEXT.__constg_swiftt: 0x1078
+  __TEXT.__swift5_typeref: 0x4c69
+  __TEXT.__swift5_builtin: 0xb4
+  __TEXT.__swift5_reflstr: 0xda2
+  __TEXT.__swift5_fieldmd: 0xb2c
+  __TEXT.__swift5_assocty: 0x418
+  __TEXT.__swift5_capture: 0x9f0
+  __TEXT.__swift5_proto: 0x114
+  __TEXT.__swift5_types: 0xf0
+  __TEXT.__swift5_mpenum: 0x10
+  __TEXT.__swift_as_entry: 0x24
+  __TEXT.__swift_as_cont: 0x2c
+  __TEXT.__swift_as_ret: 0xc
+  __TEXT.__swift5_protos: 0x4
+  __TEXT.__unwind_info: 0x1d68
+  __TEXT.__eh_frame: 0x9c4
+  __DATA_CONST.__const: 0x1c388
+  __DATA_CONST.__cfstring: 0xa080
+  __DATA_CONST.__objc_classlist: 0x200
   __DATA_CONST.__objc_catlist: 0x118
-  __DATA_CONST.__objc_protolist: 0x80
+  __DATA_CONST.__objc_protolist: 0xc0
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_protorefs: 0x28
-  __DATA_CONST.__objc_superrefs: 0x180
-  __DATA_CONST.__objc_arraydata: 0x300
+  __DATA_CONST.__objc_protorefs: 0x60
+  __DATA_CONST.__objc_superrefs: 0x150
+  __DATA_CONST.__objc_arraydata: 0x330
   __DATA_CONST.__objc_arrayobj: 0x198
-  __DATA_CONST.__objc_intobj: 0xa08
-  __DATA_CONST.__objc_doubleobj: 0x3b0
+  __DATA_CONST.__objc_intobj: 0x960
+  __DATA_CONST.__objc_doubleobj: 0x3c0
   __DATA_CONST.__objc_floatobj: 0x20
-  __DATA_CONST.__objc_dictobj: 0x190
-  __DATA_CONST.__auth_got: 0x7a0
-  __DATA_CONST.__got: 0x7b0
-  __DATA_CONST.__auth_ptr: 0x80
-  __DATA.__objc_const: 0x5d18
-  __DATA.__objc_selrefs: 0x30f8
-  __DATA.__objc_ivar: 0x28c
-  __DATA.__objc_data: 0x10e0
-  __DATA.__data: 0x750
-  __DATA.__bss: 0x970
+  __DATA_CONST.__objc_dictobj: 0x1b8
+  __DATA_CONST.__auth_got: 0x1108
+  __DATA_CONST.__got: 0xbf0
+  __DATA_CONST.__auth_ptr: 0x708
+  __DATA.__objc_const: 0x7030
+  __DATA.__objc_selrefs: 0x3068
+  __DATA.__objc_ivar: 0x284
+  __DATA.__objc_data: 0x18a0
+  __DATA.__data: 0x3088
+  __DATA.__bss: 0x2ba0
+  __DATA.__common: 0x20
   - /System/Library/Frameworks/ARKit.framework/ARKit
   - /System/Library/Frameworks/Accounts.framework/Accounts
   - /System/Library/Frameworks/CoreFoundation.framework/CoreFoundation

   - /usr/lib/libMobileGestalt.dylib
   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libobjc.A.dylib
+  - /usr/lib/swift/libswiftAVFoundation.dylib
   - /usr/lib/swift/libswiftAccelerate.dylib
+  - /usr/lib/swift/libswiftAppleArchive.dylib
+  - /usr/lib/swift/libswiftCallKit.dylib
+  - /usr/lib/swift/libswiftCompression.dylib
   - /usr/lib/swift/libswiftCore.dylib
   - /usr/lib/swift/libswiftCoreAudio.dylib
+  - /usr/lib/swift/libswiftCoreAudio_Private.dylib
   - /usr/lib/swift/libswiftCoreFoundation.dylib
   - /usr/lib/swift/libswiftCoreImage.dylib
   - /usr/lib/swift/libswiftCoreLocation.dylib
   - /usr/lib/swift/libswiftCoreMIDI.dylib
   - /usr/lib/swift/libswiftDispatch.dylib
   - /usr/lib/swift/libswiftIntents.dylib
+  - /usr/lib/swift/libswiftMLCompute.dylib
   - /usr/lib/swift/libswiftMetal.dylib
+  - /usr/lib/swift/libswiftMetalKit.dylib
+  - /usr/lib/swift/libswiftModelIO.dylib
+  - /usr/lib/swift/libswiftNaturalLanguage.dylib
   - /usr/lib/swift/libswiftOSLog.dylib
   - /usr/lib/swift/libswiftObjectiveC.dylib
+  - /usr/lib/swift/libswiftObservation.dylib
   - /usr/lib/swift/libswiftQuartzCore.dylib
   - /usr/lib/swift/libswiftSpatial.dylib
   - /usr/lib/swift/libswiftUniformTypeIdentifiers.dylib

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 2604
-  Symbols:   1796
-  CStrings:  3780
+  Functions: 4035
+  Symbols:   1917
+  CStrings:  3958
 
Symbols:
+ _GEOConfigGetArray
+ _GEOConfigRemoveBlockListener
+ _MapsConfig_ChromeContextCoordinationStuckQueueRecoveryTimeout
+ _MapsConfig_DisplayedVisitedPlacesMinimumDisplayDuration
+ _MapsConfig_EnrichmentOnboardingSheetSuppressed
+ _MapsConfig_JetEnabled
+ _MapsConfig_JetForceFallbackJetPack
+ _MapsConfig_JetHighlightRenderedUI
+ _MapsConfig_JetIgnoreJetPackCache
+ _MapsConfig_JetIgnoreJetPackSigning
+ _MapsConfig_JetInspectable
+ _MapsConfig_JetPackProduct
+ _MapsConfig_JetPackProductURLs
+ _MapsConfig_JetPackURLOverride
+ _MapsConfig_JetUseProductionBundledPack
+ _MapsConfig_MapModePickerZoomTransitionEnabled
+ _MapsConfig_MapsHomeMaxRecentsMac
+ _MapsConfig_MapsHomeRecentsRowsPerColumn
+ _MapsConfig_MapsHomeRecentsRowsPerColumnMac
+ _MapsConfig_PreferencesScreenUseJetRendering
+ _MapsConfig_SearchHomeRecentMaxColumns
+ _MapsConfig_SearchHomeRecentMaxColumnsMac
+ _MapsConfig_SearchHomeRecentRowsPerColumn
+ _MapsConfig_SearchHomeRecentRowsPerColumnEnriched
+ _MapsConfig_SearchHomeRecentRowsPerColumnMac
+ _MapsConfig_SidebarSheetWidthMultiplierMac
+ _MobileGestalt_get_current_device
+ _MobileGestalt_get_deviceClassNumber
+ _MobileGestalt_get_starkCapability
+ _MobileGestalt_get_wapiCapability
+ _NavigationConfig_Dodgeball_EnableFasterSettings
+ _NavigationConfig_Dodgeball_MinimumTimeSavings
+ _NavigationConfig_Dodgeball_MinimumTimeSavingsOptions
+ _NavigationConfig_Dodgeball_ShouldDefaultAccept
+ _OBJC_CLASS_$_GEOMapDataSubscription
+ _OBJC_CLASS_$_NSDateComponentsFormatter
+ _OBJC_CLASS_$_OBBundle
+ _OBJC_CLASS_$_OBPrivacyFlow
+ _OBJC_CLASS_$_OS_dispatch_queue
+ _OBJC_CLASS_$_UIScene
+ _OBJC_CLASS_$_UIWindow
+ _OBJC_CLASS_$_UIWindowScene
+ _OBJC_CLASS_$_VGVehicle
+ _OBJC_CLASS_$__MXExtension
+ __Block_copy
+ __Block_release
+ __availability_version_check
+ __swift_FORCE_LOAD_$_swiftAVFoundation
+ __swift_FORCE_LOAD_$_swiftAppleArchive
+ __swift_FORCE_LOAD_$_swiftCallKit
+ __swift_FORCE_LOAD_$_swiftCompression
+ __swift_FORCE_LOAD_$_swiftCoreAudio_Private
+ __swift_FORCE_LOAD_$_swiftMLCompute
+ __swift_FORCE_LOAD_$_swiftMetalKit
+ __swift_FORCE_LOAD_$_swiftModelIO
+ __swift_FORCE_LOAD_$_swiftNaturalLanguage
+ _dispatch_once_f
+ _dlsym
+ _fclose
+ _fopen
+ _fread
+ _fseek
+ _ftell
+ _malloc
+ _memcpy
+ _objc_retain_x9
+ _rewind
+ _sscanf
+ _swift_allocBox
+ _swift_allocError
+ _swift_allocateGenericClassMetadata
+ _swift_arrayDestroy
+ _swift_beginAccess
+ _swift_bridgeObjectRetain_n
+ _swift_checkMetadataState
+ _swift_continuation_await
+ _swift_continuation_init
+ _swift_continuation_throwingResume
+ _swift_continuation_throwingResumeWithError
+ _swift_cvw_assignWithCopy
+ _swift_cvw_assignWithTake
+ _swift_cvw_destroy
+ _swift_cvw_initStructMetadataWithLayoutString
+ _swift_cvw_initWithCopy
+ _swift_cvw_initWithTake
+ _swift_cvw_initializeBufferWithCopyOfBuffer
+ _swift_dynamicCast
+ _swift_dynamicCastObjCClass
+ _swift_endAccess
+ _swift_errorRelease
+ _swift_getAtKeyPath
+ _swift_getEnumCaseMultiPayload
+ _swift_getEnumTagSinglePayloadGeneric
+ _swift_getForeignTypeMetadata
+ _swift_getGenericMetadata
+ _swift_getKeyPath
+ _swift_getObjCClassFromMetadata
+ _swift_getObjectType
+ _swift_getSingletonMetadata
+ _swift_initClassMetadata2
+ _swift_initStackObject
+ _swift_isUniquelyReferenced_nonNull_bridgeObject
+ _swift_once
+ _swift_projectBox
+ _swift_release
+ _swift_release_n
+ _swift_release_x1
+ _swift_release_x20
+ _swift_release_x21
+ _swift_release_x22
+ _swift_release_x23
+ _swift_release_x24
+ _swift_release_x25
+ _swift_release_x26
+ _swift_release_x27
+ _swift_release_x28
+ _swift_retain
+ _swift_retain_x1
+ _swift_retain_x19
+ _swift_retain_x2
+ _swift_retain_x20
+ _swift_retain_x21
+ _swift_retain_x22
+ _swift_retain_x23
+ _swift_retain_x24
+ _swift_retain_x25
+ _swift_retain_x26
+ _swift_retain_x27
+ _swift_retain_x28
+ _swift_slowAlloc
+ _swift_slowDealloc
+ _swift_storeEnumTagMultiPayload
+ _swift_storeEnumTagSinglePayloadGeneric
+ _swift_task_create
+ _swift_unknownObjectRelease
+ _swift_unknownObjectRetain
+ _swift_unknownObjectWeakDestroy
+ _swift_unknownObjectWeakInit
+ _swift_unknownObjectWeakLoadStrong
+ _swift_updateClassMetadata2
+ _swift_weakDestroy
+ _swift_weakInit
+ _swift_weakLoadStrong
+ _swift_willThrow
- _GEODefaultsDomain
- _MapsConfig_FlyoverMaxSearchNotification
- _MapsSettingsCyclingTitle
- _MapsSettingsDrivingTitle
- _MapsSettingsSpokenDirectionsTitle
- _MapsSettingsTransitTitle
- _MapsSettingsWalkingTitle
- _OBJC_CLASS_$_MapsSettingsCyclingController
- _OBJC_CLASS_$_MapsSettingsDrivingController
- _OBJC_CLASS_$_MapsSettingsNavigationGuidanceController
- _OBJC_CLASS_$_MapsSettingsTransitController
- _OBJC_CLASS_$_MapsSettingsWalkingController
- _OBJC_CLASS_$_NSTextAttachment
- _OBJC_CLASS_$_PSListControllerCellHighlightingSelectionInvocationRelay
- _OBJC_CLASS_$_PedestrianARSettingsFooterView
- _OBJC_METACLASS_$_MapsSettingsCyclingController
- _OBJC_METACLASS_$_MapsSettingsDrivingController
- _OBJC_METACLASS_$_MapsSettingsNavigationGuidanceController
- _OBJC_METACLASS_$_MapsSettingsTransitController
- _OBJC_METACLASS_$_MapsSettingsWalkingController
- _OBJC_METACLASS_$_PedestrianARSettingsFooterView
- _PSAccessoryKey
- _PSListControllerCellHighlightingSelectionInvocationRelayKey
CStrings:
+ "$__lazy_storage_$_directionsPreferences"
+ "$__lazy_storage_$_spokenDirections"
+ "$__lazy_storage_$_transitPreferences"
+ "%d.%d.%d"
+ ", confirmedValue: "
+ "/System/Library/CoreServices/SystemVersion.plist"
+ "@\"<VGOEMAppSOCStreaming>\"16@0:8"
+ "@\"LSApplicationRecord\"16@0:8"
+ "@32@0:8@\"NSString\"16@\"LSApplicationRecord\"24"
+ "Accept Faster Routes"
+ "Accept Faster Routes Switch Label [Settings]"
+ "Accept Faster Routes footer [Settings]"
+ "Accessing Environment<%s>'s value outside of being installed on a View. This will always read the default value and will not update."
+ "After you tap %{ARKIT_ICON}@, you can view directions in the real world simply by raising your iPhone."
+ "Allow Apple to email “%@” with questions about issues you report."
+ "Allow Photo Providers to Use Your Photos"
+ "Allow companies that provide photos to Maps to use the photos that you add to Maps in their own products and services. Photos include their locations but not your identity. If you turn this off, photo providers may no longer use your photos, but this may take a few days to apply."
+ "An unknown error has occurred. Please try again."
+ "At least one mode of transit must be selected."
+ "Automatic Updates"
+ "Automatically removes offline maps that you haven’t used in a while."
+ "Blocked Contacts"
+ "CFDataCreateWithBytesNoCopy"
+ "CFDictionaryGetValue"
+ "CFGetTypeID"
+ "CFPropertyListCreateFromXMLData"
+ "CFPropertyListCreateWithData"
+ "CFRelease"
+ "CFStringCreateWithCStringNoCopy"
+ "CFStringGetCString"
+ "CFStringGetTypeID"
+ "Carry"
+ "ChromeContextCoordinationStuckQueueRecoveryTimeout"
+ "Contribute to Maps"
+ "Directions Pause Spoken Audio"
+ "Directions Wake Device"
+ "Directions in the Real World"
+ "Directions on Radio"
+ "DirectionsPreferencesModel"
+ "Display the location of your parked car."
+ "DisplayedVisitedPlacesMinimumDisplayDuration"
+ "EnrichmentOnboardingSheetSuppressed"
+ "Faster Route Suggestions"
+ "Faster Route Suggestions Group Title [Settings]"
+ "Follow up by Email"
+ "Ignoring photo-sharing write to %{bool,public}d: row is not ready (state: %{public}s)"
+ "Improve EV Routing"
+ "JetEnabled"
+ "JetForceFallbackJetPack"
+ "JetHighlightRenderedUI"
+ "JetIgnoreJetPackCache"
+ "JetIgnoreJetPackSigning"
+ "JetInspectable"
+ "JetPackProduct"
+ "JetPackProductURLs"
+ "JetPackURLOverride"
+ "JetUseProductionBundledPack"
+ "Last Transit Mode Accessibility Hint [Settings]"
+ "Learn More About Online Safety"
+ "MapModePickerZoomTransitionEnabled"
+ "Maps only asks if you want to change your route if it saves at least this much time."
+ "MapsHomeMaxRecentsMac"
+ "MapsHomeRecentsRowsPerColumn"
+ "MapsHomeRecentsRowsPerColumnMac"
+ "MapsSettings1"
+ "MapsSettingsRootModel"
+ "Minimum Time Savings"
+ "Minimum Time Savings Row Label [Settings]"
+ "Minimum Time Savings footer [Settings]"
+ "Navigation Accuracy"
+ "New maps you download to your iPhone will also download to “%@.”"
+ "On cars that support it, hear spoken directions when you’re listening to the radio."
+ "Only Use Offline Maps"
+ "Optimize Storage"
+ "Pause content like podcasts and audio books during spoken directions."
+ "Photo-sharing write failed; reverting to %{bool,public}d"
+ "PreferencesScreenUseJetRendering"
+ "Preferred Type of Travel"
+ "Presubmission"
+ "ProductVersion"
+ "Ratings and Photos"
+ "Refreshes your maps periodically to keep them current."
+ "Refreshes your maps periodically to keep them current. Not available if you have more than %@ of offline maps."
+ "Report and Verify While Navigating"
+ "Reporting Incidents"
+ "Restaurant Booking"
+ "SearchHomeRecentMaxColumns"
+ "SearchHomeRecentMaxColumnsMac"
+ "SearchHomeRecentRowsPerColumn"
+ "SearchHomeRecentRowsPerColumnEnriched"
+ "SearchHomeRecentRowsPerColumnMac"
+ "Share ETA is only available for driving navigation."
+ "Show Parked Location"
+ "Show Ratings and Photos Suggestions"
+ "Show in Navigation"
+ "SidebarSheetWidthMultiplierMac"
+ "Speed limit information will be shown where available."
+ "Spoken Directions"
+ "SpokenDirectionsModel"
+ "Submit reports with a few taps while navigating, and verify if crashes and other incidents are still present."
+ "Sync with Apple\u00a0Watch"
+ "T@\"<VGOEMAppSOCStreaming>\",W,N"
+ "T@\"LSApplicationRecord\",R,N"
+ "TB,N,GisEnabled"
+ "TransitPreferencesModel"
+ "Use downloaded maps even when you have an internet connection."
+ "Use iPhone’s camera and motion sensors to improve the accuracy of your location and direction when navigating. Camera and sensor data aren’t stored. Turning this on may use more battery power."
+ "VGOEMApplication"
+ "VGVehicleStateProviding"
+ "View.task @ MapsSettings/MapsSettingsRootView.swift:"
+ "When a faster route is suggested, Maps automatically switches to it if you don't respond."
+ "When available, this transportation type will be used to get directions and to estimate your travel time."
+ "When driving or cycling, spoken directions wake the display if the device is locked."
+ "Wi-Fi + Cellular"
+ "Your parked car will be shown on the map if your location can be determined when parking. A connection to your car’s Bluetooth is required."
+ "Your parked car will be shown on the map if your location can be determined when parking. A connection to your car’s Bluetooth or CarPlay stereo is required."
+ "](maps-settings://privacy)"
+ "_$observationRegistrar"
+ "_TtC12MapsSettings15ShareETASetting"
+ "_TtC12MapsSettings16OfflineMapsModel"
+ "_TtC12MapsSettings17PhotoSharingModel"
+ "_TtC12MapsSettings19AppPolicyController"
+ "_TtC12MapsSettings19DefaultsBoolStorage"
+ "_TtC12MapsSettings19TransitModesStorage"
+ "_TtC12MapsSettings20GEOConfigBoolStorage"
+ "_TtC12MapsSettings21MapsSettingsRootModel"
+ "_TtC12MapsSettings21SpokenDirectionsModel"
+ "_TtC12MapsSettings23ParkedCarEnabledSetting"
+ "_TtC12MapsSettings23RatingsAndPhotosSetting"
+ "_TtC12MapsSettings23TransitPreferencesModel"
+ "_TtC12MapsSettings24SpokenVoiceVolumeStorage"
+ "_TtC12MapsSettings26DirectionsPreferencesModel"
+ "_TtC12MapsSettings26ExtensionAvailabilityModel"
+ "_TtC12MapsSettings29MapsSettingsConfigBoolStorage"
+ "_TtC12MapsSettings29MigratingGEOConfigBoolStorage"
+ "_TtC12MapsSettings29PreferredTransportTypeSetting"
+ "_TtC12MapsSettings31AllowEmailCorrespondenceSetting"
+ "_TtC12MapsSettings32MapsSettingsConfigIntegerStorage"
+ "_TtC12MapsSettings34RatingsAndPhotosSuggestionsSetting"
+ "_allowsAutomaticUpdates"
+ "_automaticUpdatesLimitDescription"
+ "_cache"
+ "_cellularStatus"
+ "_hasEVApplications"
+ "_hasEVApps"
+ "_hasFullyLoadedSubscriptions"
+ "_hasRestaurantBookingApps"
+ "_hasRideBookingApps"
+ "_isAvailable"
+ "_isChina"
+ "_issueReportingEmailAddress"
+ "_pairedWatch"
+ "_parkedCarRestrictedByMDM"
+ "_parkedCarSupported"
+ "_restaurantBookingManager"
+ "_rideBookingAppCount"
+ "_rideBookingManager"
+ "_scrollHighlightTarget"
+ "_scrollHighlightedID"
+ "_shareETASupported"
+ "_supportsWalkingAvoidBusyRoads"
+ "_supportsWalkingAvoidHills"
+ "_supportsWalkingAvoidStairs"
+ "_updateFailed"
+ "_usesWLANTerminology"
+ "_versionID"
+ "acceptFasterRoutes"
+ "allowsCellularDownloads"
+ "automaticUpdatesPreference"
+ "avoidHighways"
+ "avoidTolls"
+ "bundleWithIdentifier:"
+ "cachedSpecifiers"
+ "chargeStreamingDelegate"
+ "connectedScenes"
+ "contentSize"
+ "countryConfigurationDidChange"
+ "cyclingAvoidBusyRoads"
+ "cyclingAvoidHills"
+ "defaultValue"
+ "deviceSupportsCarPlay"
+ "directionsOnRadio"
+ "evApplicationFinder"
+ "evRouting"
+ "extensionAvailability"
+ "fasterRouteSuggestionsEnabled"
+ "flowWithBundle:"
+ "getStateOfChargeForVehicle:completion:"
+ "getter"
+ "helper"
+ "https://apps.mzstatic.com/content/3e22681091624b6eaf72730309a89491/maps.jetpack"
+ "https://apps.mzstatic.com/content/cdcbf37fc6874dbf9112a58d7d53ce28/maps.jetpack"
+ "https://apps.mzstatic.com/content/e2206ce7479244289fdbee6d39042dd9/maps.jetpack"
+ "iOS Production"
+ "improveEVRoutingEnabled"
+ "initWithBool:"
+ "initWithIdentifier:applicationRecord:"
+ "initWithInteger:"
+ "initWithUnsignedInteger:"
+ "isKeyWindow"
+ "kCFAllocatorNull"
+ "key"
+ "labelsAlwaysInUILanguage"
+ "lastReportedHeight"
+ "listCarsWithCompletion:"
+ "listenerHandle"
+ "minimumFasterRouteTimeSavings"
+ "modes"
+ "new"
+ "offlineMaps"
+ "onContentHeightChange"
+ "onlyUseOfflineMapsPreference"
+ "opticalHeading"
+ "optimizeStorageEnabled"
+ "parkedCarEnabled"
+ "pauseSpokenAudio"
+ "photoSharing"
+ "preferredTransportType"
+ "presentMapsAppPrivacySplashViewController"
+ "r"
+ "raiseToStartPedestrianAR"
+ "ratingsAndPhotosEnabled"
+ "ratingsAndPhotosSuggestionsEnabled"
+ "reportAndVerifyIncidents"
+ "restaurantBooking"
+ "rideBooking"
+ "setAllowedUnits:"
+ "setAlwaysBounceVertical:"
+ "setChargeStreamingDelegate:"
+ "setContentInset:"
+ "setContentInsetAdjustmentBehavior:"
+ "setEstimatedRowHeight:"
+ "setEstimatedSectionFooterHeight:"
+ "setEstimatedSectionHeaderHeight:"
+ "setScrollEnabled:"
+ "setTableHeaderView:"
+ "setUnitsStyle:"
+ "setter"
+ "settings-navigation://com.apple.Settings.Apps/"
+ "shareETAEnabled"
+ "showAirQualityIndex"
+ "showHeading"
+ "showSpeedLimit"
+ "showWeatherConditions"
+ "startSendingChargeUpdatesForVehicle:"
+ "stopSendingChargeUpdatesForVehicle:"
+ "storage"
+ "stringFromTimeInterval:"
+ "syncToNano"
+ "syncToWatchEnabled"
+ "tableViewStyle"
+ "updating(pendingValue: "
+ "useEbike"
+ "v24@0:8@\"<VGOEMAppSOCStreaming>\"16"
+ "v24@0:8@\"VGVehicle\"16"
+ "v24@0:8@?<v@?@\"NSArray\"@\"NSError\">16"
+ "v32@0:8@\"VGVehicle\"16@?<v@?@\"VGVehicleState\"@\"NSError\">24"
+ "voiceVolume"
+ "wakeDevice"
+ "walkingAvoidBusyRoads"
+ "walkingAvoidHills"
+ "walkingAvoidStairs"
+ "windows"
- "@\"UIView<PSHeaderFooterView>\"24@0:8@\"PSSpecifier\"16"
- "ARKIT_ICON"
- "Bus.png"
- "Cycling"
- "Ferry.png"
- "FlyoverMaxSearchNotificationKey"
- "FlyoverTourButtonBackgroundSelectedColor"
- "FlyoverTourButtonBackgroundUnselectedColor"
- "LockscreenSettingsGroupID"
- "MapsSettingsCyclingController"
- "MapsSettingsDrivingController"
- "MapsSettingsNavigationGuidanceController"
- "MapsSettingsSpecifierBundleID"
- "MapsSettingsTransitController"
- "MapsSettingsWalkingController"
- "NavigationVoiceVolumeLoudID"
- "NavigationVoiceVolumeNormalID"
- "NavigationVoiceVolumeSoftID"
- "PSHeaderFooterView"
- "PauseSpokenAudioGroupID"
- "PedestrianARSettingsFooterView"
- "Rail.png"
- "Subway.png"
- "T@\"UILabel\",&,N,V_label"
- "UseHFPWhenAvailableGroupSpec"
- "Walking"
- "_avoidBusyRoads:"
- "_avoidHighways:"
- "_avoidHills:"
- "_avoidStairs:"
- "_avoidTolls:"
- "_configureCell:forTransitSpecifier:"
- "_defaultFontForTableViewStyle:isSectionHeader:"
- "_defaultTextColorForTableViewStyle:isSectionHeader:"
- "_enableScreenWakes:"
- "_isModeDisabled:"
- "_pauseSpokenAudio:"
- "_reportAndVerifyIncidentsWhileNavigating:"
- "_setAvoidBusyRoads:specifier:"
- "_setAvoidHighways:specifier:"
- "_setAvoidHills:specifier:"
- "_setAvoidStairs:specifier:"
- "_setAvoidTolls:specifier:"
- "_setEnableScreenWakes:specifier:"
- "_setPauseSpokenAudio:specifier:"
- "_setReportAndVerifyIncidentsWhileNavigating:specifier:"
- "_setShowHeading:specifier:"
- "_setShowSpeedLimit:specifier:"
- "_setUseEbike:specifier:"
- "_setUseHFP:specifier:"
- "_showHeading:"
- "_showSpeedLimit:"
- "_specifierIDForVoiceVolume:"
- "_toggleMode:"
- "_useEbike:"
- "_useHFP:"
- "_voiceVolumeSpecs"
- "attributedStringWithAttachment:"
- "d24@0:8d16"
- "d32@0:8d16@\"UITableView\"24"
- "d32@0:8d16@24"
- "flyoverTourButtonBackgroundColor:"
- "grayColor"
- "groupSpecifierWithID:"
- "imageNamed:inBundle:"
- "initWithSpecifier:"
- "label"
- "opticalHeading:"
- "preferredHeightForWidth:"
- "preferredHeightForWidth:inTableView:"
- "raiseToStartPedestrianAR:"
- "refreshContentsWithSpecifier:"
- "row"
- "setLabel:"
- "setOpticalHeading:specifier:"
- "setRaiseToStartPedestrianAR:specifier:"
- "setSpecifierIdentifierToScrollAndHighlight:"
- "settings-navigation://com.apple.Settings.Apps/com.apple.Maps/CyclingLinkPreferenceID"
- "settings-navigation://com.apple.Settings.Apps/com.apple.Maps/DrivingLinkPreferenceID"
- "settings-navigation://com.apple.Settings.Apps/com.apple.Maps/NavigationGuidanceLinkPreferenceID"
- "settings-navigation://com.apple.Settings.Apps/com.apple.Maps/TransitLinkPreferenceID"
- "settings-navigation://com.apple.Settings.Apps/com.apple.Maps/WalkingLinkPreferenceID"
- "tableView:cellForRowAtIndexPath:"
- "textAttachmentWithImage:"
- "v24@0:8@\"PSSpecifier\"16"
```
