## AppleCVHWA

> `/System/Library/PrivateFrameworks/AppleCVHWA.framework/AppleCVHWA`

```diff

 4.4.12.0.0
-  __TEXT.__text: 0xb5ea8
-  __TEXT.__const: 0x2f90
-  __TEXT.__gcc_except_tab: 0x555c
+  __TEXT.__text: 0xbc518
+  __TEXT.__const: 0x3030
+  __TEXT.__gcc_except_tab: 0x5620
   __TEXT.__oslogstring: 0x3b9
-  __TEXT.__cstring: 0x8ef8
-  __TEXT.__unwind_info: 0x17b8
+  __TEXT.__cstring: 0x8fa0
+  __TEXT.__unwind_info: 0x1840
   __TEXT.__eh_frame: 0x50
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_methname: 0x0
-  __DATA_CONST.__const: 0x2a8
+  __DATA_CONST.__const: 0x2c8
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__weak_got: 0x18
   __DATA_CONST.__objc_selrefs: 0x8
   __DATA_CONST.__got: 0x0
-  __AUTH_CONST.__const: 0x15a8
+  __AUTH_CONST.__const: 0x16f8
   __AUTH_CONST.__cfstring: 0x220
   __AUTH_CONST.__weak_auth_got: 0x50
   __AUTH_CONST.__objc_intobj: 0x18
   __AUTH_CONST.__auth_got: 0x5e8
   __AUTH.__data: 0x20
-  __DATA.__data: 0x7f98
+  __DATA.__data: 0x88e8
   __DATA.__crash_info: 0x148
   __DATA.__bss: 0x3c8
   __DATA.__common: 0x10

   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libc++.1.dylib
   - /usr/lib/libobjc.A.dylib
-  Functions: 1147
+  Functions: 1190
   Symbols:   448
-  CStrings:  583
+  CStrings:  585
 
CStrings:
+ "/Library/Caches/com.apple.xbs/<UUID>/TemporaryDirectory.<TMP>/Sources/AppleCVHWA/library/VIO/HWFeatureDetection/src/HwBorneo-V63.cpp"
+ "H19"
```
