## BatteryDischarge

> `/System/Library/PrivateFrameworks/BatteryDischarge.framework/BatteryDischarge`

```diff

 16.0.0.0.0
-  __TEXT.__text: 0x1e38
-  __TEXT.__objc_methlist: 0x64
+  __TEXT.__text: 0x2b00
+  __TEXT.__objc_methlist: 0x94
   __TEXT.__const: 0x25a
   __TEXT.__cstring: 0x62
-  __TEXT.__oslogstring: 0xeb
-  __TEXT.__constg_swiftt: 0xb0
-  __TEXT.__swift5_typeref: 0xf0
+  __TEXT.__oslogstring: 0x16b
+  __TEXT.__constg_swiftt: 0xc0
+  __TEXT.__swift5_typeref: 0x11e
   __TEXT.__swift5_reflstr: 0x14
   __TEXT.__swift5_fieldmd: 0x2c
-  __TEXT.__swift5_capture: 0x40
+  __TEXT.__swift5_capture: 0x60
   __TEXT.__swift5_types: 0xc
   __TEXT.__swift5_assocty: 0x30
   __TEXT.__swift5_builtin: 0x28
   __TEXT.__swift5_proto: 0x18
-  __TEXT.__unwind_info: 0xf8
+  __TEXT.__unwind_info: 0x110
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_classlist: 0x8
   __DATA_CONST.__objc_protolist: 0x8
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x68
+  __DATA_CONST.__objc_selrefs: 0x80
   __DATA_CONST.__objc_protorefs: 0x8
   __DATA_CONST.__got: 0x0
-  __AUTH_CONST.__const: 0x1f8
-  __AUTH_CONST.__objc_const: 0xe8
-  __AUTH_CONST.__auth_got: 0x1d8
-  __AUTH.__objc_data: 0xf0
+  __AUTH_CONST.__const: 0x2e8
+  __AUTH_CONST.__objc_const: 0xf8
+  __AUTH_CONST.__auth_got: 0x1e0
+  __AUTH.__objc_data: 0x100
   __AUTH.__data: 0x28
   __DATA.__data: 0x48
   __DATA.__bss: 0x300

   - /usr/lib/swift/libswiftXPC.dylib
   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswiftos.dylib
-  Functions: 79
-  Symbols:   118
-  CStrings:  9
+  Functions: 101
+  Symbols:   123
+  CStrings:  11
 
Symbols:
+ _objc_msgSend$getShippingComplianceStatusWithReply:
+ _objc_msgSend$startShippingComplianceDischargeWithThermalMode:completion:
+ _objc_retain_x20
+ _symbolic _____A2AIeyByyy_ 10ObjectiveC8ObjCBoolV
+ _symbolic _____So8NSStringCSgIeyByy_ 10ObjectiveC8ObjCBoolV
CStrings:
+ "Failed to get XPC proxy for dischargeUntilShippingCompliant"
+ "Failed to get XPC proxy for getShippingComplianceStatus"
```
