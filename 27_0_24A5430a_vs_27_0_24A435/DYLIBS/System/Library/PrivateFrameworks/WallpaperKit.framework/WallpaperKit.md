## WallpaperKit

> `/System/Library/PrivateFrameworks/WallpaperKit.framework/WallpaperKit`

```diff

 2144.0.0.0.0
-  __TEXT.__text: 0x8e670
+  __TEXT.__text: 0x8e610
   __TEXT.__objc_methlist: 0x2188
   __TEXT.__const: 0x7914
   __TEXT.__gcc_except_tab: 0xb64
   __TEXT.__cstring: 0x32d0
-  __TEXT.__oslogstring: 0x3da5
+  __TEXT.__oslogstring: 0x3dc5
   __TEXT.__swift5_typeref: 0x28f4
   __TEXT.__swift5_reflstr: 0x1bbb
   __TEXT.__swift5_assocty: 0x7d0

   __TEXT.__swift_as_entry: 0x1c
   __TEXT.__swift_as_ret: 0x18
   __TEXT.__swift_as_cont: 0x34
-  __TEXT.__unwind_info: 0x1fa0
+  __TEXT.__unwind_info: 0x1f98
   __TEXT.__eh_frame: 0x8a0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0

   __AUTH.__objc_data: 0x1b8
   __AUTH.__data: 0x828
   __DATA.__objc_ivar: 0x220
-  __DATA.__data: 0x1270
+  __DATA.__data: 0x1250
   __DATA.__bss: 0x3190
   __DATA.__common: 0x40
   __DATA_DIRTY.__objc_data: 0xd70
-  __DATA_DIRTY.__data: 0x2500
+  __DATA_DIRTY.__data: 0x2510
   __DATA_DIRTY.__crash_info: 0x148
   __DATA_DIRTY.__bss: 0x4878
   __DATA_DIRTY.__common: 0x198

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 3257
+  Functions: 3256
   Symbols:   3303
-  CStrings:  599
+  CStrings:  600
 
Symbols:
+ _objc_msgSend$wk_setDepthEffectEnabled:
- _objc_retain_x12
CStrings:
+ "Depth not supported, collapsing floating view into foreground view"
+ "Depth supported, adding floating view"
- "Adding toggle depth action since '%{public}s' has floating layer content (LayeredStill)."
```
