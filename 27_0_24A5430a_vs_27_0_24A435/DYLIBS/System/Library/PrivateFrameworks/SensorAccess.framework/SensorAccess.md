## SensorAccess

> `/System/Library/PrivateFrameworks/SensorAccess.framework/SensorAccess`

```diff

 25.0.0.0.0
-  __TEXT.__text: 0x2dbc
-  __TEXT.__objc_methlist: 0x15c
-  __TEXT.__const: 0x262
-  __TEXT.__cstring: 0x544
-  __TEXT.__swift5_typeref: 0x6a
-  __TEXT.__swift5_reflstr: 0x9
-  __TEXT.__swift5_assocty: 0x18
-  __TEXT.__constg_swiftt: 0x64
-  __TEXT.__swift5_builtin: 0x14
-  __TEXT.__swift5_capture: 0x40
-  __TEXT.__oslogstring: 0xf4
-  __TEXT.__swift5_proto: 0x1c
-  __TEXT.__swift5_types: 0x8
-  __TEXT.__swift5_fieldmd: 0x10
-  __TEXT.__unwind_info: 0x188
-  __TEXT.__eh_frame: 0xf8
+  __TEXT.__text: 0x3454
+  __TEXT.__objc_methlist: 0x1b4
+  __TEXT.__const: 0x302
+  __TEXT.__constg_swiftt: 0x90
+  __TEXT.__swift5_typeref: 0x76
+  __TEXT.__swift5_builtin: 0x28
+  __TEXT.__swift5_reflstr: 0x1b
+  __TEXT.__swift5_fieldmd: 0x2c
+  __TEXT.__swift5_assocty: 0x30
+  __TEXT.__swift5_proto: 0x24
+  __TEXT.__swift5_types: 0xc
+  __TEXT.__cstring: 0x564
+  __TEXT.__swift5_capture: 0x50
+  __TEXT.__oslogstring: 0x184
+  __TEXT.__unwind_info: 0x1b8
+  __TEXT.__eh_frame: 0x128
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
   __DATA_CONST.__const: 0x38
-  __DATA_CONST.__objc_classlist: 0x20
+  __DATA_CONST.__objc_classlist: 0x28
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0xb8
-  __DATA_CONST.__got: 0x88
-  __AUTH_CONST.__const: 0x70
-  __AUTH_CONST.__objc_const: 0x2f0
-  __AUTH_CONST.__auth_got: 0x310
-  __AUTH.__objc_data: 0x100
-  __AUTH.__data: 0xc0
-  __DATA.__data: 0x98
-  __DATA.__bss: 0x380
+  __DATA_CONST.__objc_selrefs: 0xd8
+  __DATA_CONST.__got: 0xa0
+  __AUTH_CONST.__const: 0xc0
+  __AUTH_CONST.__objc_const: 0x388
+  __AUTH_CONST.__auth_got: 0x328
+  __AUTH.__objc_data: 0x170
+  __AUTH.__data: 0xe8
+  __DATA.__data: 0xb0
+  __DATA.__bss: 0x480
   __DATA_DIRTY.__objc_data: 0x50
   __DATA_DIRTY.__data: 0x70
   - /System/Library/Frameworks/CoreFoundation.framework/CoreFoundation

   - /usr/lib/swift/libswiftXPC.dylib
   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswiftos.dylib
-  Functions: 103
-  Symbols:   157
-  CStrings:  42
+  Functions: 118
+  Symbols:   172
+  CStrings:  46
 
Symbols:
+ _OBJC_CLASS_$_SensorAccessDeviceState
+ _OBJC_METACLASS_$_SensorAccessDeviceState
+ __DATA_SensorAccessDeviceState
+ __INSTANCE_METHODS_SensorAccessDeviceState
+ __IVARS_SensorAccessDeviceState
+ __METACLASS_DATA_SensorAccessDeviceState
+ __PROPERTIES_SensorAccessDeviceState
+ _exclaves_device_state
+ _objc_msgSend$initWithState:
+ _objc_msgSend$state
+ _swift_getForeignTypeMetadata
+ _swift_retain_x19
+ _symbolic _____ So23exclaves_device_state_ta
+ _symbolic _____ s6UInt32V
+ _type_layout_string So23exclaves_device_state_ta
CStrings:
+ "Device state: %u"
+ "GetDeviceStateFailed"
+ "exclaves_device_state failed with code: %d"
+ "exclaves_device_state returned unrecognized value: %u"
```
