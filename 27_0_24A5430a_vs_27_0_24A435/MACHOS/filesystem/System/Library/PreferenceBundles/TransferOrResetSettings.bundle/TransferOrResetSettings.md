## TransferOrResetSettings

> `/System/Library/PreferenceBundles/TransferOrResetSettings.bundle/TransferOrResetSettings`

### Sections with Same Size but Changed Content

- `__TEXT.__const`
- `__TEXT.__constg_swiftt`
- `__TEXT.__swift5_typeref`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift_as_cont`
- `__TEXT.__unwind_info`
- `__TEXT.__eh_frame`
- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_intobj`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA_CONST.__auth_ptr`
- `__DATA.__objc_data`
- `__DATA.__data`

```diff

 2027.0.7.0.0
-  __TEXT.__text: 0xaf38
+  __TEXT.__text: 0xb448
   __TEXT.__auth_stubs: 0xa70
-  __TEXT.__objc_stubs: 0x1d80
-  __TEXT.__objc_methlist: 0x754
+  __TEXT.__objc_stubs: 0x1de0
+  __TEXT.__objc_methlist: 0x76c
   __TEXT.__const: 0x204
-  __TEXT.__objc_methname: 0x1f9a
+  __TEXT.__objc_methname: 0x208c
   __TEXT.__objc_classname: 0x1c7
-  __TEXT.__objc_methtype: 0x459
-  __TEXT.__cstring: 0xb46
-  __TEXT.__gcc_except_tab: 0x1b4
+  __TEXT.__objc_methtype: 0x475
+  __TEXT.__cstring: 0xc06
+  __TEXT.__gcc_except_tab: 0x20c
   __TEXT.__oslogstring: 0x63e
   __TEXT.__ustring: 0x1fa
   __TEXT.__dlopen_cstrs: 0x1b1

   __TEXT.__unwind_info: 0x308
   __TEXT.__eh_frame: 0x90
   __DATA_CONST.__const: 0x4c8
-  __DATA_CONST.__cfstring: 0x960
+  __DATA_CONST.__cfstring: 0x9a0
   __DATA_CONST.__objc_classlist: 0x50
   __DATA_CONST.__objc_protolist: 0x18
   __DATA_CONST.__objc_imageinfo: 0x8

   __DATA_CONST.__objc_arraydata: 0x10
   __DATA_CONST.__objc_arrayobj: 0x30
   __DATA_CONST.__auth_got: 0x548
-  __DATA_CONST.__got: 0x3e0
+  __DATA_CONST.__got: 0x3e8
   __DATA_CONST.__auth_ptr: 0x90
-  __DATA.__objc_const: 0xaf8
-  __DATA.__objc_selrefs: 0x9b0
-  __DATA.__objc_ivar: 0x4c
+  __DATA.__objc_const: 0xb28
+  __DATA.__objc_selrefs: 0x9d0
+  __DATA.__objc_ivar: 0x50
   __DATA.__objc_data: 0x280
   __DATA.__data: 0x2b8
   __DATA.__bss: 0x180

   - /System/Library/PrivateFrameworks/AppleAccountUI.framework/AppleAccountUI
   - /System/Library/PrivateFrameworks/AssistantServices.framework/AssistantServices
   - /System/Library/PrivateFrameworks/CMCapture.framework/CMCapture
+  - /System/Library/PrivateFrameworks/CRShipModeUI.framework/CRShipModeUI
   - /System/Library/PrivateFrameworks/CellularPlanManager.framework/CellularPlanManager
   - /System/Library/PrivateFrameworks/CoreFollowUp.framework/CoreFollowUp
   - /System/Library/PrivateFrameworks/CoreFollowUpUI.framework/CoreFollowUpUI

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 203
-  Symbols:   278
-  CStrings:  560
+  Functions: 206
+  Symbols:   279
+  CStrings:  569
 
Symbols:
+ _OBJC_CLASS_$_CRShipModeListController
CStrings:
+ "@\"CRShipModeListController\""
+ "Are you sure you want to delete all eSIMs? This cannot be undone.\nYour back SIM will be temporarily turned off and automatically turned back on after your eSIMs are deleted."
+ "Localizable-V63"
+ "T@\"CRShipModeListController\",&,N,V__prepareForShipmentListController"
+ "__prepareForShipmentListController"
+ "_prepareForShipmentListController"
+ "isEuiccActiveWithError:"
+ "set_prepareForShipmentListController:"
+ "supportsDynamicSIMConfigurationWithError:"
```
